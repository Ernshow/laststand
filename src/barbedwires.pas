...
var
    i, j, a: byte;
    x2, Y2: array[0..1] of single;
    X, Y: single;
    B: boolean;
    z: shortint;
begin
    for i := 1 to MAX_WIRES do
        if not wire[i].Active then
        begin
            a := i;
            break;
        end;
    ...
    for i := 1 to MAX_WIRES do
        if wire[i].Active then
            if Distance((wire[i].x1 + wire[i].x2) * 0.5, (wire[i].bundle[0].y +
                wire[i].bundle[1].y) * 0.5, X, Y) < 10.0 then
            begin
                if errors then
    ...
    begin
        ...
        begin
            X2[j] := X2[j] + 5 * z;
            if not RayCast(X2[j], Y2[j], X2[j] - 4 * z, Y2[j], True, False, False) then
                Y2[j] := Y2[j] - 2
            else
                B := True;
            if not B then if not RayCast(X2[j], Y2[j], X2[j] - 2 * z, Y2[j] + 2, True, False, False)
                then Y2[j] := Y2[j] + 4
                else
                    B := True;
            if not B then if not RayCast(X2[j], Y2[j], X2[j] - 2 * z, Y2[j] -
                    2, True, False, False) then
                begin
                    if errors then
    ...
    for i := 0 to 1 do
    begin
        if (not RayCast(X2[i], Y2[i], X2[i], Y2[i], False, False, False)) or
            (RayCast2(0, 2, 22, X2[i], Y2[i])) then
        begin
            if errors then
                WriteConsole(owner, 'Ground is too uneven to place a barbed wire', RED);
            exit;
    ...
    wire[a].c := wire[a].bundle[0].x * (wire[a].bundle[1].y - 13) -
        (wire[a].bundle[0].y - 13) * wire[a].bundle[1].x;
    wire[a].d := Sqrt(wire[a].a * wire[a].a + wire[a].b * wire[a].b);
    wire[a].owner := owner;
    wire[a].Active := True;
    wire[a].DestructionStage := 4;
    // there are 5 hp destruction stages, in each one one barb is removed (just cosmetics)
...
var
    i: integer;
begin
    if player[owner].task = 1 then
    begin
        if Players[owner].Alive then
        begin
            case Players_OnGround(owner, False, 15) of
                1:
                    if player[owner].Wires > 0 then
                    begin
                        ...
                        WriteConsole(owner, 'You don''t have any barbed wires left', RED);
                -1: WriteConsole(owner, 'You have to be on the solid ground to place a barbed wire',
                        RED);
                else
                    WriteConsole(owner, 'You have to be on the ground to place a barbed wire', RED);
            end;
        end
        else
            WriteConsole(owner, 'You are already dead', RED);
    end
    else
        WriteConsole(owner, 'You are not the mechanic', RED);
end;
...
begin
    ...
    begin
        ...
        if wire[i].hp <= 0 then
        begin
            if wire[i].DestructionStage = 0 then
            begin
                Wires_ClearWire(i);
            ...
            begin
                ...
                wire[i].LastDamagedBundle := not wire[i].LastDamagedBundle;
                // once destroy a barb from one bundle, once from the second
                if wire[i].LastDamagedBundle then j := 0
                else
                    j := 1;
                if not effect then
                begin
                    for b := 0 to 2 do // spawn active barbs of the wire
                        CreateBulletX(wire[i].bundle[j].x, wire[i].bundle[j].y - 4,
                            RandFlt(-3.0, 3.0), RandFlt(-3.0, -1.0), 0, 7, wire[i].owner);
                    effect := True;
                end;
...
begin
    ...
    begin
        ...
        begin
            ...
            for j := 1 to MaxID do
            begin// hurt all zombies in wire range
                p := Players[j];
                if p.Alive then
                    ...
                    begin
                        if p.x <= wire[i].x1 then
                            if p.x >= wire[i].x2 then
                                if Abs(wire[i].a * p.x + wire[i].b * p.y + wire[i].c) / wire[i].d < 25 then
                                begin
                                    if n[j] = 0 then
                                    begin
                                        ZombieOn := True;
            ...
            begin
                if ZombieOn then
                begin // if some zombie was in wire range
                    Wires_Damage(i, 1);
                    ZombieOn := False;
                end;
                wire[i].t := wire[i].t + 1;
                ...
                for j := 0 to 1 do // spawn active barbs of the wire
                    if wire[i].bundle[j].barb[wire[i].t] then
                        CreateBulletX(wire[i].bundle[j].x, wire[i].bundle[j].y,
                            wire[i].t * 0.25 - 0.50, 0.5, 0, 7, wire[i].owner);
            end;
        end;
    end;
    for j := 1 to MaxID do
...
begin
    ...
    begin
        if ID = wire[i].owner then
            if wire[i].active then
            begin
                d := SqrDist(X, Y, (wire[i].x1 + wire[i].x2) * 0.5,
                    (wire[i].bundle[0].y + wire[i].bundle[1].y) * 0.5);
                if d < max then
                begin
                    max := d;
                    Result := i;
    ...
    if Result > 0 then
    begin
        hp := (single(integer(wire[Result].hp)) +
            single(integer(wire[Result].DestructionStage)) * WIREHITPOINTS / 5.0) / WIREHITPOINTS;
        BigText_DrawMap(ID,
            'Wire: ' + IntToStr(Round(hp * 100.0)) + '%',
            180, $88000000 + RG_Gradient(hp, 0.5), 0.04, Trunc(wire[Result].bundle[1].X) - 5,
            Trunc((wire[Result].bundle[0].Y + wire[Result].bundle[1].Y) * 0.5 + 3.0)
            );
    end;
end;
