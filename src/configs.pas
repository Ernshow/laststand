...
const
    ...
    WEAPONSINI = 'weapons_realistic';
    //default, path to used weapons.ini, if not set in config
...
const
    DEFAULT_END_TEXT = 'GAME OVER';

procedure Config_ApplyMapSettings(Map: string);
var
    str: string;
    changed, FoundKey, Enabled: boolean;
begin

    if not LSMap.Settings.Loaded then
    begin
...
begin
    ...
    if FileExists(LSMap.Settings.Path) then
    begin
    ...
    begin
        if INI_Load(ini, ConfigPath) then
        begin
            try
                Config.Mode[1].Maxplayers :=
                    StrToInt(INI_Get(ini, 'MAXPLAYERS', 'SurvivalHard', '', b));
            except
                INI_Set(ini, 'MAXPLAYERS', 'SurvivalHard', IntToStr(MAXPLAYERS_SURVIVAL), changed);
            end;
            try
                Config.Mode[2].Maxplayers :=
                    StrToInt(INI_Get(ini, 'MAXPLAYERS', 'SurvivalVeteran', '', b));
            except
            ...
            try
                Config.Mode[4].Maxplayers :=
                    StrToInt(INI_Get(ini, 'MAXPLAYERS', 'Infection', '', b));
            except
            ...
            Config.Mode[1].WeaponSys :=
                INI_Get(ini, 'WEAPON_SYSTEM', 'SurvivalHard', '0', b) = '1';
            ...
            Config.Mode[1].WeaponPath :=
                INI_Get(ini, 'WEAPON_PATH', 'SurvivalHard', WEAPONSINI, b);
    ...
    begin
        WriteFile(ConfigPath,
            '[GENERAL]' + #13#10 + 'DefaultMode=1' + #13#10 + 'ForceDefaultMode=0' +
            #13#10 + #13#10 + '[MODE_ENABLED]' + #13#10 + 'Survival=1' +
            #13#10 + 'SurvivalHard=1' + #13#10 + 'Versus=1' + #13#10 +
            'Infection=1' + #13#10 + #13#10 + '[MAXPLAYERS]' + #13#10 +
            'SurvivalHard=' + IntToStr(MAXPLAYERS_SURVIVAL) + #13#10 +
            'SurvivalVeteran=' + IntToStr(MAXPLAYERS_VETERAN) + #13#10 +
            'Versus=' + IntToStr(MAXPLAYERS_VERSUS) + #13#10 + 'Infection=' +
            IntToStr(MAXPLAYERS_INFECTION) + #13#10 + 'Butchery=' +
            IntToStr(MAXPLAYERS_BUTCHERY) + #13#10 + #13#10 + '[WEAPON_SYSTEM]' +
            #13#10 + 'Survival=0' + #13#10 + 'SurvivalHard=1' + #13#10 +
            'Versus=0' + #13#10 + 'Infection=0' + #13#10 + 'Butchery=0'
            );
        CreateFile := False;
    end;
...
begin
    ...
    if FileExists(News.FilePath) then
    begin
        INI_Clear(ini);
        if INI_Load(ini, News.FilePath) then
        begin
            try
            ...
            repeat
                ...
                if Text = '' then exit;
                try
                    News.Msg[n].Color := StrToInt(INI_Get(ini, 'TEXT' +
                        IntToStr(n + 1), 'Color', '$FFFFFF', b));
                except
                    News.Msg[n].Color := WHITE;
                end;
                News.Msg[n].Style := StrToIntDef(INI_Get(ini, 'TEXT' +
                    IntToStr(n + 1), 'Type', '1', b), 1);
    ...
    if CreateFile then
        WriteFile(News.FilePath,
            '[NEWS]' + #13#10 + 'NumberOfNews=1' + #13#10 + 'Time=360' +
            #13#10 + #13#10 + '[TEXT1]' + #13#10 + 'Text=' + #13#10 +
            'Color=$FFFFFF' + #13#10 + 'Type=1'
            );
end;
