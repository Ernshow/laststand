...
begin
    ...
    WriteConsole(statgun[SG.Num].owner, 'Statgun complete! ' + IntToStr(
        player[statgun[SG.Num].owner].statguns) + ' statguns left.', GREEN);
...
begin
    ...
    begin
        ...
        begin
            ...
            WriteConsole(statgun[SG.Num].owner, 'Statgun retrieved! ' + IntToStr(
                player[statgun[SG.Num].owner].statguns) + ' statguns available.', GREEN);
        end;
        for a := ID to SG.Num - 1 do
        begin
...
begin
    ...
    for i := 1 to MAX_STATS do
        if statgun[i].reference > 0 then
            if Map.Objects[statgun[i].reference].Active then
            begin
                Map.Objects[statgun[i].reference].Kill();
                statgun[i].reference := Objects_Spawn(Round(statgun[i].X),
                    Round(statgun[i].Y) - 15, 27);
            end;
end;

procedure Statguns_ProcessConstruction();
begin
    if (SG.BuildTimer > 1) or (SG.BuildTimer < -1) then
    begin
        if SG.BuildTimer > 1 then
        begin
            if player[statgun[SG.Num].owner].Frozen then exit;
            if IsInRange(statgun[SG.Num].owner, statgun[SG.Num].X,
                statgun[SG.Num].Y, 25, False) then
            begin
                SG.BuildTimer := SG.BuildTimer - 1;
                BigText_DrawScreenX(DTL_COUNTDOWN, statgun[SG.Num].owner,
                    'Construction [' + IntToStr(SG.BuildTimer - 1) + ']', 100, DT_CONSTRUCTION, 0.08, 20, 370);
                if SG.BuildTimer > 4 then if (SG.BuildTimer mod 3 = 0) or (RandInt(1, 5) = 1) then
                        CreateBulletX(statgun[SG.Num].X, statgun[SG.Num].Y,
                            RandInt(20, 100) / 80 * iif(SG.BuildTimer mod 2 = 0, 1, -1), -1.3, 0, 7,
                            statgun[SG.Num].owner);
            end
            else
            begin
                BigText_DrawScreenX(DTL_COUNTDOWN, statgun[SG.Num].owner,
                    'Construction failed', 100, DT_FAIL, 0.08, 20, 370);
                SG.BuildTimer := 0;
            end;
        end
        else
        ...
        begin
            ...
            if IsInRange(statgun[SG.RetrieveNum].owner, statgun[SG.RetrieveNum].X,
                statgun[SG.RetrieveNum].Y, 25, False) then
            begin
                SG.BuildTimer := SG.BuildTimer + 1;
                BigText_DrawScreenX(DTL_COUNTDOWN, statgun[SG.RetrieveNum].owner,
                    'Deconstruction [' + IntToStr(-SG.BuildTimer - 1) + ']', 100, DT_CONSTRUCTION, 0.08, 20, 370);
            end
            else
            begin
...
var
    i: integer;
begin
    if player[ID].Task = 1 then
    begin
        if player[ID].statguns > 0 then
        begin
            ...
            begin
                ...
                WriteConsole(ID, 'You must be on the ground to build a statgun', RED);
        end
        else
            WriteConsole(ID, 'You do not have a statgun', RED);
    end
    else
