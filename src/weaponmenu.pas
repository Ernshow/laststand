...
type
    WeaponMenu_WeaponChoiceCallback = procedure(ID, Pri, Sec: byte);
...
procedure WeaponMenu_Init(WeaponTakenCallback: WeaponMenu_WeaponChoiceCallback;
    OnTickInterval: integer);
...
begin
    ...
    for j := 1 to 14 do
    begin
        if (WMP[ID].Weapons[i].Refreshn > 0) then
        begin
...
begin
    ...
    if (menu_weap >= 1) and (menu_weap <= 10) then
    begin
        if WMP[ID].Weapons[menu_weap].Enabled then
        begin
            Result := weap;
        end
        else
    ...
    if (menu_weap >= 11) and (menu_weap <= 14) then
    begin
        if WMP[ID].Weapons[menu_weap].Enabled then
        begin
            Result := weap;
...
begin
    ...
    begin
        ...
        for i := 1 to MaxID do
        begin
            if WMP[i].Updating then
            begin
                WMP[i].Updating := WeaponMenu_UpdateCycle(i);
            end;
            if WMP[i].GettingWeapon then
            begin
                if WMP[i].Respawning then
                begin
                    WMP[i].Respawning := False;
                    WMP[i].RX := Players[i].X;
                end
                else
                if Abs(WMP[i].RX - Players[i].X) > 33.0 then
                begin
...
begin
    ...
    if WMP[ID].GettingWeapon then
    begin
        // Protection against throwing weapon on spawn
        if (Primary = WTYPE_NOWEAPON) then
        begin
            if (Players[ID].Secondary.WType = Secondary) and
                (Players[ID].Primary.WType <> WTYPE_NOWEAPON) then
            begin
                //Kill the weapon he dropped
                for i := 1 to MAXOBJ do
                    if Map.Objects[i].Active then
                        if Distance(Map.Objects[i].X, Map.Objects[i].Y, Players[ID].X,
                            Players[ID].Y) <= 50.0 then
                            try
                ...
                //Give it back to him
                Weapons_Force(ID, Players[ID].Primary.Wtype, Secondary,
