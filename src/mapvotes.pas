...
var
    ...
    str: string;
begin
    if (not VOTEROUND) and (GameRunning) then
    begin
    ...
    // vote id = map id + 1
    if ((voteID <= MapList.Length) and (voteID >= 0)) or (voteID = VOTE_RANDOM) then
    begin
        if voteID = 0 then
        begin
            a := LSMap.CurrentNum;
        ...
        if (player[ID].VoteReady) then
            ...
            str := Players[ID].Name + ' has voted for ';

        player[ID].VoteReady := True;
        Player[ID].Autovote := Player[ID].StoreAutoVote;

        if player[ID].VotedMap = LSMap.CurrentNum then
            str := str + 'the current map [' + IntToStr(MapVotes_VotedNum()) +
                '/' + IntToStr(Players_HumanNum()) + ']'
        else if voteID = VOTE_RANDOM then
            str := str + 'a random map [' + IntToStr(MapVotes_VotedNum()) +
                '/' + IntToStr(Players_HumanNum()) + ']'
        else
            str := str + MapList.List[player[ID].VotedMap] + '(' + IntToStr(voteID) + ') [' +
        ...
        if MapVotes_VotedNum() = 1 then
            for a := 1 to MaxID do
                if a <> ID then
                begin
                    WriteConsole(a, 'Type /vote to vote for starting the game on the current map',
                        GREEN);
                    WriteConsole(a, 'Use /votehelp for more information, /list for a list of votes',
                        GREEN);
                end;
        CheckVotes := True;
    end
    else
        WriteConsole(ID, 'This vote option does not exist, use numbers 1-' +
            IntToStr(MapList.Length), GREEN);
end;
...
begin
    ...
    SetLength(votedOption, MapList.Length + 1);
    //Last entry if current map isn't on the list
    ...
    for i := 1 to MAX_UNITS do
        if Players[i].Active then
            ...
            begin
                ...
                if Player[i].VotedMap = -1 then votedOption[MapList.Length] :=
                        votedOption[MapList.Length] + 1
                else
                    votedOption[player[i].VotedMap] := votedOption[player[i].VotedMap] + 1;
            end;
    ...
    begin
        ...
        else if votedOption[i] = x then
        begin
            occ[n] := i;
            n := n + 1;
        end;
    end;
    if n <= 1 then choosenVote := h
    else
        choosenVote := occ[RandInt(0, n - 1)];
    WriteDebug(5, 'Vote result: ' + IntToStr(choosenVote));

    if choosenVote = LSMap.CurrentNum then
    begin
        WriteConsole(0, 'Vote successful', GREEN);
        if (TimeLeft = 0) or (TimeLeft >= 60 * MAPSWITCHTIME) then
        begin
    ...
    begin
        ...
        MapchangeStart := True;
        PlayerLeft := False;
        WriteConsole(0, 'Vote successful, changing the map to ' +
            MapList.List[choosenVote], GREEN);
    end;
    for i := 1 to MAX_UNITS do
        player[i].VoteReady := False;
end;
