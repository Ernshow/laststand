...
const
    ...
    // global difficulty percent, influences on all difficulty levels;
    ...
    EQUALIZATIONDIFF = 120;
    // equalization of difficulty between all modes (don't touch);
    MAPSWITCHTIME = 90;
    ...
    VSPOINTSFORHEALTH = 15;
    // maximal points earned by humans for not getting damaged in a wave (pts = VSPOINTSFORHEALTH * healthleft/healthmax)
    SPECIALWAVEMP = 2.5; // multiplier of VSPOINTSFORWAVE for special waves
    SHOWPLAYERCOMMANDS = True;

    SUPPLYTIME = 11;
    // basic supply time (supply time = SUPPLYTIME + (SUPPLYTIME2 / NumberOfPlayers))
    SUPPLYTIME2 = 4;
    // divided by number of players (supply time = SUPPLYTIME + (SUPPLYTIME2 / NumberOfPlayers))
