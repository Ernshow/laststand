...
type
    ...
    tLSPlayer = record
        // Basic player info
        Status: shortint;
        // -2: dead-zombie-player; -1 zombie-player; 0: not-playing/zombie-bot; 1: alive-player; 2: temp-infected-player;
        Participant: shortint;
        // -1: zombie team (human player); 0: bot or something; 1: human team.

        // Equipment
        Waves, mre, Mines, Charges, Wires, Sentrys, SentryAmmo, Molotovs,
        Statguns, Scarecrows, HolyWater: word;
        ...
        FlakTime: cardinal;
        HurtTime, TicksAtSpawn, BossPlayTime, ZombiePlayTime, RespawnTime,
        ZombChatTime: longint;
        ...
        bitten, voted, justjoined, kicked, ReloadMode, AutoVote, StoreAutoVote,
        ShowTaskinfo, played,
...
function Players_OnGround(ID: byte; RC_CheckPlayerOnlyCollide: boolean;
    range: single): smallint;
...
begin
    ...
    if (not RC_CheckPlayerOnlyCollide) then
    begin
        GetPlayerXY(ID, x, y);
        if Raycast(x, y, x, y + range, False, False, False) then Result := -1
        else
            Result := 1;
    end
    else
    begin
        if Players[ID].OnGround then Result := 1
        else
        begin
            GetPlayerXY(ID, x, y);
