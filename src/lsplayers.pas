...
type
	...
	tLSPlayer = record
		// Basic player info
		{ Status =
			-2: dead-zombie-player;
			-1: zombie-player;
			0: not-playing/zombie-bot;
			1: alive-player;
			2: temp-infected-player;
		}
		Status: shortint;
		{ Participant =
			-1: zombie team (player);
			0: bot or something;
			1: survivor team (player);
		}
		Participant: shortint;

		// Amount of waves the player has lived for??
		Waves: word;

		// Equipment
		mre, Mines, Charges, Wires, Sentrys,
			SentryAmmo, Molotovs, Statguns, Scarecrows, HolyWater: word;
		...
		VoteReady, ModeReady, JustResp, GetX, Zombie, frozen, AttackReady,
			Respawned, AZSS_DontObserve, bitten, voted, justjoined, kicked,
			ReloadMode, AutoVote, StoreAutoVote, ShowTaskinfo, played,
...
const
	STATUS_PLAYER_DEAD_ZOMBIE 			= -2;
	STATUS_PLAYER_ALIVE_ZOMBIE 			= -1;
	STATUS_BOT_ZOMBIE					 			=  0;
	STATUS_SPECTATOR					 			=  0;
	STATUS_PLAYER_ALIVE_SURVIVOR		=  1;
	STATUS_PLAYER_INFECTED_SURVIVOR	=  2;
