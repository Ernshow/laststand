...
procedure BaseWeapons_Add(Weap: byte; Num: integer; Owner: byte);
//Owner should not be told of "new weapon in base" -> call with 0 if no owner
...
begin
    ...
    begin
        ...
        begin
            if (WeaponMenu_IsGettingWeapon(j)) or (player[j].Status = 2) then
            begin
                for i := 1 to 14 do
                begin
...
begin
    ...
    begin
        for i := 1 to MaxID do
            if player[i].Status = 1 then
                if player[i].TempActiveWeapons[Weap] then
                begin
                    WeaponMenu_SwitchWeapon(i, Weap, True);
                    if i <> Owner then
                        if was_zero then
                            if not WeaponMenu_IsGettingWeapon(i) then
                                WriteConsole(i, 'New weapon (' + WeaponName(menu2weap(Weap)) +
                                    ') in base, go to the base flag to /get it', NWSCOL);
                end;
    end
    else
    if Weapon[Weap].Num <= 0 then
        for i := 1 to MaxID do // update if someone has menu open at the moment
            if player[i].Status = 1 then
                if not player[i].TempActiveWeapons[Weap] then
                begin
                        ...
                        begin
                            ...
                            begin
                                ...
                                begin
                                    ...
                                    WriteConsole(ID, WeaponName(Players[ID].Primary.WType) +
...
begin
    ...
    begin
        ...
        BigText_DrawScreenX(DTL_WEAPONLIST, ID, str, 150, NWSCOL, 0.08 -
            0.002 * t, 20, 370 - 10 * t);
    end;
end;
...
begin
    for i := 1 to MAX_OBJECTS do
        if Map.Objects[i].Active then
            if Distance(Map.Objects[i].x, Map.Objects[i].y, Base.X, Base.Y) <= BASE_RANGE then
            begin
                //Is it a weapon?
...
begin
    ...
    if Player[ID].ActiveWeapons[15] then
    begin
        if (Players[ID].Secondary.WType = WTYPE_NOWEAPON) then
        begin
            Weapons_Force(ID, Players[ID].Primary.WType, WTYPE_FLAMER,
                Players[ID].Primary.Ammo, 255);
        end
        else
        if (Players[ID].Primary.WType = WTYPE_NOWEAPON) then
        begin
            Weapons_Force(ID, WTYPE_FLAMER, Players[ID].Secondary.WType, 255,
                players[ID].Secondary.Ammo);
        end
        else
        if (Players[ID].Secondary.WType = WTYPE_USSOCOM) then
        begin
            Weapons_Force(ID, Players[ID].Primary.WType, WTYPE_FLAMER,
                Players[ID].Primary.Ammo, 255);
        end;
    ...
    begin
        pr := False;
        if (Primary <> WTYPE_NOWEAPON) then
        begin
        ...
        sr := False;
        if (Secondary <> WTYPE_NOWEAPON) then
        begin
        ...
        begin
            ...
            begin
                WriteConsole(Cop.ID, TaskToName(Player[ID].Task, False) + ' ' +
                    Players[ID].Name + ' has run out of weapons!', NWSCOL);
            end;
            str := 'Type "/buy';
            for i := 1 to 10 do
                if player[ID].ActiveWeapons[i] then
...
begin
    if IsInRange(ID, Base.X, Base.Y, 110, False) then
        if not Player[ID].JustResp then
        begin
            b := Weapons_IsRegular(Players[ID].Primary.Wtype) and
                (Players[ID].Primary.Wtype <> WTYPE_MINIGUN);
            c := Weapons_IsRegular(Players[ID].Secondary.Wtype) and
                (Players[ID].Secondary.Wtype <> WTYPE_MINIGUN);
            if (b) or (c) then
                WriteConsole(ID, iif(b, WeaponName(Players[ID].Primary.WType), '') +
                    iif((b) and (c), ' and ', '') + iif(c, WeaponName(Players[ID].Secondary.WType),
                    '') + ' put back into base.', NWSCOL);
            if b then
                BaseWeapons_Add(weap2menu(Players[ID].Primary.WType), 1, ID);
            if c then
...
procedure BaseWeapons_DrawWeaponList(ID: byte; AdditionalInfo: boolean;
    X, Y, incr: integer);
var
    str, str2: string;
    i, t: integer;
...
begin
    ...
    begin
        ...
        if (InBase) or (WeaponMenu_IsGettingWeapon(ID)) then
            if not ((ID = Cop.ID) and (Cop.Shop.Active)) then
            begin
                if InBase then
                begin
