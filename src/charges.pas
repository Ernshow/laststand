...
procedure Expl(x, y: single; step: word;
    explosion_angle, direction_angle, min_penetration: single; var amount: byte;
    var vec: array of tTVector);
var
    ang, ang2, ang3, angle, vx, vy, rc, h, high: single;
    extend_array, element_used: boolean;
    i, j, arrheight, index, n: byte;
begin
    // step 1: the first ring
    angle := ANGLE_60;
    n := Trunc(explosion_angle / angle + 0.9);
    // get the number of branches in based on explosion_angle
    ...
    begin
      ...
      {$endif}
        ...
        high := -1;
        for j := 0 to 5 do
        begin // find the best position around this angle
            vx := cos(ang3) * step;
            vy := sin(ang3) * step;
            rc := RayCast3(x, y, x + vx, y + vy, 5, 5); // mearsure how much the point is visible
            h := rc - Abs(ang2 - ang3) / angle / 2;
            if rc >= min_penetration then
            begin
                if h > high then
                begin // pick the best vector
                    high := h;
                    vec[index].vx := vx;
                    vec[index].vy := vy;
                    vec[index].X := x + vx;
...
procedure Charge_Explosion(x, y: single; step: word; amount, owner: byte);
var
    vec: array of tTVector;
    vec2: array of tTVector;
    i, j, l: smallint;
    rd: single;
    s: byte;
    elements: tStack8;
begin
    ...
    Expl(x, y, step, ANG_2PI, 0, 0.39, amount, vec);
    // first step, get vectors through reference
    l := Length(vec);
    for i := 0 to l - 1 do
    begin // extend explosion in all directions, get more vectors through reference
        if not RayCast(vec[i].X, vec[i].Y, vec[i].X, vec[i].Y + 10, True, False, False) then
            s := 4
        else
        if RandBool then s := 4
        else
            s := 2;
        CreateBulletX(vec[i].X, vec[i].Y, 0, 0, 50, s, owner);
        Expl(vec[i].X, vec[i].Y, step, ANG_2PI / 6, Math.arctan2(vec[i].vy, vec[i].vx),
            0.59, amount, vec2);
        if amount = 0 then exit; // if reached the bullet limit then exit;
    end;
    l := 0;
    for i := 0 to Length(vec2) - 1 do
    begin
        CreateBulletX(vec2[i].X, vec2[i].Y, 0, 0, 50, 4, owner);
        if amount = 0 then exit;
        rd := Math.arctan2(vec2[i].vy, vec2[i].vx);
        if (Abs(rd) < ANGLE_45) or (Abs(pi - rd) < ANGLE_45) then
        begin
...
begin
    ...
    for i := 0 to 1 do
    begin
        BigText_DrawMapX(WTL_CHARGES + CID, charge[CID].owner, '[' + IntToStr(CID + 1) +
            ']', 99999999, col, 0.04, Trunc(charge[CID].X + CHAR_MARK_X_OFFSET),
            Trunc(charge[CID].Y));
    end;
end;
...
begin
    ...
    if choice >= 0 then
        if choice <> player[ID].DetonationQueue.arr[player[ID].DetonationQueue.length - 1] then
        begin
...
begin
    ...
    begin
        case Players_OnGround(ID, False, 20) of
            1: if (player[ID].task = 2) then
                begin
                    if (player[ID].charges > 0) then
                    begin
                        if (_remote) or ((chargeTime >= 1) and (chargeTime <= 60)) then
                        begin
                            ...
                            begin
                                ...
                                begin // lower it due to latency of cluster bomb explosion
                                    ...
                                    for i := 1 to MaxID do
                                        if Players[i].Alive then
                                            if Players[i].Human then
                                                if i <> ID then
                                                    WriteConsole(i, 'Demolition Expert ' + Players[ID].Name +
                                                        ' has placed a remote charge', GREEN)
                                                else
                                                    WriteConsole(i, 'Charge with ID ' + IntToStr(found) +
                                                        ' placed. Charges in game: ' + Charges_InGameInfo(-1), GREEN);
                                    WriteConsole(ID, 'Type /act (Default Alt+4) to detonate the last charge(s)',
                                        GREEN);
                                    WriteConsole(ID, IntToStr(player[ID].charges) + ' charges left', GREEN);
                                end
                                else
                                    WriteConsole(ID, 'Time charge placed (' + IntToStr(player[ID].charges) +
                                        ' left)! Get away from it!', GREEN);
                            end
                            else
                                WriteConsole(ID, 'Maximal number of charges in game exceeded', RED);
                        end
                        else
                            WriteConsole(ID, 'Invalid time argument, use values 1-60', RED);
                    end
                    else
                        WriteConsole(ID, 'You do not have any charges left', RED);
                end
                else
            ...
            -1: WriteConsole(ID, 'You have to be on the solid ground to place a charge', RED);
                ...
                WriteConsole(ID, 'You have to be on the ground to place a charge', RED);
        end;
    end
    else
        WriteConsole(ID, 'You have to be alive to place a charge', RED);
end;
...
var
    a, i, n: byte;
begin
    for a := 0 to Charges_MaxID do
    begin
        if charge[a].placed then
        begin
            if not charge[a].remote then
            begin
                BigText_DrawMapX(WTL_CHARGES + a, charge[a].owner, IntToStr(charge[a].timer + 1),
                    80, YELLOW, 0.04, Trunc(charge[a].X) - 5, Trunc(charge[a].Y));
                charge[a].timer := charge[a].timer - 1;
            end;
            if charge[a].timer = 0 then
            begin
                if charge[a].remote then
                begin
                    if n < 2 then n := n + 1
                    else
                        continue;
                    ...
                    for i := 0 to 3 do
                        CreateBulletX(charge[a].X, charge[a].Y, RandFlt(-7, 7), -RandFlt(3, 4),
                            99, 10, charge[a].owner);
                    Damage_ZombiesAreaDamage(charge[a].owner, charge[a].X, charge[a].Y,
                        40, 175, 4000, Explosion);
                    Charges_Unmark(a);
                end
                else
                begin
                ...
                if a = Charges_MaxID then
                begin
                    while Charges_MaxID > 0 do
                    begin
...
begin
    ...
    if player[ID].DetonationsNum >= 5 then
    begin
        WriteConsole(ID, 'HINT: Charge detonation:', HINTCOL);
        WriteConsole(ID,
            '"/act 0" to detonate all remote charges, "/act ID" for a charge with specified ID',
            HINTCOL);
        WriteConsole(ID, 'You can also point a charge with your mouse to make it detonate next.',
            HINTCOL);
...
const
    ERR_NORCH = 'There are no remote charges placed!';
...
begin
    ...
    begin
        ...
        if a = $FF then
        begin
            if player[ID].DetonationQueue.length > 0 then
            begin
        ...
        if (charge[a].placed) and (charge[a].remote) then
        begin
            charge[a].timer := 0;
            WriteConsole(ID, 'Charge[' + IntToStr(a + 1) + '] activated! Placed charges left: ' +
                Charges_InGameInfo(a), GREEN);
        end
        else
            WriteConsole(ID, 'There is no charge with ID ' + IntToStr(a + 1), RED);
    end;
end;
...
begin
    ...
    if owner > 0 then
        if Players[owner].Active then
        begin
            for i := 0 to player[owner].DetonationQueue.length - 1 do
                if a = player[owner].DetonationQueue.arr[i] then
                begin
