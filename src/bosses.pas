	...
	if ForcedPower > 0 then begin
		...
		if (Players[Boss.ID].Human) then begin
			case Pwr.InUse of
				...
				91: Pwr.CountDown := 1;
				92: Pwr.CountDown := 4;
				100: Pwr.CountDown := 5; //Butcher parts
...
procedure ButcherDeath(Victim: TActivePlayer);
var
	i: integer;
begin
	//WriteConsole(0,'[DEBUG] Butcher death invoked',BLUE);
	for i := 1 to 6 do
		Zombies_SpawnOne(
	...
	if (Victim.Human) then begin
		if (not PointNotInPoly(Victim.X, Victim.Y + 10, false, true, true)) then CreateBulletX(Victim.X, Victim.Y + 10, 0, 0, 0, 4, Victim.ID);
		//InfectedDeath(Victim.ID);
		Damage_DoAbsolute(Victim.ID, Victim.ID, Victim.Health);
	end;
end;
...
begin
	if Boss.Intro = 0 then begin
		...
		for i := 0 to SummonMinionsBots-1 do begin
			...
			BallisticCast(X2, Y2, -G_BUL*Game.Gravity/(LSMap.Gravity + 0.0001), vec, true, true);
...
begin
	case PwID of
		...
		//Butcher
		//procedure Bosses_OnZombieKill(Killer, Victim: TActivePlayer; BulletID: byte);
		100: Bosses_OnZombieKill(Players[Victim], Players[Shooter], 1);
	end;
end;
...
begin
	...
	case Power1 of
		...
		100: case Power2 of //Split to parts
			100: Result := true;
		end;
	end;
end;
...
case SpellID of
		...
		61: begin
					for i := 1 to MaxId do
						if ((Players[i].Alive) and (player[i].Status = STATUS_PLAYER_ALIVE_SURVIVOR))
								or (i = Scarecrow.ID)
								or (i = Sentry.ID) then
							if SqrDist(
					...
					for i := 1 to MAX_WIRES do
						if wire[i].Active then
							if SqrDist(
								Players[ID].X, Players[ID].y,
								(wire[i].bundle[0].x + wire[i].bundle[1].x) / 2,
								(wire[i].bundle[0].y + wire[i].bundle[1].y) / 2
							) <= FF_HEATRANGE then begin
								Result := True; Exit;
							end;
					for i := 1 to (SG.Num - 1) do
						if SqrDist(
							Players[ID].X, Players[ID].y,
							statgun[i].x, statgun[i].y
						) <= FF_HEATRANGE then
						begin
							Result := True; Exit;
						end;
		...
		100: Result := false; //Butcher parts
	end;
end;
...
begin
	Result := false;
	// WriteDebug(6, 'ManualCheckSpell(' + Show(SpellID) + ')');
	case SpellID of
		//Perished priest spells
		51: Result := GetKeyPress(Boss.ID, 'Grenade'); //Summon Altar
		//Fire fighter spells
		61: begin
			Result := GetKeyPress(Boss.ID, 'Shoot'); //Small heat
			// WriteDebug(6, 'Boss is pressing SHOOT? -> ' + Show(Result));
		end;
		62: begin
			Result := GetKeyPress(Boss.ID, 'Reload');  //Portal
			// WriteDebug(6, 'Boss is pressing RELOAD? -> ' + Show(Result));
		end;
		...
		100: Result := GetKeyPress(Boss.ID, 'Grenade'); //Split to parts
	end;
end;
...
begin
	...
	case SpellID of
		...
		91: Result := true;
		92: Result := true;
		100: begin //Butcher parts
			if players[Boss.ID].Health <= Trunc(MAXHEALTH*1/4) then begin
				Result := true
			end;
		end;
	end;
...
begin
	if (Boss.bID = 0) or (Boss.ID = 0) then exit;
	//be sure that Boss.ID > 0 first
	if not Players[Boss.ID].Alive then exit;

	//Process passive powers
	case Boss.bID of
		FIREFIGHTER_ID: begin
			ProcessTrap();
			ProcessTrail();
		end;
		PLAGUE_ID: ProcessPlagueMinions();
	end;
	...
	if DoBrain then begin
		if player[Boss.ID].bitten then begin
			...
			begin
				if HackermanMode then begin
					BigText_DrawMap(0,
						IntTostr(RandInt_(1)),
						RandInt(40, 60), $22FF22, RandFlt(0.02, 0.05),
						trunc(Players[Boss.ID].X) + RandInt(-10, 10),
				...
				BigText_DrawScreenX(
					DTL_ZOMBIE_UI, 0, s, 120,
					RG_Gradient(players[Boss.ID].Health / MAXHEALTH, 0.9),
					0.08, 20, 370
				);
			end;
		end;
			...
			begin
				...
				begin
					if Players[i].Active
						...
						and ((not AutoBrain) or BrainCheckSpell(min, i))
						and CheckSpell(min, i, Boss.Power[slot]) then
					begin
		...
		if Length(SpellQueue) > 0 then begin //TODO: spell prioritizing
			...
			WriteDebug(6, 'Casting Spell: ' + PowerNumToName(SpellQueue[i].SpellID));
