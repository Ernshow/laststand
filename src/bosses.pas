...
const
    ...
    ExplodeWarnings = 2;
    //How many times flame half-circle is created around satan before he explodes
    ...
    ArtifactKillPhrase = 'NOOOOOOOOO!!!';
    //What bot says while Artifact dies (fireworks begins)
...
procedure PlagueOnMinionDamage(ID: byte; Damage: single; weap: byte;
...
begin
    ...
    case player[t].task of
        1, 4: if player[t].mre > 0 then
            begin
                player[t].mre := player[t].mre - 1;
                WriteConsole(0, 'Zombie has ' + verb + 'a meal ready to eat from the ' +
                    taskToName(player[t].task, False), RED);
            end;
        2: if player[t].Mines >= 2 then
            begin
                j := RandInt(1, 2);
                player[t].Mines := player[t].Mines - j;
                WriteConsole(0, 'Zombie has ' + verb + iif(j = 1, 'a mine', '2 mines') +
                    ' from the demolition expert', RED);
            end
            else if player[t].charges > 0 then
            begin
                player[t].charges := player[t].charges - 1;
                WriteConsole(0, 'Zombie has ' + verb + 'a charge from the demolition expert', RED);
            end;
        5: if player[t].molotovs > 0 then
            begin
                player[t].molotovs := player[t].molotovs - 1;
                WriteConsole(0, 'Zombie has ' + verb + 'a molotov from the sharpshooter', RED);
            end;
        6: if Cop.SupplyPoints >= 3 then
            begin
                j := Trunc(Cop.SupplyPoints) div 3;
                if j > 10 then j := 10;
                Cop.SupplyPoints := Cop.SupplyPoints - j;
                WriteConsole(0, 'Zombie has ' + verb + IntToStr(j) +
                    ' supply points from the police officer', RED);
            end;
        7: if player[t].holyWater >= 100 then
            begin
                j := ToRangeI(0, ToRangeI(RandInt(50, 60), player[t].holywater div 2,
                    RandInt(50, 160)), player[t].holyWater - 30);
                player[t].holyWater := player[t].holyWater - j;
                WriteConsole(0, 'Zombie has ' + verb + IntToStr(j) +
                    ' ml of holy water from the priest', RED);
            end;
    end;
end;
...
begin
    ...
    begin
        if Boss.Power[i].InUse = 72 then
        begin
            if Boss.Power[i].Victim > 0 then
            begin
...
begin
    ...
    begin
        if rc then
        begin
            if PointNotInPoly(x, y, False, False, False) then
                CreateBulletX(x, y, 0, 0, 0, 5, Owner);
        end
        else
            CreateBulletX(x, y, 0, 0, 0, 5, Owner);
        x := x + dx;
...
begin
    ...
    if Boss.TempDmgInputFactor > 1.3 then Boss.TempDmgInputFactor := 1.3
    else
...
begin
    ...
    begin
        ...
        for i := 0 to SummonMinionsBots - 1 do
        begin
            ...
            if not RayCast(X, Y, X, Y - 16.0, True, False, False) then
            begin
                for k := 1 to 20 do
                begin
                    dx := RandFlt(-20, 20);
                    dy := RandFlt(-20, 20);
                    if RayCast(X + dx, Y + dy, X + dx, Y + dy - 16.0, True, False, False) then
                    begin
                        X := X + dx;
                        Y := Y + dy;
                        break;
    ...
    begin
        ...
        begin
            if not Satan.Minions[i].spawned then
                if Timer.Value >= Satan.Minions[i].time then
                begin
                    Satan.Minions[i].spawned := True;
                    Zombies_SpawnOne(Spawn.Wave[Spawn.LastWave].ZombieHp *
                        0.0015, Spawn.Wave[Spawn.LastWave].ZombieDmg * 0.005, 0, 71,
                        Satan.Minions[i].X, Satan.Minions[i].Y - 10, True, 0);
        ...
        if Boss.Intro = 0 then
        begin
            if (player[Boss.ID].Participant = 0) and (not Players[Boss.ID].Human) then
            begin
                player[Boss.ID].kicked := True;
                Players[Boss.ID].Kick(TKickSilent);
            end
            else if Modes.CurrentMode > 1 then
...
begin
    ...
    begin
        ...
        begin
            ...
            if (not Weapons_IsInMenu(Pwr.Var1)) and (not Weapons_IsInMenu(Pwr.Var2)) then
                Pwr.Var1 := WTYPE_USSOCOM;
    ...
    begin
        ...
        begin
            ...
            begin
                z := menu2weap(i);
                if (z = Pwr.Var1) or (z = Pwr.Var2) then
                begin
...
begin
    ...
    begin
        ...
        if player[Shooter].Status = -1 then
        begin
            PentagramData.range := PentagramData.range * 3 div 4;
        end;
        PentagramData.bulletAmount :=
            Trunc(ANG_2PI * PentagramData.range / (PentagramBulletStep));
    ...
    begin
        Pwr.Progress := Pwr.Progress - 1;
        if Pwr.Progress > 4 + PentagramSleep then
        begin
            for i := 1 to 5 do
            begin
                CreateBulletX(Pwr.X + PentagramData.range * cos(
                    (ANG_2PI * (Pwr.Progress - 4 - PentagramSleep + i * PentagramData.bulletAmount / 5)) /
                    PentagramData.bulletAmount - ANGLE_45),
                    Pwr.Y + PentagramData.range * sin(
                    (ANG_2PI * (Pwr.Progress - 4 - PentagramSleep + i * PentagramData.bulletAmount / 5)) /
                    PentagramData.bulletAmount - ANGLE_45),
                    0, 0, 0, 5, Shooter);
            end;
        end
        else if Pwr.Progress <= 4 then
        begin
                ...
                for i := 1 to MaxID do
                    if Players[i].Alive then
                        if not player[i].zombie then
                            if IsInRange(i, Pwr.X, Pwr.Y, PentagramData.range * 3 div 4, False) then
                            begin
...
begin
    ...
    for i := 1 to SummonMinionsBots do
    begin
        sine := sin(ANG_2PI * i / SummonMinionsBots);
        cosine := cos(ANG_2PI * i / SummonMinionsBots);
        for j := k downto 0 do
            if RayCast(cosine * c * j + Pwr.X, sine * c * j + Pwr.Y, Pwr.X, Pwr.Y, True, False, False) or
                (j = 0) then
            begin
                Zombies_SpawnOne(Spawn.Wave[Spawn.LastWave].ZombieHp * 0.0015,
                    Spawn.Wave[Spawn.LastWave].ZombieDmg * 0.005, 0, 71, cosine * c * j + Pwr.X,
                    sine * c * j + Pwr.Y, True, 0);
                break;
            end;
    end;
    ...
    begin
        ...
        if (Pwr.Progress = 0) or (Pwr.Progress = 1) then
            ...
            begin
                ...
                if IsInRange(i, Players[t].X, Players[t].Y, S666_LigRange, False) then
                begin                     // checks if players are in range for the lightning flash.
                    if HackermanMode then
                        BigText_DrawScreen(i, 'XXX', 50 - Pwr.Progress * 25, $0000fe, 46, -3920, -4260)
                    // makes screen flash blue on haxmaps.
                    else
                        BigText_DrawScreen(i, 'XXX', 40 - Pwr.Progress * 25, $FFFFFF, 46, -3920, -4260);
                    // makes screen flash white when lightning strikes.
                end;
            end;
        if Pwr.Progress = 0 then
        begin
            ...
            begin
                x := x + RandInt(-100, 100);
                if RayCast(x, y, x, y, True, False, False) then
                begin
                    Result := True;
                    x2 := x;
                    y2 := y;
                    ...
                    begin
                        ...
                        if RayCast(x2, y2, x, y, True, False, False) then
                        begin
                            y2 := y;
                            x2 := x;
                        end
...
begin
    ...
    begin
        repeat
            ...
            Pwr.Progress := Pwr.Progress - 1;
            if RayCast(Pwr.X, Pwr.Y, Pwr.X - CircleRadius * cosine, Pwr.Y -
                CircleRadius * sine, False, True, True) then
            begin
                CreateBulletX(Pwr.X - CircleRadius * cosine, Pwr.Y - CircleRadius * sine,
                    0, 0, -1, 4, Shooter);
                break;
            end;
...
begin
    ...
    begin
        ...
        for i := 0 to ExplodeAmount do
        begin
            sine := sin(pi * i / ExplodeAmount);
            cosine := cos(pi * i / ExplodeAmount);
            if RayCast(Pwr.X + 50 * sine, Pwr.Y + 50 * cosine, Pwr.X + 50 * sine,
                Pwr.Y + 50 * cosine, False, True, True) then
                CreateBulletX(Pwr.X + RandFlt(25, 35) * sine, Pwr.Y +
                    RandFlt(25, 35) * cosine, RandFlt(-1, 1), RandFlt(-1, 1), 0, 5, ID);
        end;
        if Pwr.Progress = 0 then
        begin
            for i := 0 to ExplodeAmount do
            begin
                ...
                cosine := cos(pi * i / ExplodeAmount);
                j := RandInt(-3, 3);
                if RayCast(Pwr.X - 50 * cosine, Pwr.Y - 50 * sine, Pwr.X - 60 * cosine,
                    Pwr.Y - 60 * sine, False, True, True) then
                begin
                    if i mod 2 = 0 then s := 4
                    else
                        s := 8;
                    CreateBulletX(Pwr.X - 50 * cosine, Pwr.Y - 50 * sine,
                        -(ExplodeStrength + j) * cosine, -(ExplodeStrength + j) * sine, -5, s, ID);
                end;
            end;
            ResetPower(Pwr);
        end;
...
begin
    ...
    begin
        ...
        while i <= 400 do
        begin
            if RayCast(Pwr.X + i - 200, Pwr.Y - 350, Pwr.X + i - 200, Pwr.Y - 360,
    ...
    begin
        ...
        begin
            ...
            j := RandInt(20, 40);
            BigText_DrawMap(0, '*', 55, RGB(j, j, j), RandFlt(0.5, 1.0),
                trunc(X), trunc(Pwr.Y - y));
            BigText_DrawMap(0, '*', 55, RGB(j, j, j), RandFlt(0.5, 1.0),
                trunc(X + RandFlt(-50, 50)), trunc(Pwr.Y - y));
...
begin
    ...
    begin
        ...
        tsin := sin(a);
        tcos := cos(a);
        if i mod 2 = 0 then s := 7
        else
            s := 3;
        CreateBulletX(x - tcos * 20, y - tsin * 20, -tcos * ShowerStrength,
            -tsin * ShowerStrength, -100, s, ID);
        CreateBulletX(x + tcos * 20, y - tsin * 20, tcos * ShowerStrength,
            -tsin * ShowerStrength, -200, s, ID);
    end;
    ResetPower(Pwr);
end;
...
begin
    ...
    Zombies_SpawnOne(Spawn.Wave[Spawn.LastWave].ZombieHp * 0.0025,
        Spawn.Wave[Spawn.LastWave].ZombieDmg * 0.005, 0, 81, Pwr.X, Pwr.Y, True, 0);
    ResetPower(Pwr);
end;
...
begin
    ...
    for i := 1 to MaxID do
        ...
        begin
            GetPlayerXY(i, x, y);
            Zombies_SpawnOne((Sqrt(NumberOfWave + 1)) * 10 * ZombieHpInc / 100,
                (NumberOfWave + 1) * ZombieDmgInc / 100, i, 0, x, y, True, 0);
        end;
    Boss_IncreaseTempInputFactor(0.1);
    ResetPower(Pwr);
end;
...
begin
    ...
    if Pwr.Progress = 0 then
    begin
        Pwr.Progress := FireworksIteration * FireworksAmount +
            (FireworksIteration - 1) * FireworksSleepTime;
    ...
    begin
        ...
        if FireworksData.counter = 0 then
        begin
            if FireworksData.sleeping then
            begin
                FireworksData.sleeping := False;
                FireworksData.counter := FireworksAmount;
            end
        ...
        if not FireworksData.sleeping then
        begin
            GetPlayerXY(ID, Pwr.X, Pwr.Y);
            j := RandInt(60, 120);
            x := sin(j * pi / 180);
            y := cos(j * pi / 180);
            CreateBulletX(Pwr.X - y * 20, Pwr.Y - x * 20, -y * FireworksStrength,
                -x * FireworksStrength, 100, 4, ID);
        end;
...
begin
    ...
    begin
        ...
        WriteConsole(0, 'Satan has been thrown into the sea of flames. Heaven reigns!',
            $FF0000);
    end
    else
    begin
        Boss.Intro := Boss.Intro - 1;
        if Length(Boss.Power[0].Spawns) = 0 then
        begin
            i := 0;
            while i <= 250 do
            begin
                j := 0;
                if RayCast(Boss.Power[0].X + i - 125, Boss.Power[0].Y,
                    Boss.Power[0].X + i - 125, Boss.Power[0].Y, False, True, True) then
                begin
                    while RayCast(Boss.Power[0].X + i - 125, Boss.Power[0].Y +
                            j, Boss.Power[0].X + i - 125, Boss.Power[0].Y + j, False, True, True) and (j < 100) do
                        j := j + 5;
                    if j >= 100 then j := 0
                    else
                        j := j - 5;
                ...
                begin
                    while not RayCast(Boss.Power[0].X + i - 125, Boss.Power[0].Y + j,
                            Boss.Power[0].X + i - 125, Boss.Power[0].Y + j, False, True, True) and (j > -100) do
                        j := j - 5;
                    if j <= -100 then j := 0;
                end;
        ...
        for i := 0 to (Length(Boss.Power[0].Spawns) - 1) do
            CreateBulletX(Boss.Power[0].X + (i * 5) - 125 + RandFlt(-5, 5),
                Boss.Power[0].Y + Boss.Power[0].Spawns[i] + RandFlt(-4, 4), 0, 0, 0, 5, Boss.ID);
        if Boss.Intro = 0 then
        begin
            //crucifix
            ...
            //M2 circle
            for i := 0 to 11 do
                CreateBulletX(Boss.Power[0].X, Boss.Power[0].Y, 0 + (i * 10) -
                    50, -25, 0, 14, Boss.ID);
            //reset procedure
            if (player[Boss.ID].Participant = 0) and (not Players[Boss.ID].Human) then
            begin
...
begin
    if Timer.Cycle <> 1 then exit; //1hz power
    for a := 1 to FF_MAXTRAPS do
    begin
        if Firefighter.Trap[a].ID > 0 then
        begin
            if not Firefighter.Trap[a].InProgress then continue;
            GetPlayerXY(Firefighter.Trap[a].ID, X, Y);
            for n := 1 to MaxID do
                if Players[n].Alive then
                    if player[n].Zombie <> Player[Firefighter.Trap[a].ID].Zombie then
                    begin
                        GetPlayerXY(n, X2, Y2);
                        if Y2 < Y + 100 then
                        begin
                            if (X2 + Players[n].VELX * 90 >= X) = (X2 >= X) then
                                // if player with his velocity is not going to cross the trap then check the distance
                                if Abs(X - X2) > 200 then continue;
                            // if the distance is close then let it activate else check another player
                            while RayCast(X, Y - 10, X, Y - (i * 20) - 10, False, True, True) do
                            begin
                                //CreateBulletX(X, Y - (i * 25), 0, 0, 0, 5, Firefighter.Trap.ID);
                                CreateBulletX(X, Y, RandFlt(-0.05, 0.05), -i - 5, 400,
                                    7, Firefighter.Trap[a].ID);
                                CreateBulletX(X, Y, RandFlt(-0.05, 0.05), -i - 5 - 1,
                                    400, 3, Firefighter.Trap[a].ID);
                                i := i + 2;
                                if i >= 20 then break;
                            end;
                            for i := 1 to 10 do // spam
                            begin
                                CreateBulletX(X - 10, Y - RandInt(1, 10), -RandInt(1, 10) * 0.1,
                                    -1, 100, 3, Firefighter.Trap[a].ID);
                                CreateBulletX(X + 10, Y - RandInt(5, 10), RandInt(1, 10) * 0.1,
                                    -1, 100, 3, Firefighter.Trap[a].ID);
                            end;
                            break;
                        end;
...
begin
    ...
    begin
        ...
        begin
            ...
            x := -75 + i * 8;
            y := 0.0;
            while not RayCast(Pwr.X + x, Pwr.Y - 50, Pwr.X + x, Pwr.Y + y, True, True, True) do
            begin
                y := y + 10.0;        //Raycasting, until no polygon stops this
                if (y > 200.0) then break;
            end;
            Pwr.Spawns[i] := Round(y);
            //If the place is fine, the Y-value is written into the array
        end;
    ...
    begin
        ...
        if (Pwr.Progress > FF_Fading) then
            if ((Pwr.Progress mod 2) = 0) then
                if Pwr.Progress < FF_PORTALWARNINGS + Minions * 2 then
                begin
        ...
        for i := 1 to MaxID do
            if Players[i].Alive then
                if player[i].Status = 1 then
                    if IsInRange(i, Pwr.X, Pwr.Y - 20, 40, False) then
                    begin
...
procedure CheckForHeatItem(ItemType: byte; ItemID: byte; Range: single;
    var Pwr: tPower; ItemX, ItemY: single; velocity: single);
var
    dist: single;
    i: byte;
begin
    ...
    if ItemType = 0 then
        Firefighter.ScheduledHeatItem[i].Amount :=
            FF_HEATDMGMAX * iif(dist < 20, 1, 1 - (dist - 20) / Range) *
            iif(RayCast(Pwr.X, Pwr.Y, ItemX, ItemY, False, False, False), 1, 0.2);
...
begin
    ...
    begin
        ...
        begin
            ...
            CreateBulletX(Pwr.X + inner_range_x * x * RandFlt(0.9, 1.1),
                Pwr.Y + inner_range_y * y * RandFlt(0.9, 1.1), vx * RandFlt(0.9, 1.1),
                vy * RandFlt(0.9, 1.1), 0, 5, ID);
        end;
        ...
        //Targets of heat
        for i := 1 to MaxId do
            if (Players[i].Alive) and (player[i].Status = 1) then
            begin
                GetPlayerXY(i, x, y);
        ...
        for i := 1 to MAX_WIRES do
            if wire[i].Active then
                CheckForHeatItem(1, i, Range, Pwr, (wire[i].bundle[0].x +
                    wire[i].bundle[1].x) / 2, (wire[i].bundle[0].y + wire[i].bundle[1].y) / 2, velocity);
    ...
    begin
        ...
        for i := 1 to Firefighter.ActiveHeatItems do
        begin
            if Firefighter.ScheduledHeatItem[i].CountdownTicks > 0 then
            begin
                Firefighter.ScheduledHeatItem[i].CountdownTicks :=
                    Firefighter.ScheduledHeatItem[i].CountdownTicks - 8;
                ...
                begin
                    case Firefighter.ScheduledHeatItem[i].ItemType of
                        ...
                        begin
                            ...
                            Damage_DoRelative(ID, Boss.ID,
                                Firefighter.ScheduledHeatItem[i].Amount, Heat);
                            //TODO Screen Overlay
                        end;
                        1:  //Wires
                        begin
                            CreateBulletX(wire[ID].bundle[0].x,
                                wire[ID].bundle[0].y - 5, 0, 0, 0, 5, Boss.ID);
                            CreateBulletX(wire[ID].bundle[1].x,
                                wire[ID].bundle[1].y - 5, 0, 0, 0, 5, Boss.ID);
                            PlaySound(0, 'firecrack.wav', wire[ID].bundle[0].x,
                                wire[ID].bundle[0].y);
                            WriteConsole(wire[ID].owner,
                                'A wire was destroyed by heat.', RED);
                            Wires_ClearWire(ID);
                        end;
                        2:  //Statguns
                        begin
                            CreateBulletX(statgun[ID].X - 10, statgun[ID].Y, 0,
                                0, 0, 5, Boss.ID);
                            CreateBulletX(statgun[ID].X + 10, statgun[ID].Y, 0,
                                0, 0, 5, Boss.ID);
                            CreateBulletX(statgun[ID].X, statgun[ID].Y -
                                10, 0, 0, 0, 5, Boss.ID);
                            PlaySound(0, 'firecrack.wav', statgun[ID].X, statgun[ID].Y);
                            WriteConsole(statgun[ID].owner,
                                'A statgun was destroyed by heat.', RED);
                            Statguns_DestroySG(ID, False);
                        end;
                        3:  //Mines
                        begin
                            CreateBulletX(Mines.Mine[ID].X, Mines.Mine[ID].Y,
                                0, 0, 0, 5, Boss.ID);
                            WriteConsole(Mines.Mine[ID].owner,
                                'A mine was destroyed by heat.', RED);
                            Mines_Clear(ID);
                        end;
                        4:  //Charges
                        begin
                            CreateBulletX(charge[ID].X, charge[ID].Y, 0, 0, 0, 5, Boss.ID);
                            WriteConsole(charge[ID].owner, 'Charge [' +
                                IntToStr(ID + 1) + '] destroyed by heat.', RED);
                            Charges_Clear(ID);
                        end;
                        5:  //Scarecrow
                        begin
                            ...
                            WriteConsole(Scarecrow.owner,
                                'Scarecrow was destroyed by heat.', RED);
                            Scarecrow_Clear();
                        end;
                        6:  //Sentry
                        begin
                            ...
                            CreateBulletX(x, y, 0, 0, 0, 5, Boss.ID);
                            PlaySound(0, 'firecrack.wav', x, y);
                            WriteConsole(Sentry.owner,
                                'Sentry was destroyed by heat.', RED);
                            Sentry_Clear(True);
                        end;
                    end;
                end;
...
begin
    ...
    Plague.Minions[j].ID := Zombies_SpawnOne(Spawn.Wave[Spawn.LastWave].ZombieHp *
        0.0007, Spawn.Wave[Spawn.LastWave].ZombieDmg * 0.005, 0, 91, x, y - 10, True, 0);
...
begin
    ...
    if (Damage_Type = Explosion) or ((Damage_Type = Heat) and (Damage_Direct)) or
        (weap = WTYPE_M79) or (weap = WTYPE_FRAGGRENADE) or (weap = WTYPE_CLUSTER) or
        (weap = WTYPE_BOW2) or (weap = WTYPE_LAW) or (weap = WTYPE_FLAMER) then
        for i := 1 to Plague.MinionLimit do
        begin
            if Plague.Minions[i].ID = ID then
            begin
...
begin
    ...
    begin
        ...
        begin
            ...
            if not Plague.Minions[i].blown then
            begin
                BigText_DrawMapX(WTL_PLAGUE + ID, Boss.ID, '^', 360, $42C3BB,
                    0.1, Trunc(Plague.Minions[i].X) - 7, Trunc(Plague.Minions[i].Y - 5));
            end;
            break;
        end;
    end;
...
begin
    for j := 1 to Plague.MinionLimit do
        ...
        begin
            ...
            begin
                if Player[Plague.Minions[j].ID].Status < 0 then
                begin //If not human zombie
...
begin
    ...
    begin
        ...
        if (Plague.Minions[j].ID <> 0) and (Plague.Minions[j].dead) and
            (not Plague.Minions[j].blown) then
            if IsInRange(Plague.ID, Plague.Minions[j].X, Plague.Minions[j].Y,
                PLAGUE_REVIVE_DISTANCE, False) then
            begin
                BigText_DrawMapX(WTL_PLAGUE + Plague.Minions[j].ID, Boss.ID,
                    '', 360, $42C3BB, 0.1, 0, 0);
                revivedCount := revivedCount + 1;
                Plague.Minions[j].dead := False;
                PutPlayer(Plague.Minions[j].ID, ZOMBIETEAM, Plague.Minions[j].X,
                    Plague.Minions[j].Y, False);
            end;
    end;
    if (revivedCount > 0) then
    begin
        WriteConsole(Plague.ID, 'You recalled ' + IntToStr(revivedCount) +
            ' Minions.', GREEN);
...
begin
    ...
    begin
        ...
        tcos := cos(a);
        CreateBulletX(x - tcos * 20, y - tsin * 20, -tcos * ShowerStrength,
            -tsin * ShowerStrength, -1, 3, ID);
        CreateBulletX(x + tcos * 20, y - tsin * 20, tcos * ShowerStrength,
            -tsin * ShowerStrength, -2, 3, ID);
    end;
end;
...
begin
    ...
    Zombies_SpawnOne(Spawn.Wave[Spawn.LastWave].ZombieHp * 0.0025,
        Spawn.Wave[Spawn.LastWave].ZombieDmg * 0.005, 0, 51, Players[Shooter].X,
        Players[Shooter].Y, True, 0);
    for i := 1 to 9 do
    begin
        if HackermanMode then
        begin
        ...
        ;
        BigText_DrawMap(0, s, RandInt(100, 150), col, RandFlt(0.03, 0.1),
            trunc(x), trunc(y));
        x := Players[Shooter].X + (Players[Victim].X - Players[Shooter].X) * i / 9 +
            RandFlt(-10, 0);
        y := Players[Shooter].Y + (Players[Victim].Y - Players[Shooter].Y) * i / 9 +
            RandFlt(-10, 0);
        BigText_DrawMap(0, s, RandInt(100, 150), col, RandFlt(0.03, 0.1),
            trunc(x), trunc(y));
    end;
    ...
    if not AutoBrain then
        BigText_DrawScreenX(DTL_ZOMBIE_UI, Shooter,
            'Altar servers left: ' + IntToStr(player[Shooter].Mines), 100, RED, 0.1, 20, 370);
    ResetPower(Pwr);
end;
...
begin
    ...
    yl := yp - 5;
    if HackermanMode then
        BigText_DrawScreen(ID, 'XXX', 10, $0000fe, 46, -3920, -4260)
    // makes screen flash blue when lightning strikes.
    else
        BigText_DrawScreen(ID, 'XXX', 10, $D8D8D8, 46, -3920, -4260);
    // makes screen flash white when lightning strikes.
    while yl > yp - 200 do
    begin
...
begin
    while Boss.Power[0].Victim < MaxID do
    begin
        Boss.Power[0].Victim := Boss.Power[0].Victim + 1;
        if Players[Boss.Power[0].Victim].Alive then
            if player[Boss.Power[0].Victim].Status = 1 then
            begin
                break;
            end;
    end;
    if Boss.Power[0].Victim = MaxID then
    begin
        if player[Boss.Power[0].Victim].Status = 1 then LigUp(Boss.Power[0].Victim);
        Boss.Intro := Boss.Intro - 1;
    end
    else
        LigUp(Boss.Power[0].Victim);
end;
...
begin
    ...
    for i := 0 to MAX_POWERS do
        if Boss.Power[i].InUse > 0 then
            if Boss.Power[i].CountDown = 0 then
            begin
                if CheckPowerColision(Power, Boss.Power[i].InUse) then
                begin
                    Result := Boss.Power[i].InUse;
                    exit;
                end;
            end
            else if Power = Boss.Power[i].InUse then
            begin
                Result := Power;
                exit;
            end;
...
begin
    ...
    i := Trunc(((0.02 * i - 1) / sqrt(0.05 + (0.02 * i - 1) * (0.02 * i - 1)) + 1) * 30 + 40);
    //x->((0.02*x-1)/(0.05+(0.02*x-1)^2)^0.5+1)*30+40
...
begin
    ...
    case SpellID of
        ...
        62: begin  //Portal
            ...
            begin
                if (not AutoBrain) or (players[Boss.ID].Health <= Trunc(MAXHEALTH * 3 / 4)) then
                begin
                    GetPlayerXY(Boss.ID, X, Y);
                    if RayCast(X - 25, Y - 30, X + 25, Y - 30, True, False, False) then
                    begin
                        if RayCast(X, Y - 5, X, Y - 40, True, False, False) then
                        begin
                            if not RayCast(X, Y, X, Y + 30, True, False, False) then
                            begin
                                Result := True;
                            end
                            else if not AutoBrain then WriteConsole(Boss.ID,
                                    'You have to stand on the ground to spawn the Fire', RED);
                        end
                        else if not AutoBrain then WriteConsole(Boss.ID,
                                'Not enough space for the Fire, try somewhere else', RED);
                    end
                    else if not AutoBrain then WriteConsole(Boss.ID,
                            'Not enough space for the Fire, try somewhere else', RED);
                    ...
                    'There are still flames from the last Fire alive', RED);
        end;
        63: begin  //Trap
            for j := FF_MAXTRAPS downto 1 do
                if Firefighter.Trap[j].ID > 0 then
                begin
                    if AutoBrain then
                        if PlayersInRange(Firefighter.Trap[j].ID, Boss.ID, 200, False) then
                            //TODO: shooter...
                            exit;
                end
                else
            ...
            GetPlayerXY(Boss.ID, X, Y);
            Y := Y - 5;
            if (not Autobrain) or (RayCast(X, Y, X, Y - 200, True, False, False)) then
            begin//Condition: Enough vertical space
                case Players_OnGround(Boss.ID, True, 20) of
                    //Condition: Checking, if on ground
                    1: begin
                        sgY[0] := Y;
                        sgY[0] := sgY[0] - 10;
                        sgY[1] := sgY[0];
                        ...
                        begin
                            if Abs(sgY[0] - sgY[1]) <= 15 then
                                ...
                                'Ground is too step to place a trap', RED);
                    end;
                    -1: if not AutoBrain then WriteConsole(Boss.ID,
                                'You must stand on the solid ground to place a trap', RED);
                    else
                        if not AutoBrain then WriteConsole(Boss.ID,
                                'You must stand on the ground to place a trap', RED);
                end;
            end;
            // else if not AutoBrain then WriteConsole(Boss.ID, 'Not enough space (vertical) to spawn a trap', RED);
        end;
        //Satan 1 spells
        71: begin //Minions
            if AliveZombiesInGame <= 3 then
        ...
        83: Result := True;
        //Explode TODO: prevent explosions in tunnels, or modify them to fit
        84: Result := True; //HellShower
        85: Result := True; //Arrow
        86: Result := (not Satan.Shadows) and (players[Boss.ID].Health < MAXHEALTH / 2);
        //Shadows
        87: Result := (not Satan.Artifact) and (players[Boss.ID].Health < MAXHEALTH / 3);
        //Artifact
...
begin
    ...
    case SpellID of
        ...
        61: begin
            for i := 1 to MaxId do
                if ((Players[i].Alive) and (player[i].Status = 1)) or
                    (i = Scarecrow.ID) or (i = Sentry.ID) then
                    if SqrDist(Players[ID].X, Players[ID].y, Players[i].X, Players[i].Y) <=
                        FF_HEATRANGE then
                    begin
                        Result := True;
                        Exit;
            ...
            for i := 1 to MAX_WIRES do
                if wire[i].Active then
                    if SqrDist(Players[ID].X, Players[ID].y,
                        (wire[i].bundle[0].x + wire[i].bundle[1].x) / 2, (wire[i].bundle[0].y +
                        wire[i].bundle[1].y) / 2) <= FF_HEATRANGE then
                    begin
                        Result := True;
                        Exit;
                    end;
            for i := 1 to (SG.Num - 1) do
                if SqrDist(Players[ID].X, Players[ID].y, statgun[i].x, statgun[i].y) <=
                    FF_HEATRANGE then
                begin
                    Result := True;
                    Exit;
            ...
            for i := 1 to Mines.MaxMineID do
                if Mines.Mine[i].placed then
                    if SqrDist(Players[ID].X, Players[ID].y, Mines.Mine[i].x,
                        Mines.Mine[i].y) <= FF_HEATRANGE then
                    begin
                        Result := True;
                        Exit;
                    end;
            for i := 0 to Charges_MaxID do
                if charge[i].placed then
                    if SqrDist(Players[ID].X, Players[ID].y, charge[i].x, charge[i].y) <=
                        FF_HEATRANGE then
                    begin
                        Result := True;
                        Exit;
        ...
        92: for i := 1 to Plague.MinionLimit do
                if (Plague.Minions[i].ID <> 0) and (Plague.Minions[i].dead) and
                    (not Plague.Minions[i].blown) then
                    if PlayersInRange(Boss.ID, Plague.Minions[i].ID,
                        PLAGUE_REVIVE_DISTANCE, False) then
                    begin
                        Result := True;
                        break;
...
begin
    ...
    case SpellID of
        ...
        51: begin //Summon Altar
            ...
            if PlayersInRange(ID, Boss.ID, 250, True) then
                if (ID <> player[Boss.ID].statguns) then
                    ...
                    BigText_DrawScreenX(DTL_ZOMBIE_UI, Boss.ID,
                        'You can''t attack the same player (' + Players[ID].Name + ')', 100, RED, 0.1, 20, 370);
        end;
        //Fire fighter spells
        61: Result := True; //Small heat
        ...
        74: begin //Lighting
            if Players[ID].Alive then
                if not player[ID].frozen then
                    if PlayersInRange(ID, Boss.ID, 320, True) then
                    begin
                        GetPlayerXY(ID, Pwr.X, Pwr.Y);
        ...
        82: begin //HellRain
            if Players[ID].Alive then
                ...
                begin
                    GetPlayerXY(ID, Pwr.X, Pwr.Y);
                    if RayCast(Pwr.X - 200, Pwr.Y - 350, Pwr.X + 200, Pwr.Y - 360,
                        False, True, True) then
                        Result := True
                    else if not AutoBrain then
...
begin
    ...
    if Boss.CountDown > 0 then
    begin
        Data := 'Unready [' + IntToStr(Boss.CountDown) + ']' + #13#10;
        col := $888888;
    end
    else if Boss.PwID > 0 then
    begin
        Data := 'Looking for target [' + PowerNumToName(Boss.PwID) + '] ' +
            Dots((Timer.Value div 60) mod 4) + #13#10;
        col := $888888;
    end
    else
    begin
    ...
    begin
        ...
        begin
            if Boss.Power[i].CountDown > 0 then
            begin
                Data := Data + PowerNumToName(Boss.Power[i].InUse) +
                    ' [' + IntToStr(Boss.Power[i].CountDown) + ']' + #13#10;
            end
            else
                Data := Data + PowerNumToName(Boss.Power[i].InUse) + ' ' +
                    Dots((Timer.Value div 60) mod 4) + #13#10;
...
begin
    ...
    for i := 0 to MAX_POWERS do
        Data := Data + #13#10 + 'slot ' + IntToStr(i) + ':' + iif(Boss.Power[i].InUse >
            0, PowerNumToName(Boss.Power[i].InUse) + '[' + IntToStr(Boss.Power[i].CountDown) + ']', '');
    ...
    begin
        ...
        begin
            ...
            begin
                if HackermanMode then
                begin
                    BigText_DrawMap(0, IntToStr(RandInt_(1)), RandInt(40, 60),
                        $22FF22, RandFlt(0.02, 0.05), trunc(Players[Boss.ID].X) + RandInt(-10, 10),
                        trunc(Players[Boss.ID].Y) + RandInt(-10, 0));
                    s := 'Players[Boss.ID].Health = 0x' +
                        UIntToHex(Round(players[Boss.ID].Health / MAXHEALTH * 255));
                end
                else
                begin
                    s := BossName(Boss.bID) + ': ' +
                        IntToStr(Round(players[Boss.ID].Health / MAXHEALTH * 100)) + '%';
                end;
                BigText_DrawScreenX(DTL_ZOMBIE_UI, 0, s, 120,
                    RG_Gradient(players[Boss.ID].Health / MAXHEALTH, 0.9), 0.08, 20, 370);
            end;
        end;
        ...
        if Boss.Power[0].InUse = 0 then
            case Boss.bID of
                8: begin
                    if InitialCheckSpell(86) then
                    begin
                        CastSpell(86, Boss.ID, 0, 0);
                        Satan.Shadows := True;
                    end;
                    if InitialCheckSpell(87) then
                    begin
                        CastSpell(87, Boss.ID, 0, 0);
                        Satan.Artifact := True;
                    end;
                    if InitialCheckSpell(88) then
                    begin
                        Satan.DoFireworks := False;
        ...
        begin
            if InitialCheckSpell(min) and (AutoBrain or ManualCheckSpell(min)) then
            begin
                for i := 1 to MaxID do
                    if Players[i].Active then
                        if Players[i].Alive then
                            if not player[i].zombie then
                                if (not AutoBrain) or BrainCheckSpell(min, i) then
                                begin
                                    if CheckSpell(min, i, Boss.Power[slot]) then
                                    begin
        ...
        if Length(SpellQueue) > 0 then
        begin //TODO: spell prioritizing
...
begin
    ...
    if Victim.ID = Satan.ArtifactID then
    begin
    ...
    case Boss.bID of
        6:
            for i := 1 to FF_MAXTRAPS do
                if Firefighter.Trap[i].ID > 0 then
                    if (Players[Firefighter.Trap[i].ID].Health < 1) then
                        //i have no idea for what someone did it like that, leaving just in case
                    begin
                        Players[Firefighter.Trap[i].ID].Kick(TKickSilent);
    ...
    if (player[Victim.ID].task = 2) or (player[Victim.ID].task = 91) then
    begin
        x := Victim.X;
        y := Victim.Y - 5;
        vx := Victim.VELX * 0.7;
        for i := 0 to 6 do
            CreateBulletX(x, y - RandFlt(5, 10), RandFlt(-6, 6) + vx,
                RandFlt(-4, 0), 4, 3, Victim.ID);
    end;
end;
...
begin
    ...
    begin
        for i := 1 to FF_MAXTRAPS do
            if ID = Firefighter.Trap[i].ID then
            begin
                Firefighter.Trap[i].ID := 0;
                Firefighter.Trap[i].InProgress := False;
            end;
