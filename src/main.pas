...
begin
	...
	if player[ID].Zombie then begin
		...
		if (PrimaryNum <> player[ID].pri) and (PrimaryNum <> WTYPE_NOWEAPON) then begin
			if (player[ID].task = 3) and (Players[ID].Human) then begin
				Weapons_Force(ID, player[ID].pri, WTYPE_NOWEAPON, 255, 0); // If butcher loses saw to respawn, give it back
			end else
...
begin
	...
	// Increment waves for each active survivor
	for i := 1 to MaxID do
		if Players[i].Active and (player[i].Participant = 1) then
			player[i].Waves := player[i].Waves + 1;

	// Notify players of the new wave
	BigText_DrawScreenX(
		DTL_NOTIFICATION, 0,
		'Wave #'
			+ IntToStr( NumberOfWave )
			+ br
			+ WaveMessages[RandInt(0, MAXWAVEMESSAGES)],
		300,
		Mode[Modes.RealCurrentMode].Color,
		0.07, 20, 380
	);
	if Style = 0 then begin
		if Modes.CurrentMode = 2 then begin
			...
			if Modes.RealCurrentMode = 2 then
				if NumberOfWave < 11 then
					n := (3 + 2 * Players_HumanNum) * NumberOfWave
				else
					n := (3 + 2 * Players_HumanNum) * 11
			else if NumberOfWave < 15 then
				n := 6 * NumberOfWave
			else n := 90;
	...
	case Modes.CurrentMode of
		...
		2: if NumberOfWave > 1 then begin
				...
				Players.WriteConsole('Score - [Team 2]: '
						+ IntToStr(Score[2][2]+ Score[2][1])
						+ ' (' + IntToStr(Score[2][2]) + ')'
						+ '  [Team 1]: ' + IntToStr(Score[1][2] + Score[1][1])
						+ '(' + IntToStr(Score[1][2]) + ')', $CCCC99);
		end;
			...
			if ZombiesLeft = 0 then begin
				case Modes.CurrentMode of
					2: if (civilians > 0) and (Players_StatusNum(1) > 0) then begin
						i:= VSPOINTSFORWAVE;
						if Boss.mbID > 0 then i := i * SPECIALWAVEMP;
...
function GetLSPlayerNameList(): String;
var
	i: Integer;
begin
	for i := 1 to MAX_UNITS do
		if players[i].Active
				and (player[i].Participant = 1)
				and (player[i].Waves >= NumberOfWave div 4)
		then
			Result := Result + Players[i].Name + ', ';

	for i := 0 to Length(MapRecord.Survival.GonePlayers) - 1 do
		if (MapRecord.Survival.GonePlayers[i].Points >= NumberOfWave div 4)
				and (MapRecord.Survival.GonePlayers[i].Name <> '')
		then
			Result := Result + MapRecord.Survival.GonePlayers[i].Name + ', ';
...
begin
	...
	case Modes.CurrentMode of
		// SURVIVAL
		1: begin
			WriteDebug(6, 'EndGame, Mode: 1 (Survival)');
			bestP := Players_MostKills();
			points := CalculatePoints();

			setLength(fields, 8);
			fields[0] := makeFieldItem('Map', CurrentMap2, False);
			fields[1] := makeFieldItem('Score', IntToStr(points), True);
			fields[2] := makeFieldItem('Most Kills',
					iif(bestP > 0, Players[bestP].Name + ' | ' + IntToStr(player[bestp].kills), 'N/A'),
					True
				);
			fields[3] := makeFieldItem('Waves Survived', IntToStr(NumberOfWave), True);
			fields[4] := makeFieldItem('Civilians Alive', IntToStr(civilians), True);
			fields[5] := makeFieldItem('Zombies Killed', IntToStr(zombiesKilled), True);
			fields[6] := makeFieldItem('Time Survived',
					IntToStr((Timer.Value div 60) div 60) + ':'
						+ iif((Timer.Value div 60) mod 60 <= 9,'0','')
						+ IntToStr((Timer.Value div 60) mod 60),
					True
				);
			fields[7] := makeFieldItem('Team', GetLSPlayerNameList(), False);

			if points > MapRecord.Survival.Value then
			begin
				embed.title := 'Survival Match Report';
				embed.description := 'A team of survivors smashed the record for a new HIGH SCORE!';
				embed.thumbnail_url := 'https://i.imgur.com/k6HdYFw.png';
				embed.colour := RGB(0, 255, 255);
				embed.fields := fields;
				Discord_SendWebhookMessage(
					ServerName,
					'A Survival team just scored!',
					embed,
					LS_WEBHOOK_LINK
				);
			end else if NumberOfWave > 30 then
			begin
				embed.title := 'Survival Match Report';
				embed.description := 'A team of survivors put up a good fight.';
				embed.thumbnail_url := 'https://i.imgur.com/n2lkDb5.png';
				embed.colour := RGB(0, 0, 255); //$0000FF;
				embed.fields := fields;
				Discord_SendWebhookMessage(
					ServerName,
					'A Survival team just finished a game.',
					embed,
					LS_WEBHOOK_LINK
				);
				WriteDebug(6, 'EndGame, Tried to send webhook message');
			end;

			if Players_StatusNum(1) = 0 then
				Players.WriteConsole('Battle over, the base has been overrun!', RED);

			for a := 0 to 7 do
			begin
				Players.WriteConsole(RightPad(fields[a].name + ':', ' ', 18) + fields[a].value, GREEN);
			end;

			if points > MapRecord.Survival.Value then begin
				Players.WriteConsole('Your team has managed to beat the high score, congratulations!', INFORMATION);
		...
		// VERSUS
		2: begin
			WriteDebug(6, 'EndGame, Mode: 2 (Versus), Round: ' + IntToStr(VersusRound));
		...
		// INFECTION
		3: begin
			WriteDebug(6, 'EndGame, Mode: 3 (Infection)');
			if Players_StatusNum(1) = 0 then
				Players.WriteConsole('Battle over, the base has been overrun!', RED);
			bestP := Players_MostKills();
			points := CalculatePoints();
			...
			setLength(fields, 9);
			fields[0] := makeFieldItem('Map', CurrentMap2, False);
			fields[1] := makeFieldItem('Survivors Score', IntToStr(points), True);
			fields[2] := makeFieldItem('Infected Score', IntToStr(Score[2][1]), True);
			fields[3] := makeFieldItem('Most Kills',
					iif(bestP > 0, Players[bestP].Name + ' | ' + IntToStr(player[bestp].kills), 'N/A'),
					True
				);
			fields[4] := makeFieldItem('Waves Survived', IntToStr(NumberOfWave), True);
			fields[5] := makeFieldItem('Civilians Alive', IntToStr(civilians), True);
			fields[6] := makeFieldItem('Zombies Killed', IntToStr(zombiesKilled), True);
			fields[7] := makeFieldItem('Time Survived',
					IntToStr((Timer.Value div 60) div 60) + ':'
						+ iif((Timer.Value div 60) mod 60 <= 9,'0','')
						+ IntToStr((Timer.Value div 60) mod 60),
					True
				);
			fields[8] := makeFieldItem('Team', GetLSPlayerNameList(), False);

			if NumberOfWave > 30 then
			begin
				embed.title := 'Infection Match Report';
				embed.description := 'A team of survivors fought off a horde of infected.';
				embed.thumbnail_url := 'https://i.imgur.com/n2lkDb5.png';
				embed.colour := $FF00FF;
				embed.fields := fields;
				Discord_SendWebhookMessage(
					ServerName,
					'An Infection team just finished a game.',
					embed,
					LS_WEBHOOK_LINK
				);
				WriteDebug(6, 'EndGame, Tried to send webhook message');
...
// IgnitionBulletOwner should be different than kamikaze's id because bullets
// don't collide with owner's body. If 0, it will search for ir.
...
begin
	...
	if player[ID].JustResp then begin // if just spawned
		...
		if (Abs(p.X - player[ID].sx) > 20) or (player[ID].task = 3) then begin // if moved from spawn
...
procedure CastTeleport(Priest, Target: byte);
var
	x, y: single;
	i: shortint;
	s: string;
...
begin
	for i := 1 to 9 do begin
		col := $8E44AD;
		s := '*';
		x := Players[Target].X + 20.0*cos(ANG_2PI*i/9)-5;;
		y := Players[Target].Y + 20.0*sin(ANG_2PI*i/9)-5;;
		BigText_DrawMap(0, s, RandInt(100, 150), col, RandFlt(0.03, 0.1), trunc(x), trunc(y));
		x := Players[Priest].X + (Players[Target].X-Players[Priest].X)*i/9 + RandFlt(-10, 0);
		y := Players[Priest].Y + (Players[Target].Y-Players[Priest].Y)*i/9 + RandFlt(-10, 0);
		BigText_DrawMap(0, s, RandInt(100, 150), col, RandFlt(0.03, 0.1), trunc(x), trunc(y));
	end;
	MovePlayer(Priest, Players[Target].X, Players[Target].Y);

end;

procedure PriestMinionZombie_Teleport(ID: integer);
var
	jump: boolean;
	i: shortint;
	d, h: single; p: tActivePlayer;
begin
	begin
		...
		begin
			if Players[i].Active
				and Players[i].Alive
				and (not player[i].zombie)
				and PlayersInRange(i, ID, 250, true) then
			begin
				CastTeleport(ID, i);
				player[ID].charges := ZOMBIE_TELEPORT_COOLDOWN; // start cooldown
				if RandInt_(7) = 0 then BotChat(ID, 'Surprise!');
				break;
			end;
		end;
	end;
...
procedure Process_PriestMinionZombie_Human(ID: integer; MainCall: boolean);
var
	p: tActivePlayer;
begin
	p := Players[ID];
	...
	if MainCall then begin
		if player[ID].charges > 0 then begin
			player[ID].charges := player[ID].charges - 1;
			if player[ID].charges = 0 then begin
				WriteConsole(ID, 'Ready to teleport, (hold [Grenade] key)', $8E44AD);
			end else begin
				if p.KeyGrenade then begin
					BigText_DrawScreenX(DTL_ZOMBIE_UI, ID, 'Unready, wait ' + IntToStr(player[ID].charges) + ' seconds', 100, RED, 0.1, 20, 370);
				end;
	...
	// Teleport to player when grenade key pressed
	if p.KeyGrenade then
	if player[ID].charges = 0 then begin
		PriestMinionZombie_Teleport(ID);
	end;
end;
		...
		for i := 1 to MaxID do begin
			if player[i].Status = 1 then begin
				...
				if RayCast(Players[i].X, Players[i].Y-15, p.X, p.Y-15, true, false, false) then begin
					//p.KeyUp := true;
					//WriteConsole(0,'[DEBUG] Setting velocity to ' + IntToStr(p.VelX*1.16) + ' ' + IntToStr(p.VelY*1.05-0.1),BLUE);
			...
			if player[i].Zombie then begin // zombies
				...
				if Players[i].Human then begin
					...
					case player[i].task of
						...
						51: Process_PriestMinionZombie_Human(i, MainCall);
					end;
...
begin
	...
	Ux := Vx*shift;
	Uy := (Vy + G_BUL*Game.Gravity/(LSMap.Gravity + 0.0001))*shift;
	...
	if Ticks mod 60 = 0 then begin // call #0
		...
		if CheckModeVote then begin
			...
			if Players_HumanNum() > 0 then
				if Modes_CheckVotes() then
					// Update MapRecord (fixes the clearing/overwriting bug?)
	...
	if SHOWPLAYERCOMMANDS then begin
		if not player[ID].Admin then
			if (player[ID].Status <> 0) and (player[ID].Task > 0) then begin
				WriteLn(
					'<' + Players[ID].Name + '> ('
						+ TaskToShortName(player[ID].Task, player[ID].Status < 0) + ') '
						+ Text
				);
			end else
...
{case_of_1}
		...
		if not Game.Paused then case str of
			...
			'sprin','spr':
				WriteConsole(ID, 'Sprinkler feature moved to [Grenade] key', WHITE);
			'showe', 'shw': Priest_TryOperation(ID, 1);
			'seppu': if player[ID].status = 1 then
...
{/case_of_1}
	...
	begin
		TimeStats_End('OnPlayerCommand');
		// if command is already found then stop checking
		exit;
	end;
	b := false;
...
{case_of_3}
	case str of
		...
		//if neither of commands above match then check command in the 2nd case of
		else begin
			if Cop.ID = ID then
	...
	case copy(Text,2,6) of
		...
		//    '/pizg':begin ParticleCast(StrToInt(GetPiece(Text,' ',1)), StrToInt(GetPiece(Text,' ',2)), 0.3, 0.3, 200, player[id].X, player[id].Y);
			//    end;
				...
				'/s': try
					if Sentry_SetStat(GetPiece(Text, ' ', 1), StrToInt(GetPiece(Text, ' ', 2))) then
				...
				'/enableplayerbutcher': try
					Zombies_SetPlayerButcherEnabled(true);
					WriteMessage(0, 'Player-controlled Undead Butcher is now enabled', BLUE);
				except
					WriteMessage(ID, S_ERROR_ARG + '/' + S_ERROR_MSG, RED);
					exit;
				end;
