...
var
    ...
    SurvivalSlotsNum, UPCountDown, StartGameCountdown, molotovOwner,
    molotovsInGame, LastJoinedID, ShowVersusStats: byte;
    ...
    ScoreAntifloodTime, RulesAntiflood, ScorePlayers: integer;

    PerformReset, MapChange, RespSG, CheckNumPlayers, SatanCame,
    FFCame, PlagueCame: boolean;
...
function Spray(X, Y: single; Speed, Damage, Range: single; Style: byte;
    ID: byte; RC: boolean): byte;
var
    InRange: boolean;
    i: byte;
    X2, Y2, angle: single;
...
begin
    ...
    for i := 1 to MaxID do
        if Players[i].Alive then
            if player[i].Zombie <> player[ID].Zombie then
            begin
                GetPlayerXY(i, X2, Y2);
                Y2 := Y2 - 10;
                if PointsInRange(X, Y, X2, Y2, Range, RC) then
                begin
...
begin
    ...
    if (SecondaryNum = WTYPE_NOWEAPON) then
    begin
        ...
        if PrimaryNum = Players[ID].Primary.WType then
        begin
            Ammo := Players[ID].Primary.Ammo;
        end
        else if PrimaryNum = Players[ID].Secondary.WType then
        begin
            Ammo := Players[ID].Secondary.Ammo;
        end;
        Weapons_Force(ID, PrimaryNum, WTYPE_KNIFE, Ammo, 1);
        Players[ID].BigText(DTL_MOLOTOV, 'Molotovs left: ' + IntToStr(player[ID].Molotovs),
            200, RGB(230, 100, 30), 0.1, 20, 370);
...
begin
    case PrimaryNum of
        WTYPE_KNIFE: begin
            ...
            if (Players[ID].Primary.WType = WTYPE_NOWEAPON) and
                (Players[ID].Secondary.WType = SecondaryNum) then
            begin
        ...
        begin
            ...
            if (Players[ID].Primary.WType = WTYPE_KNIFE) and
                (Players[ID].Secondary.WType = SecondaryNum) then
            begin
                molotovOwner := ID;
                molotovsInGame := molotovsInGame + 1;
                if player[ID].Molotovs > 0 then
                begin
                    player[ID].Molotovs := player[ID].Molotovs - 1;
                    if player[ID].Molotovs = 0 then
                    begin
                        BigText_DrawScreenX(DTL_MOLOTOV, ID, 'No more molotovs left',
                            100, RGB(230, 30, 30), 0.1, 20, 370);
                    end;
                end;
            end
            else if (PrimaryNum = WTYPE_NOWEAPON) or (SecondaryNum = WTYPE_NOWEAPON) then
            begin
...
begin
    ...
    begin
        if PrimaryNum = WTYPE_KNIFE then
        begin
            BurnZombie(ID);
        end
        else
        if (PrimaryNum <> player[ID].pri) and (PrimaryNum <> WTYPE_NOWEAPON) then
        begin
    ...
    else if (Players[ID].Dummy) and (not Players[ID].Human) then
    begin
        if ID = Sentry.ID then
        begin
            Weapons_Force(ID, WTYPE_BOW, WTYPE_NOWEAPON, 1, 0);
        end
        else if ID = ScareCrow.ID then
        begin
    ...
    begin
        ...
        if (PrimaryNum = WTYPE_KNIFE) or (players[ID].Primary.Wtype = WTYPE_KNIFE) then
        begin
            Weapons_OnMolotovWeaponChange(ID, PrimaryNum, SecondaryNum);
        end;
...
begin
    ...
    if fac < 1 then fac := 1;
    Result := Trunc(Math.Pow(Score[1][1], Modes.ScoreExponent) * fac);
end;
...
var
    c: byte;
begin
    if NumberOfWave mod Modes.SpecialWaveDelay = 0 then
    begin
        if NumberOfWave > 12 * WaveDelayFactor div WaveDelayDivisor then
        begin
            if NumberOfWave = 13 * WaveDelayFactor div WaveDelayDivisor then
                SatanCame := False;
            if not SatanCame then // Satan 2
                if (RandInt(1, 1000) <= 333 * WaveDelayDivisor div WaveDelayFactor) or
                    (NumberOfWave = 15 * WaveDelayFactor div WaveDelayDivisor) then
                begin
                    Result := 8;
                    SatanCame := True;
                end;
        ...
        if NumberOfWave > 10 * WaveDelayFactor div WaveDelayDivisor then
        begin
            if not PlagueCame then // Plague
                if (RandInt(1, 1000) <= 333 * WaveDelayDivisor div WaveDelayFactor) or
                    (NumberOfWave = 12 * WaveDelayFactor div WaveDelayDivisor) then
                begin
                    Result := 9;
                    PlagueCame := True;
                end;
        ...
        if NumberOfWave > 8 * WaveDelayFactor div WaveDelayDivisor then
        begin
            if not FFCame then // Firefighter
                if (RandInt(1, 1000) <= 333 * WaveDelayDivisor div WaveDelayFactor) or
                    (NumberOfWave = 10 * WaveDelayFactor div WaveDelayDivisor) then
                begin
                    Result := 6;
                    FFCame := True;
                end;
        ...
        if (NumberOfWave > 5 * WaveDelayFactor div WaveDelayDivisor) and
            (NumberOfWave <= 8 * WaveDelayFactor div WaveDelayDivisor) then
        begin
        ...
        if (Result < 6) or (Result > 9) then
            if (NumberOfWave = 5) or (NumberOfWave = 7) or (NumberOfWave >= 9) then
            begin
...
begin
    ...
    BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'Wave #' + IntToStr(
        NumberOfWave) + br + WaveMessages[RandInt(0, MAXWAVEMESSAGES)], 300,
        Mode[Modes.RealCurrentMode].Color, 0.07, 20, 380);
    if Style = 0 then
    begin
        if Modes.CurrentMode = 2 then
        begin
            if NumberOfWave < 15 then n := 9 + 11 * NumberOfWave div 2
            else
                n := 90;
        end
        else
        if Modes.RealCurrentMode = 2 then
            if NumberOfWave < 11 then n := (3 + 2 * Players_HumanNum) * NumberOfWave
                ...
                n := (3 + 2 * Players_HumanNum) * 11
        else if NumberOfWave < 15 then n := 6 * NumberOfWave
        else
            n := 90;
    ...
    case Modes.CurrentMode of
        1: begin
            ...
            if NumberOfWave > 1 then
                WriteConsole(0, 'Zombies killed: ' + IntToStr(zombiesKilled) +
                    ' (score: ' + IntToStr(points) + ')', $CCCC99);
            if points > MapRecord.Survival.Value then
                if MapRecord.Survival.Value > 0 then
                begin
                    MapScore_UpdateRecord(points, not MapRecord.Survival.Shown);
                    if not MapRecord.Survival.Shown then
                    begin
                        MapRecord.Survival.Shown := True;
                        WriteConsole(0,
                            'Your team has managed to beat the high score, congratulations!',
                            WHITE);
                        MapScore_Display(0);
                    end;
                end;
        end;
        2: if NumberOfWave > 1 then
            begin
                if VersusRound = 1 then
                    WriteConsole(0, 'Score - [Team 1]: ' + IntToStr(Score[1][1]) +
                        '  [Team 2]: ' + IntToStr(Score[2][1]), $CCCC99)
                else
                    WriteConsole(0, 'Score - [Team 2]: ' + IntToStr(Score[2][2] + Score[2][1]) +
                        ' (' + IntToStr(Score[2][2]) + ')' + '  [Team 1]: ' +
                        IntToStr(Score[1][2] + Score[1][1]) + '(' + IntToStr(Score[1][2]) + ')', $CCCC99);
            end;
    ...
    WriteConsole(0, 'Wave #' + IntToStr(NumberOfWave) + ' (zombies'' hp: ' +
        IntToStr(Spawn.Wave[1].ZombieHp) + '%, zombies'' damage: ' + IntToStr(
        Spawn.Wave[1].ZombieDmg) + '%)', Mode[Modes.RealCurrentMode].Color);
...
begin
    ...
    begin
        if Spawn.Wave[Spawn.CurrentWave].Active then
        begin
    ...
    if (not Spawn.Active) and (not StartGame) then
        if Boss.Intro = 0 then
            ...
            begin
                case Modes.CurrentMode of
                    2: if (civilians > 0) and (Players_StatusNum(1) > 0) then
                        begin
                            i := VSPOINTSFORWAVE;
                            if (HumHealthMax > 0) and (HumDamageTaken < HumHealthMax) then
                                i := i + Round((HumHealthMax - HumDamageTaken) / HumHealthMax * VSPOINTSFORHEALTH);
...
begin
    if player[ID].Zombie then
        if ID <> Boss.ID then
        begin
            if players[ID].Health > AREADMG then
            begin
                Damage_DoRelative(ID, owner, -AREADMG, Heat);
            end
            else
            begin
                Damage_DoRelative(ID, owner, -Trunc(AREADMG - players[ID].Health - 1), Heat);
                // "-" is here just to filter out negative dmg in OPD
            end;
        end;
    Map.CreateBullet(Players[ID].X, Players[ID].Y + RandFlt(-12.0, -2.0),
        0.5 * Players[ID].VelX, 0.5 * Players[ID].VelY, FLAMEAREADMG, 5, Players[ID]);
end;
...
begin
    ...
    begin
        case _type of
            2:
                if player[ID].holyWater >= EXOCOST then
                begin
                    if player[ID].ExoTimer > 0 then
                    begin
                        WriteConsole(ID, 'Wait ' + IntToStr(player[ID].ExoTimer) +
                            ' seconds to do an exorcism', RED);
                        exit;
                    end;
                    if player[ID].ExoTimer < 0 then
                    begin
                    ...
                    if Spawn.CurrentWave > 0 then
                        if Spawn.Wave[Spawn.CurrentWave].Style <> 0 then
                            WriteConsole(ID,
                                'Warning: Zombies in this wave may be resistant to exorcism', BLUE);
                    WriteConsole(ID, 'Holy water left: ' + IntToStr(player[ID].HolyWater) + ' ml', BLUE);
                end
                else
                    WriteConsole(ID, 'Not enough holy water to do an exorcism', RED);

            1:
                if player[ID].HolyWater >= SHOWERCOST then
                begin
                    if player[ID].ShowerCooldown = 0 then
                    begin
                        player[ID].HolyWater := player[ID].HolyWater - SHOWERCOST;
                        player[ID].ShowerCooldown := SHWDELAY;
                        WriteConsole(ID, 'Holy water left: ' + IntToStr(player[ID].HolyWater) +
                            ' ml', GREEN);
                        a := -90;
                        GetPlayerXY(ID, X, Y);
                        Y := Y - 15;
                        ...
                        WriteConsole(ID, 'Shower is not ready yet. Wait ' + IntToStr(
                            player[ID].ShowerCooldown) + ' seconds', RED);
                end
                else
                    WriteConsole(ID, 'Not enough water to drop shower', RED);
        end;
    end
    else
        WriteConsole(ID, 'You are not the priest', RED);
end;
...
begin
    ...
    if GameRunning then
    begin
        WriteDebug(4, 'cmmd:' + IntToStr(cmmd[0]) + ' cc:' + IntToStr(cmmd[1]) +
            ' hex:' + IntToStr(cmmd[2]));
...
begin
    ...
    begin
        VSStats[1] := Score[1][1] + Score[1][2];
        //Points Team 1 (first survivor, then zombie)
        VSStats[2] := Score[2][2] + Score[2][1]; //Points Team 2 (first zombie, then survivor)
    end;

    if VSStats[1] > VSStats[2] then a := -1
    else
        a := 1; //a = winnerparticipant; at the end of game, team 1 is zombie team and team 2 is surivor team
    if a = -1 then winTeam := 1
    else
        winTeam := 2;
    ...
    begin
        ...
        begin
            ...
            str := 'It''s a Tie!';
        BigText_DrawScreenX(DTL_NOTIFICATION, 0, str, 400, GREEN,
            iif(VSStats[1] <> VSStats[2], 0.08, 0.12), 30, 320);
    end;
        ...
        for i := 1 to MaxID do
            if Players[i].Active then
                case ShowVersusStats of
                    2: begin
                        if VSStats[1] <> VSStats[2] then
                            if Player[i].Participant <> 0 then
                                if Player[i].Participant = a then
                                    WriteConsole(i, 'Congratulations, Your team has won with ' +
                                        IntToStr(VSStats[winTeam]) + ' Points.', GREEN)
                                else
                                    WriteConsole(i, 'Your team has lost with ' + iif(winTeam = 1,
                                        IntToStr(VSStats[2]), IntToStr(VSStats[1])) + ' points.', RED);
                    end;
                    3: begin
                        ...
                        if VSStats[1] <> VSStats[2] then
                            if Player[i].Participant <> 0 then
                                str := 'The other team ' + iif(Player[i].Participant = a, 'lost', 'won') +
                                    ' with ' + iif(Player[i].Participant = a,
                                    iif(winTeam = 1, IntToStr(VSStats[2]), IntToStr(VSStats[1])),
                                    IntToStr(VSStats[winTeam])) + ' points.'
                            else
                                str := 'Team ' + IntToStr(winTeam) + 'won with ' +
                                    IntToStr(VSStats[winTeam]) + ' points over Team ' +
                                    iif(winTeam = 1, '2', '1') + ' (' + iif(winTeam = 1, IntToStr(VSStats[2]),
                                    IntToStr(VSStats[1]))
                        else
                            str := 'It''s a tie!';
                    ...
                    4: begin
                        timestr := IntToStr(VSStats[5] div 60) + ':' +
                            iif(VSStats[5] mod 60 <= 9, '0', '') + IntToStr(VSStats[5] mod 60);
                        timestr2 := IntToStr(VSStats[6] div 60) + ':' +
                            iif(VSStats[6] mod 60 <= 9, '0', '') + IntToStr(VSStats[6] mod 60);

                        if VSStats[5] > VSStats[6] then winTeam := 1
                        else
                            winTeam := 2;
                        if VSStats[5] <> VSStats[6] then
                            str := 'Longest Time survived: ' + iif(winTeam = 1, timestr, timestr2) +
                                ' by ' + iif(Player[i].Participant = 0, 'Team ' + IntToStr(winTeam),
                                iif(Player[i].Participant = a, 'your team ', 'the enemy team')) +
                                ' (' + iif(Player[i].Participant = 0, 'Team ' + iif(winTeam = 1, '2: ', '1: '),
                                iif(Player[i].Participant = a, 'Enemy Team: ', 'Your Team: ')) +
                                iif(winTeam = 1, timestr2, timestr) + ')'
                        else
                            str := 'Both Teams survived ' + timestr + ' minutes';
                        WriteConsole(i, str, BLUE);

                        if VSStats[3] <> VSStats[4] then
                            str := 'Most Waves survived: ' +
                                IntToStr(iif(VSStats[3] > VSStats[4], VSStats[3], VSStats[4])) +
                                ' by ' + iif(Player[i].Participant = 0, 'Team ' + iif(VSStats[3] >
                                VSStats[4], '1', '2'), iif(Player[i].Participant = iif(VSStats[3] > VSStats[4], -1, 1),
                                'your team.', 'the enemy team.'))
                        else
                            str := 'Both teams survived ' + IntToStr(VSStats[3]) + ' waves.';
                        WriteConsole(i, str, BLUE);
                    end;
                    5: if VSStats[10] <> 0 then
                            WriteConsole(i, 'Most Zombies killed by ' + Players[VSStats[9]].Name +
                                ': ' + IntToStr(VSStats[10]) + iif(i = VSStats[9], '', ' (You killed ' +
                                IntToStr(Player[i].kills) + ')'), BLUE);

                    6: begin
                        if VSStats[12] <> 0 then
                            WriteConsole(i, 'Most Civilians eaten by ' + Players[VSStats[11]].Name +
                                ': ' + IntToStr(VSStats[12]) + iif(i = VSStats[11], '', ' (You ate ' +
                                IntToStr(Player[i].civkills) + ')'), $FF6666);
                        if VSStats[14] <> 0 then
                            WriteConsole(i, 'Most Survivors killed by ' + Players[VSStats[13]].Name +
                                ': ' + IntToStr(VSStats[14]) + iif(i = VSStats[13], '', ' (You killed ' +
                                IntToStr(Player[i].survkills) + ')'), $FF6666);
                    end;
                end;
    end;
    ...
    begin
        ...
        if VSStats[5] > VSStats[6] then
        begin
            if VSStats[7] = 0 then i := VSStats[8]
            else
                i := VSStats[7];
        end
        else
        if VSStats[8] <> 0 then i := VSStats[8]
        else
            i := VSStats[7];
        if i <> 0 then

            if Players[i].Active then
                WriteConsole(0, 'Last Survivor standing the longest: ' +
                    Players[i].Name, BLUE);
    end;
    ...
    begin
        ...
        WriteConsole(0,
            'Use /vote ID or /vote <name> to make your choice. Type /votehelp for more info.',
            INFORMATION);
        ShowVersusStats := 0;
    end
    else
        ShowVersusStats := ShowVersusStats + 1;
end;
    ...
    case Modes.CurrentMode of
        1: begin
            ...
            if Players_StatusNum(1) = 0 then
                WriteConsole(0, 'Battle over, the base has been overrun!', RED);
            ...
            WriteConsole(0, 'Time Survived:    ' + IntToStr(
                (Timer.Value div 60) div 60) + ':' + iif((Timer.Value div 60) mod 60 <= 9, '0', '') +
                IntToStr((Timer.Value div 60) mod 60), GREEN);
            if bestP > 0 then
            begin
                kills := player[bestp].kills;
                WriteConsole(0, 'Most kills by ' + Players[bestP].Name + ': ' +
                    IntToStr(kills), GREEN);
            ...
            BigText_DrawScreenX(DTL_NOTIFICATION, 0, LSMap.EndText, 180,
                RGB(50, 50, 255), 0.2, 20, 370);
        end;
        2: begin
...
begin
    ...
    begin
        ...
        if player[ID].PunishmentZombie.Active then
        begin
            WriteConsole(0, Players[ID].Name + ' has been convicted for "' +
                player[ID].PunishmentZombie.Reason + '"', B_RED);
            WriteConsole(0, 'He has ' + FormatTime(player[ID].PunishmentZombie.Time) +
                ' to do as a Penitential Zombie', B_RED);
...
begin
    ...
    for i := 0 to Length(BlockedPlayers) - 1 do
        if (BlockedPlayers[i].HWID = IDToHW(ID)) then
        begin
            if admmessage then
        ...
        if BlockedPlayers[i].IP[0] = IP[0] then
            if BlockedPlayers[i].IP[1] = IP[1] then
            begin
                if (IP[2] = BlockedPlayers[i].IP[2]) and (IP[3] = BlockedPlayers[i].IP[3]) then
                begin
                    if admmessage then
                ...
                else if BlockedPlayers[i].Name = Name then
                begin
                    Result := True;
                    if admmessage then
                ...
                begin // in this case don't block, only warn
                    if message then
                    begin
                        message := False;
                        WriteConsole(ID,
                            'Changing your IP/HWID to rejoin the game after death, with different task or team will result in a ban (!rules)',
                            RED);
                    end;
                    if admmessage then
...
begin
    ...
    begin
        ...
        for i := 0 to n do
        begin
            if full then Result := Result + TaskToName(tasks.arr[i], False)
            else
                Result := Result + TaskToShortName(tasks.arr[i], False);
...
begin
    tasks := FreeTasks();
    if tasks.length > 0 then Result := tasks.arr[RandInt(0, tasks.length - 1)]
    else
        Result := 4;
end;
...
begin
    ...
    if PlayerBlocked(ID, True, True) then
    begin
    ...
    if ID > 12 then
    begin
        WriteConsole(ID, 'Please rejoin in order to play, your id is too high -' +
            IntToStr(ID), RED);
        Exit;
    end;

    case Modes.CurrentMode of
        1: if SurvivalSlotsNum = 0 then
            begin
                WriteConsole(ID,
                    'There are no free slots in this game left, please wait until the round ends', RED);
                exit;
            end
            else
            begin
        ...
        2: begin
            ...
            if w >= Game.MAXPLAYERS - Game.MAXPLAYERS div 2 then
            begin
                WriteConsole(ID, 'There are no free slots in the survivors team', RED);
                exit;
            end
            else
            if w > Players_ParticipantNum(-1) then
            begin
                WriteConsole(ID,
                    'There are fewer players in the zombie team. Try joining it instead (/joinZ)', RED);
                exit;
            end;
        end;
        //3:
    ...
    WriteConsole(0, Players[ID].Name + ' has joined the surivors team as a ' +
        TaskToName(w, False), WHITE);
...
begin
    ...
    begin
        ...
        begin
            if player[ID].Status = 0 then
            begin
                if not PlayerBlocked(ID, True, True) then
                begin
                    w := Players_ParticipantNum(-1);
                    if (w < Game.MAXPLAYERS div 2) or (Modes.CurrentMode = 3) then
                    begin
                        if (Players_ParticipantNum(1) >= w) or (Modes.CurrentMode = 3) then
                        begin
                            CreateUnit(ID, -1, -1);
                            Player[ID].SpawnTimer := round(15 * ((randInt(1, 9) - 5) / 10 + 1));
                            WriteConsole(0, Players[ID].Name +
                                ' has joined the game as a zombie', WHITE);
                            WriteConsole(ID, 'You will be spawned soon', GREEN);
                            WriteDebug(5, Players[ID].Name + ' has joined as a zombie');
                        end
                        else
                            WriteConsole(ID,
                                'There are fewer players in survivors team. Try joining survivors team instead (/joinS)',
                                RED);
                    end
                    else
                        WriteConsole(ID, 'There are no free slots left in the zombie team', RED);
                end
                else
                    WriteConsole(ID, 'You''ve already played in this round', RED);
            end
        ...
        WriteConsole(ID, 'Joining zombie team is possible only in Versus and Infection mode',
            RED);
end;
...
begin
    ...
    case Modes.CurrentMode of
        1: begin
            for i := 1 to MaxID do // create humans stack
                if Players[i].Active then
                    if Players[i].Human then
                        if not player[i].PunishmentZombie.Active then
                        begin
                            hn := hn + 1;
                            stack8_push(hum, i);
                        end
        ...
        2: begin
            ...
            while specs.length > 0 do
                if inf.length < hum.length then
                    stack8_push(inf, stack8_pop(specs, RandInt(0, specs.length - 1)))
                else
    ...
    for i := 1 to MAX_TASKS do // create tasks stack
        if (i <> 6) and ((Modes.RealCurrentMode <> 1) or ((i <> 6) and (i <> 1))) and
            ((i <> 3) or (hum.length < 3)) then  // <> doc; <> cop; <> mech
            stack8_push(tasks, i);
    case hum.length of
    ...
    while hum.length > 0 do
        CreateUnit(stack8_pop(hum, RandInt(0, hum.length - 1)),
            stack8_pop(tasks, RandInt(0, tasks.length - 1)), 1);
    ...
    begin // limited slots for survival
            ...
            if Players[i].Active then
                if Players[i].Human then
                    if player[i].Status = 2 then
                        if TaskAvailable[player[i].Task] > 0 then
                        begin
...
var
    i: byte;
begin
    i := Players_ParticipantNum(0);
    // get the number of "spectators" who didn't play in any team
    if (Players_HumanNum <= 1) or (Players_ParticipantNum(1) + i = 0) or
        (Players_ParticipantNum(-1) + i = 0) then
    begin
        WriteConsole(0,
            'There are no players to form two teams and start the second Versus round',
            WHITE);
        EndGame();
    end
    else
    begin
        ...
        Reset();
        BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'GAME OVER!', 180,
            RGB(50, 50, 255), 0.15, 20, 370);
        StartMatch(False);
    end;
end;
...
begin
    ...
    for i := 1 to MAX_OBJECTS do
        if Map.Objects[i].Active then
            if Map.Objects[i].Style = 24 then
            begin
                x := Map.Objects[i].X;
                y := Map.Objects[i].Y;
                ...
                if RayCast(x, y - 60, x, y - 60, False, True, False) then
                begin
                ...
                for j := 0 to 5 do
                    CreateBulletX(x, y, RandInt(-90, 90) / 10, -RandInt(-10, 30) / 10,
                        -5, 5, molotovOwner);
                Fire_CreateBurningArea(x, y, 12, 120, 12, 7, molotovOwner);
                if molotovsInGame > 0 then molotovsInGame := molotovsInGame - 1
                else
                    break;
            end;
end;
...
begin
    ...
    for i := 0 to l - 1 do
        if not SpraySyst.Spray[i].Active then
        begin
            found := True;
            l := i;
            break;
...
begin
    ...
    CreateBulletX(X, Y, vx + RandFlt(-1, 1), vy + RandFlt(-2, 2) *
        (1 - SpraySyst.Spray[n].vFactor), 1, 3, SpraySyst.Spray[n].Owner);
...
var
    ...
    Active: boolean;
begin
    if SpraySyst.Active then
    begin
        for i := 0 to SpraySyst.N do
            if SpraySyst.Spray[i].Active then
            begin
                if SpraySyst.Spray[i].T > 0 then
                begin
                    Active := True;
                    Spray_Tick(i);
                end
...
begin
    ...
    if T > 0 then
        if Players[T].Dummy then
        begin
            if T = Scarecrow.ID then Scarecrow_OnDamage(20)
            else
            if T = Sentry.ID then Sentry_OnDamage(1);
        end;
end;

procedure KamiKaboom(ID: byte; r: single);
// IgnitionBulletOwner should be different than kamikaze's id because bullets don't collide with owner's body. If 0, it will search for ir.
var
    i: byte;
    k: word;
    x, y, x2, y2, d: single;
...
begin
    ...
    for i := 1 to MaxID do
        ...
        begin
            ...
            begin
                ...
                if i = Scarecrow.ID then Scarecrow_OnDamage(k)
                else
                if i = Sentry.ID then Sentry_OnDamage(k);
            end;
        end;
    for i := 1 to (SG.Num - 1) do
        if SqrDist(X, Y, statgun[i].X, statgun[i].Y) <= r * r / 4 then
        begin
    ...
    for i := 1 to MAX_WIRES do
        ...
        begin
            d := SqrDist(X, Y, (wire[i].bundle[0].x + wire[i].bundle[1].x) / 2,
                (wire[i].bundle[0].y + wire[i].bundle[1].y) / 2);
            if d <= r * r then
            begin
    ...
    Damage_DoAbsolute(ID, iif(player[ID].KamiKiller = 0, ID, player[ID].KamiKiller),
        MaxHealth);
...
var
    x, y, a, vx, vy, dist, close_dist, angle_diff, d: single;
    i: integer;
begin
    GetPlayerXY(ID, x, y);
    vx := x2 - x;
    ...
    close_dist := dist;
    if close_dist > 14 then close_dist := 14;
    // keep start distance from shooter not to set him on fire
    ...
    for i := 1 to MaxID do
        if Players[i].Alive then
            ...
            begin
                ...
                if dist < 210.0 then
                begin
                    angle_diff := Math.abs(ShortenAngle(Math.arctan2(Players[i].Y - y,
                        Players[i].X - x) - a));
                    if angle_diff < ANGLE_30 then
                    begin
                        ...
                        if Players[i].Dummy then
                        begin
                            if i = Scarecrow.ID then Scarecrow_OnDamage(Round(40.0 * d))
                            else
...
begin
    case Modes.CurrentMode of
        1: begin
            case StartGameCountdown of
                0: begin
                    BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'The Last Stand - ' +
                        Mode[Modes.RealCurrentMode].Name, 450, Mode[Modes.RealCurrentMode].Color,
                        0.1, 20, 370);
                    MapVotes_Reset();
                end;
                2: begin
                ...
                4: begin
                    ...
                    for i := 1 to MAX_UNITS do
                        if player[i].Participant = 1 then
                        begin
                            WriteConsole(0, ' ' + taskToName(player[i].task, False) +
                                ': ' + Players[i].Name, WHITE);
                            BigText_DrawScreenX(DTL_NOTIFICATION, i, 'You are the ' +
                                taskToName(player[i].task, False), 400, RGB(255, 255, 100), 0.08, 20, 370);
                        end;
        ...
        2: begin
            case StartGameCountdown of
                0: begin
                    BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'The Last Stand - Versus' +
                        br + 'Round: ' + IntToStr(VersusRound) + '/2', 450, Mode[3].Color, 0.1, 20, 370);
                    if VersusRound = 1 then MapVotes_Reset();
                    if Players_ParticipantNum(-1) + Players_ParticipantNum(1) = 1 then
                        StartGameCountdown := 6;
                end;
                2: if (Players_ParticipantNum(-1) > 0) then
                        if (Players_ParticipantNum(1) > 0) then
                        begin
                            for i := 1 to MAX_UNITS do
                                if player[i].Participant = 1 then
                                    str := str + ' ' + Players[i].Name + br;
                            BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'Team ' +
                                IntToStr(VersusRound) + ' - Survivors' + br + str, 220, WHITE, 0.1, 30, 320);
                        end
                        else
                            StartGameCountdown := 6;
                5: begin
                ...
                6: if (Players_ParticipantNum(-1) > 0) then
                        if (Players_ParticipantNum(1) > 0) then
                        begin
                            for i := 1 to MAX_UNITS do
                                if player[i].Participant = -1 then
                                    str := str + ' ' + Players[i].Name + br;
                            BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'Team ' +
                                IntToStr(VersusRound mod 2 + 1) + ' - Zombies' + br + str, 330,
                                $FF6666, 0.1, 30, 320);
                        end;
                8: begin
                    if Players_HumanNum = 0 then
                ...
                9: begin
                    for i := 1 to MAX_UNITS do
                        ...
                        begin
                            BigText_DrawScreenX(DTL_NOTIFICATION, i, 'You are the ' +
                                taskToName(player[i].task, False), 400, $FFFF64, 0.1, 20, 370);
                        end;
                end;
        ...
        3: begin
            case StartGameCountdown of
                0: begin
                    BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'The Last Stand - ' +
                        Mode[4].Name, 450, Mode[4].Color, 0.1, 20, 370);
                    MapVotes_Reset();
                end;
                2: begin
                    if Players_HumanNum = 0 then
        ...
        4: begin
            case StartGameCountdown of
                0: begin
                    BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'The Last Stand - ' +
                        Mode[5].Name, 450, Mode[5].Color, 0.1, 20, 370);
                    MapVotes_Reset();
                end;
                2: begin
                    if Players_HumanNum = 0 then
    ...
    begin
        ...
        if LSMap.DifficultyPercent <> 100 then
            WriteConsole(0, ' Map difficulty: ' + IntToStr(LSMap.DifficultyPercent) +
                '%', WHITE);
        if weaponsystem.Enabled then
        begin
    ...
    if Timer.Value mod 180 = 0 then
        for i := 1 to MaxID do
            if Players[i].Alive then
                if player[i].Status = -1 then
                begin
                    if player[i].AfkSpawnTimer > 0 then
                    begin
                        if player[i].GetX then continue;
                        GetPlayerXY(i, x, y);
                        if Abs(player[i].sx - x) < 50 then
                        begin
                            ...
                            begin
                                if i = Boss.ID then
                                begin
                                    if player[i].AfkSpawnNum < 2 then
                                    begin
                                    ...
                                    begin
                                        if Players_ParticipantNum(-1) > 1 then
                                        begin
                                ...
                                begin
                                    if player[i].AfkSpawnNum = 0 then
                                    begin
...
begin
    ...
    begin // if just spawned
        if player[ID].GetX then
        begin // get player's position after spawn
            if (
{$IFDEF FPC}AZSS.{$ENDIF}
                AZSS.Active) and (player[ID].Status = 0) then
                AZSS_OnZombieResp(ID);
            player[ID].sx := p.X;
            player[ID].GetX := False;
        end
        else
        if Abs(p.X - player[ID].sx) > 20 then
        begin // if moved from spawn
            player[ID].JustResp := False;
            if not ((pri = player[ID].pri) or (p.Secondary.WType = player[ID].pri)) then
                // if zombie doesnt have it's task specific weapon (ie butcher & saw) then force it
    ...
    if pri <> player[ID].pri then
        if pri <> WTYPE_NOWEAPON then
        begin
            if p.Secondary.WType = player[ID].pri then
            begin
    ...
    if player[ID].AntiBlockProtection then
    begin // [opt]
        if RayCast(p.x, p.y - 5, p.x, p.y - 10, True, False, False) then
        begin
        ...
        if player[ID].LastPos.Active then
            if player[ID].StuckCD < 4 then player[ID].StuckCD := player[ID].StuckCD + 1
            else
            begin
                player[ID].StuckCD := 0;
                h := Players[ID].Health;
...
begin
    ...
    if Player[ID].Status = -2 then
    begin // if he's a dead human-zombie player waiting for spawn
        if player[ID].SpawnTimer > 0 then
        begin
            Player[ID].SpawnTimer := Player[ID].SpawnTimer - 1;
            if Player[ID].SpawnTimer > 0 then
            begin
                if (Player[ID].SpawnTimer < 5) or (Player[ID].SpawnTimer mod 5 = 0) then
                    BigText_DrawScreenX(DTL_COUNTDOWN, ID, 'Ready to spawn in ' +
                        IntToStr(Player[ID].SpawnTimer) + ' second' + pl(Player[ID].SpawnTimer),
                        100, RGB(255, 80, 80), 0.1, 20, 370);
            end
            else
            begin
                BigText_DrawScreenX(DTL_COUNTDOWN, ID, 'Ready to spawn', 200,
                    RGB(255, 80, 80), 0.1, 20, 370);
            end;
        end;
    end;
end;
    ...
    if MainCall then
        for i := 1 to MaxID do
            if (player[i].Status = 1) or (Players[i].Dummy) then
                if PlayersInRange(i, ID, 30, False) then
                begin
...
begin
    p := Players[ID];
    if (p.KeyGrenade) or (player[ID].KamiDetonate > 0) then
    begin
        KamiKaboom(ID, 100);
        player[ID].KamiDetonate := 0;
    end;
...
begin
    p := Players[ID];
    if p.Human then
    begin // if zombie is a real player, direction of jump is controlled by velocity
        d := p.VELX * 5;
        jump := d > 0;  // set direction of jump

    end
    else // for bot player d var was set before
    if d < 0 then
    begin  // set direction of jump
        d := d / -20;
        jump := False;
    end
    else
    ...
    if jump then
    begin // jump in right side
        if PointNotInPoly(p.x - d, p.y + h, False, True, True) then
        begin
            if p.Human then
                BigText_DrawScreenX(DTL_ZOMBIE_UI, ID, 'You need to be on the ground',
                    100, RED, 0.1, 20, 370);
    ...
    case Round(d / 4) of
        // very important code, choose the proper "weee" depending on jump power
...
begin
    ...
    if Timer.Value - player[ID].HurtTime > 300 then exit;
    // zombie can jump only for up to 5 seconds after being attacked
    x := p.X;
    y := p.Y;
    ...
    for i := 1 to MaxID do
        if ID <> i then // if there are other players close then cancel
            if Players[i].Active then
                if Players[i].Alive then
                    if IsInRange(i, x, y, 50, False) then
                    begin
                        exit;
                    end;
            ...
            if RandInt_(10) + 1 <= VOMIT_JUMP_CHANCE then
                if d < 160000 then // 400^2
                    if d > 10000 then // 100^2
                        if RayCast(x, y - 15, Players[i].X, Players[i].Y - 15, True, False, False) then
                        begin // if target visible
                            d := x - Players[i].X;
                            if (d > 0) = (p.VELX > 0) then exit;
                            // jump only if the zombie is getting towards it's target
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                begin
                    BigText_DrawScreenX(DTL_ZOMBIE_UI, ID, 'Unready, wait ' +
                        IntToStr(player[ID].charges) + ' seconds', 100, RED, 0.1, 20, 370);
                end;
            end;
        end;
    end;
...
begin
    ...
    begin
        ...
        begin
            player[ID].charges := player[ID].charges - 1;
            if player[ID].charges = 0 then
                WriteConsole(ID, 'Burning breath is ready! HOLD the [Shoot] key to breathe',
                    $FF7830);
        end;
    end
    else
...
begin
    p := Players[ID];
    if Timer.Value - player[ID].HurtTime > 180 then
    begin
        if players[ID].Health < MAXHEALTH then
        begin
...
begin
    ...
    begin
        player[ID].X := p.X;
        player[ID].Y := p.Y;
        if Distance(p.X, p.Y, player[ID].X, player[ID].Y) < 100.0 then
        begin
            if player[ID].mines < 8 then player[ID].mines := player[ID].mines + 1;
            // healing speed
            if player[ID].mines = 6 then
            begin
                WriteConsole(ID, 'Move! Decent zombies do not camp!', B_RED);
            end
            else
            if player[ID].mines = 8 then
            begin
        ...
        begin
            for i := 1 to MaxID do
                if player[i].Status = 1 then
                    if PlayersInRange(ID, i, 300, False) then
                    begin
...
begin
    ...
    begin
        ...
        begin
            p := Players[ID];
            if player[ID].KamiDetonate = 1 then
            begin
                t := 0;
                for i := 1 to MaxID do
                begin
                    if player[i].Status = 1 then
                        if SqrDist(Players[i].X, Players[i].Y, p.X, p.Y) < 40000.0 then
                        begin // 200^2
                            t := t + 1;
                        end;
                end;
...
begin
    ...
    begin
        for i := 1 to MaxID do
            if (player[i].Status = 1) then
                if PlayersInRange(i, ID, 150, False) then
                begin
                    player[ID].KamiDetonate := TICKING_BOMB_TIME;  // detonate!
                    CreateBulletX(Players[ID].X, Players[ID].Y, 0, 0, 0, 5, ID);
                    break;
...
var
    ...
    d: single;
begin
    if Players_OnGround(ID, True, 15) = 1 then
    begin
        p := Players[ID];
        for i := 1 to MaxID do
        begin
            ...
            begin
                ...
                if d < 330.0 then
                    if dot(Players[i].X - p.X, Players[i].Y - p.Y, p.VelX, p.VelY) > 0 then
                        // if butcher is getting towards it's victim
                        if RayCast(Players[i].X, Players[i].Y - 15, p.X, p.Y - 15, True, False, False) then
                        begin
                            //p.KeyUp := true;
                            p.SetVelocity(p.VelX * 1.16, p.VelY * 1.05 - 0.1);
                            if d < 200.0 then
                                if Timer.Value > player[ID].ZombChatTime + 480 then
                                    if p.VelX * p.VelX + p.VelY * p.VelY > 16.0 then
                                    begin // 4^2
...
begin
    ...
    begin
        ...
        begin
            p := Players[ID];
            if lookForTarget(ID, p.X, p.Y, 0, 50, False) = 0 then
            begin
...
begin
    p := Players[ID];
    if player[ID].ReloadTime > 0 then
    begin
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                begin
                    if r < 550 then
                        if RayCast3(Players[i].x, Players[i].y - 15.0, p.x, p.y - 15.0, 5, 3) > 0.5 then
                        begin
                            ZombNear := ZombNear + 1;
                        end;
                end
...
begin
    ...
    begin
        ...
        if Abs(Players[ID].X - player[ID].sx) > 20.0 then
        begin // if player moved from his spawn position
            player[ID].JustResp := False;
            if player[ID].Status < 0 then
    ...
    if Player[ID].RespawnPlayer then
    begin
        H := Players[ID].Health;
        V := Players[ID].Vest;
...
begin
    ...
    begin
        ...
        begin
            case Modes.CurrentMode of
                1, 2: begin
                    if player[ID].bitten then
                    begin
                        if Zombies_SpawnOne((Sqrt(NumberOfWave + 1)) * 10 *
                            ZombieHpInc / 100, (NumberOfWave + 1) * ZombieDmgInc / 100, ID, 0, player[ID].X,
                            player[ID].Y, True, 0) > 0 then
                        begin
            ...
            BigText_DrawScreenX(DTL_NOTIFICATION, 0, Players[ID].Name +
                ' perished!', 180, $FF3232, 0.1, 20, 370);
...
var
    target, n: byte;
    b: boolean;
    x, y, vx, vy, dist: single;
begin
    if player[ID].WrenchCooldown = 0 then
    begin
        if not player[ID].Frozen then
            if Players[ID].Primary.Wtype = WTYPE_NOWEAPON then
                if Players[ID].KeyShoot then
                begin
                        ...
                        for n := 1 to MaxID do
                            if Players[n].Alive then
                                if Player[n].Zombie then
                                    if PlayersInRange(ID, n, 40, False) then
                                    begin
                                        vx := Players[n].x - x;
                                        vy := Players[n].y - y;
                                        dist := sqrt(vx * vx + vy * vy);
    ...
    if player[ID].MousePointTime = 60 then
    begin
...
begin
    if player[ID].MousePointTime = 30 then
    begin
...
begin
    ...
    begin
        ...
        begin
            ...
            if player[ID].ShowerCooldown = 0 then
                WriteConsole(ID, 'Holy shower ready, (/shw)', BLUE);
        end;
        ...
        begin
            ...
            begin
                ...
                begin
                    BigText_DrawScreenX(DTL_COUNTDOWN, ID,
                        'Exorcism [' + IntToStr(-player[ID].ExoTimer) + ']', 200, BLUE, 0.1, 20, 370);
                end
                else
                begin
                    BigText_DrawScreenX(DTL_COUNTDOWN, ID, 'Exorcism failed.', 100,
                        DT_FAIL, 0.1, 20, 370);
            ...
            begin
                ...
                case RandInt(1, 4) of
                    1: BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'Go back to hell!', 200,
                            EXO_COL, 0.12, 20, 370);
                    2: BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'Amen!', 200,
                            EXO_COL, 0.12, 20, 370);
                    3: BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'For God''s sake!', 240,
                            EXO_COL, 0.12, 20, 370);
                    4: BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'Curse you!', 200,
                            EXO_COL, 0.12, 20, 370);
                end;
    ...
    if player[ID].SprinkleCooldown > 0 then
    begin
    ...
    begin
        if players[ID].KeyGrenade then
            if not player[ID].Frozen then
                if players[ID].Grenades = 0 then
                begin
                    if player[ID].HolyWater > SPRINKLECOST then
                    begin
                        GetPlayerXY(ID, x, y);
                        y := y - 10;
...
begin
    p := Players[ID];
    if Distance(P.MouseAimX, P.MouseAimY, player[ID].LastMouseX,
        player[ID].LastMouseY) < 10.0 then
    begin
    ...
    for i := 1 to MaxID do
        ...
        begin
            ...
            begin
                ...
                else if Player[i].Status = 1 then
                begin// if not player.zombie
                    ...
                    if Players[i].Primary.WType = Players[i].Secondary.WType then
                        if Players[i].KeyChangeWeap then
                        begin
...
begin
    ...
    Sleep(100);
    BigText_DrawScreenX(DTL_NOTIFICATION, 0, 'Seppuku!', 240, RGB(255, 200, 155),
        0.15, 20, 370);
    Sleep(300);
                ...
                begin
                    for i := x to y do
                        if Players[i].Active then
                            if player[i].Participant = 1 then
                            begin
                                WriteConsole(i, '[Team 1]:      Current: ' +
                                    IntToStr(Score[1][2]) + '; Total: ' + IntToStr(Score[1][1] + Score[1][2]), SILVER);
                                WriteConsole(i, '[Team 2](You): Current: ' +
                                    IntToStr(Score[2][2]) + '; Total: ' + IntToStr(Score[2][1] + Score[2][2]), WHITE);
                            end
                            else
                            begin
                                WriteConsole(i, '[Team 1](You): Current: ' +
                                    IntToStr(Score[1][2]) + '; Total: ' + IntToStr(Score[1][1] + Score[1][2]), WHITE);
                                WriteConsole(i, '[Team 2]:      Current: ' +
                                    IntToStr(Score[2][2]) + '; Total: ' + IntToStr(Score[2][1] + Score[2][2]), SILVER);
                            end;
                end;
            end
            else
...
begin
    case style of
        ...
        begin
            ...
            WriteConsole(ID, ' /task      - Shows your current task and task-specific commands.',
                WHITE);
            WriteConsole(ID, ' /list      - Shows a list of players with their tasks.', WHITE);
            WriteConsole(ID,
                ' /vote      - Makes a vote for starting the game or changing the map',
                WHITE);
            ...
            case Modes.CurrentMode of
                1: begin
                    WriteConsole(ID, ' !score    - Shows the high score for the current map.',
                        WHITE);
                end;
                2, 3: begin
        ...
        4: begin
            ...
            WriteConsole(ID,
                ' !sbug, !spect - Moves you in game if you have the "spectator bug"',
                WHITE);
            if Modes.CurrentMode <> 2 then
                WriteConsole(ID, ' !pause      - Pauses a game. (!unpause to cancel the pause)',
                    WHITE);
            WriteConsole(ID, ' /bug <text> - Use this command to report a bug', WHITE);
            WriteConsole(ID, ' /quick      - Shows a list of quick, task-specific commands',
                SHORTCOMMAND);
            WriteConsole(ID, ' /table      - Shows a table of quick commands for all tasks',
                SHORTCOMMAND);
            WriteConsole(ID, ' /eqp        - Shows your equipment', WHITE);
            WriteConsole(ID, ' /autovote   - Makes you vote automatically after the game ends',
                WHITE);
            ...
            begin
                ...
                WriteConsole(ID, ' /servers    - Shows a list of connected servers and commands',
                    $ADFF2F);
            end;
            WriteConsole(ID, 'Join us @ www.eat-that.org, #soldat.eat-this! @ Quakenet', RED);
        end;
        3: if GameRunning then
            begin
                case Modes.CurrentMode of
                    1: begin
                        // SURVIVAL
                        for a := 1 to MaxID do
                            ...
                            begin
                                stri := Copy(Players[a].Name, 1, 20) + ': ' + taskToName(
                                    player[a].task, False);
                                l := Length(stri);
                                stri := stri + StringOfChar(' ', 35 - l);
                                if player[a].Status = 2 then stri := stri + ' [ INFECTED ]'
                                else
                                    stri := stri + '             ';
                                ...
                                begin
                                    ...
                                    begin
                                        ...
                                        if Player[a].VotedMap = VOTE_RANDOM then stri := stri + 'Random'
                                            ...
                                            stri := stri + MapList.List[player[a].VotedMap] + '(' +
                                                IntToStr(Player[a].VotedMap + 1) + ')';
                                        stri := stri + '} ';
                                    end;
                                    stri := stri + ']';
                                end;
                    ...
                    2, 3: begin
                        // VERSUS
                        WriteConsole(ID, iif(Modes.CurrentMode = 2, 'Team ' +
                            IntToStr(VersusRound) + ' - ', '') + 'Survivors:', WHITE);
                        for a := 1 to MaxID do
                            if Players[a].Human then
                                if player[a].Participant = 1 then
                                begin // human team
                                    stri := ' ' + Copy(Players[a].Name, 1, 20) + ': ' +
                                        taskToName(player[a].task, False);
                                    l := Length(stri);
                                    stri := stri + StringOfChar(' ', 35 - l);
                        ...
                        begin
                            WriteConsole(ID, iif(Modes.CurrentMode = 2, 'Team ' +
                                IntToStr(VersusRound mod 2 + 1) + ' - ', '') + 'Zombies:', WHITE);
                            for a := 1 to MaxID do
                                if Players[a].Human then
                                    if player[a].Participant = -1 then
                                    begin
                                        if player[a].Status = -1 then
                                        begin
                                            stri := ' ' + Copy(Players[a].Name, 1, 17) +
                                                ': ' + TaskToName(player[a].Task, True);
                                        end
                                        else
                                            stri := ' ' + Copy(Players[a].Name, 1, 20);
                                        l := Length(stri);
              ...
              if Players[a].Human then
                ...
                for a := 1 to MaxID do
                    ...
                    begin
                        ...
                        if player[a].ModeReady then
                            stri := stri + ' [ MODE ' + IntToStr(player[a].VotedMode) + ' ]'
                        else
                            stri := stri + '           ';
        ...
        5: begin
            ...
            //if Modes.CurrentMode <> 1 then
            WriteConsole(ID,
                ' Do not change your IP to rejoin the game after death, with different task or team',
                WHITE);
        ...
        9: begin
            ...
            WriteConsole(ID, ' Survivors'' task is to survive as many waves as possible',
                WHITE);
            WriteConsole(ID, ' Zombies have to kill them or eat all civilians', WHITE);
            WriteConsole(ID,
                ' When Survivors are finally defeated, teams are swapped and the second round begins.',
                WHITE);
            WriteConsole(ID, ' After two rounds game ends. The team with more points wins',
                WHITE);
        end;
        10: begin
            WriteConsole(ID, 'Infection mode:', GREEN);
            WriteConsole(ID,
                ' All players start as Survivors. When survivor dies, he joins the Zombie team. ',
                WHITE);
            WriteConsole(ID,
                ' The game ends when all Survivors die or Zombies eat all civilians',
                WHITE);
        end;
        11: begin
            WriteConsole(ID, 'Butchery mode:', GREEN);
            WriteConsole(ID, '  Random player becomes a boss, others have to defeat him',
                WHITE);
        end;
        20: begin
            WriteConsole(ID, 'The Last Stand v' + S_VERSION + ' by tk and others:', WHITE);
            WriteConsole(ID, ' Coding: tk, Falcon, TheOne, Saike, Spkka, MetalWarrior, Gizd',
                WHITE);
            ...
            WriteConsole(ID,
                '  Horo, MetalWarrior, Monsteri, Falcon, TheOne, L[0ne]R, Steppenwo1f for the maps',
                WHITE);
            WriteConsole(ID, '  breach for The Infection weapon mod', WHITE);
            WriteConsole(ID, '  ... and the entire Eat This community for support and ideas',
                WHITE);
            WriteConsole(ID, 'Join us @ ' + ADV_SITE +
                ', #soldat.eat-this! @ Quakenet', WHITE);
        end;

        // Help
        22: begin
            WriteConsole(ID, 'Welcome to Last Stand, a zombie survival gamemode.', WHITE);
            if (GameRunning) or (StartGame) then
            begin
                if Player[ID].Participant <> 0 then
                begin
                    WriteConsole(ID,
                        ' /task      - Shows your current task and task-specific commands.', WHITE);
                    WriteConsole(ID, ' /list      - Shows a list of players with their tasks.',
                        WHITE);
                end
                else
                begin
            ...
            begin
                WriteConsole(ID,
                    ' /vote      - Makes a vote for starting the game or changing the map', WHITE);
            end;
...
begin
    ...
    begin
        WriteMessage(ID, ' ' + IntToStr(j) + '. ' + iini.Section[i].Key[j].Name +
            '=' + iini.Section[i].Key[j].Value, ADM_INT1);
    end;
end;
...
begin
    Result := True;
    CrossFunc([GameRunning, Players_StatusNum(1), civilians, NumberOfWave,
        (Timer.Value div 60), zombiesKilled, Modes.RealCurrentMode],
        CrossAddressMsg + '.InputServerInfo');
end;
...
begin
    ...
    begin // call #0
        ...
        if News.Active then
        begin
            if (Timer.Value div 60) mod News.Time = 1 then
            begin
                incB(News.Last, 1);
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    WriteConsole(0, IntToStr(UPCountDown) + '...', $CCCC99);
            end;
        end
        else
        begin
            if Ticks mod 12600 = 0 then
            begin // 60*210
    ...
    if Timer.Cycle = 0 then
    begin // Game Running, Call #0
        if RespSG then
        begin
            RespSG := False;
            Statguns_Respawn();
...
{$IFDEF FPC}AZSS.{$ENDIF}
            ...
            begin
                ...
                for i := 1 to MaxID do
                    if Players[i].Active then
                        if Players[i].Human then
                            if player[i].Status = 0 then
                                if not PlayerBlocked(i, False, False) then
                                begin
                                    if Modes.CurrentMode = 2 then
                                    begin
                                        if a then
                                            if ticks < Game.MAXPLAYERS - Game.MAXPLAYERS div 2 then
                                                ...
                                                WriteConsole(i,
                                                    'There are free slots in the surivors team, type /joinS to join as a surivor', WHITE);
                                        if b then
                                            if mi < Game.MAXPLAYERS div 2 then
                                                WriteConsole(i,
                                                    'There are free slots in the zombie team, type /joinZ to join as a zombie', WHITE);
                                    end
                                    else
                                    ...
                                    if SurvivalSlotsNum > 0 then
                                    begin
                                        WriteConsole(i,
                                            'There are free slots in the game, type /joinS to join', WHITE);
                                    end;
                                end;
            end;
        end;
    ...
    begin
        ...
        if Timer.Cycle = 20 then
        begin
        end
        else

        if Timer.Cycle = 30 then
        begin
            {>} ProcessIntro();
        end
        else

        if Timer.Cycle = 40 then
        begin
            {>} Kits_Process();
            {>} FMSG_Process();
        end
        ...
        if Timer.Cycle = 50 then
        begin
            if mechanic > 0 then
            begin
                if Players[mechanic].Alive then
                begin
    ...
    begin // mod <> 10
        ...
        if Timer.Cycle mod 10 = 6 then
        begin // (6 hz) 6, 16, 26, 36, 46, 56
            if
{$IFDEF FPC}EarthQuake.{$ENDIF}
            EarthQuake.Active then
                {>} EarthQuake_Process(Timer.Cycle = 6);
            {>} Hax_OnTick();
            {>} Scarecrow_Process(Timer.Cycle = 6, Timer.Value mod 600 = 6);
        end
        else
        if Timer.Cycle mod 20 = 4 then
        begin // (3 hz) 4, 24, 44
            {>} ProcessMolotovExplosion();
        end
        else
        if Timer.Cycle mod 15 = 7 then
        begin // (4 hz) 7, 22, 37, 52
            {>} ProcessMainPlayerLoop();
        end
        else
        if Timer.Cycle mod 20 = 9 then
        begin
            if mechanic > 0 then
                {>} Wires_Process(Timer.Cycle = 9); // (3 hz) 9, 29, 49
        end
        else
        if Timer.Cycle mod 20 = 14 then
        begin
            {>} Mines_Process();    // (3 hz) 14, 34, 54
        end
        else
        if Timer.Cycle mod 30 = 8 then
        begin // (2 hz) 8, 38
    ...
    if Timer.Cycle mod 4 = 1 then
    begin // (15 hz) 1, 5, 9, ..., 57
        {>} Strike_Process(Timer.Cycle = 5);
        {>} Spray_Process();
        {>} ProcessBosses(Timer.Cycle = 33);
        {>} TaskMedic_Process(Timer.Cycle = 9);
        {>} if ForcedPower > 0 then
            ProcessPower(ForcedPower, HaxID, Boss.Power[0].Victim, 0);
    end;
    if Timer.Cycle mod 5 = 2 then
    begin // (12 hz) 2, 7, ...
        {>} WeaponMenu_OnTick();
    end;

    {>} Sentry_Process(Timer.Cycle = 5);  // (60 hz)
    if Timer.Cycle mod 3 = 1 then
    begin  // (20 hz)
        {>} Fire_Process();
    end;
...
begin
    ...
    if player[ID].mre > 0 then
        WriteConsole(ID, 'Meals: ' + IntToStr(player[ID].mre), GREEN);
end;
...
begin
    ...
    if SHOWPLAYERCOMMANDS then
    begin
        if not player[ID].Admin then
            if (player[ID].Status <> 0) and (player[ID].Task > 0) then
            begin
                WriteLn('<' + Players[ID].Name + '> (' +
                    TaskToShortName(player[ID].Task, player[ID].Status < 0) + ') ' + Text);
            end
            else
    ...
    begin
      ...
      {$IFNDEF FPC}
            case Copy(Text, 2, 4) of
                'heal', 'smok': Text :=
                        Cmmd2RealCmmd(player[ID].task + iif(player[ID].Zombie, 60, 0), 1);
                'def', 'taba': Text :=
                        Cmmd2RealCmmd(player[ID].task + iif(player[ID].Zombie, 60, 0), 2);
                'ofs', 'take': Text :=
                        Cmmd2RealCmmd(player[ID].task + iif(player[ID].Zombie, 60, 0), 3);
                'smn', 'summ', 'vict': Text :=
                        Cmmd2RealCmmd(player[ID].task + iif(player[ID].Zombie, 60, 0), 4);
                else
                    b := True;
            end;
      {$ENDIF}
    ...
    {$IFNDEF FPC}
        if not Game.Paused then case str of
                ...
                'activ', 'act': if DemoMan.ID = ID then
                    begin
                        str := LowerCase(GetPiece(Text, ' ', 1));
                        try
                            a := StrToInt(str) - 1;
                            if (a > MAX_CHARGES - 1) or (a < -1) then
                            begin
                    ...
                    else if (player[ID].task = 4) and (player[ID].Status = 1) then
                ...
                'get': begin
                    ...
                    begin
                        ...
                        if (z > 0) then  Statguns_Get(ID, z)
                            ...
                            Sentry_TryOperation(ID, retrieve);
                    end;
                    if WeaponSystem.Enabled then
                    begin
                        BaseWeapons_Get(ID);
                ...
                'mre':
                    ...
                    begin
                        ...
                        WriteConsole(ID, 'You eat your MRE and regain 50% health [' +
                            IntToStr(player[ID].mre) + ' left]', GREEN);
                ...
                'remot', 'rch': if DemoMan.ID = ID then
                        Charges_TryPlace(ID, 255, True)
                    else //255 - infinite time;
                ...
                'seppu':
                    if player[ID].status = 1 then
                        if GetPiece(Text, ' ', 1) = '' then
                            if Players_StatusNum(1) = 1 then Seppuku(ID, False)
                            else
                                WriteConsole(ID, 'You can do seppuku only when you are the last alive player',
                                    RED);
                ...
                //end;
                //Bosses
                'auto': if ID = Boss.ID then
                    begin
                        AutoBrain := not AutoBrain;
                        WriteConsole(ID, 'Automatic mode ' + iif(AutoBrain, 'enabled',
                            'disabled'), ORANGE);
                    end;
                ...
                'reloa':
                    ...
                    begin
                        ...
                        player[ID].ReloadTime := 0;
                        WriteConsole(ID, 'Reloading info mode ' +
                            iif(player[ID].ReloadMode, 'en', 'dis') + 'abled', GREEN);
                    end;
                else
                    b := True;
            end
        ...
        begin
            if PlayerPause = True then WriteConsole(ID, 'The game is paused (!unpause)', RED)
                ...
                WriteConsole(ID, 'The game is paused by an admin', RED);
            b := True;
        end;
    {$ENDIF}
    end
    ...
    {$ENDIF}
        ...
        begin
            ...
            else if z <> a then WriteConsole(Boss.ID,
                    'Can''t perform ' + PowerNumToName(z) + ' while ' + PowerNumToName(a) + ' is in progress', RED);
            TimeStats_End('OnPlayerCommand');
            exit; // if command already found then stop checking
        end;
        b := False;
  ...
  {$IFNDEF FPC}
    case str of
        'table': begin
            ...
            WriteConsole(ID, '                 | /cc1    | /cc2    | /cc3    | /cc4    |',
                SHORTCOMMAND);
            WriteConsole(ID, 'TASK             | Alt+1   | Alt+2   | Alt+3   | Alt+4   |',
                SHORTCOMMAND);
            if player[ID].Status >= 0 then
            begin
                z := MAX_TASKS;
            ...
            begin
                ...
                if (player[ID].task + team = a) then WriteConsole(ID, UpperCase(str), $FF7F50)
                else
        ...
        'quick': if player[ID].task > 0 then
            begin
                WriteConsole(ID, 'Short commands for the ' + taskToName(player[ID].task,
                    False) + ':', ORANGE);
                if player[ID].Zombie then team := 60
                else
                    team := 0;
                WriteConsole(ID, 'Quick command | Task command', SHORTCOMMAND);
                WriteConsole(ID, ' /cc1, Alt+1  - ' + Cmmd2RealCmmd(player[ID].task + team, 1),
                    INFORMATION);
                WriteConsole(ID, ' /cc2, Alt+2  - ' + Cmmd2RealCmmd(player[ID].task + team, 2),
                    INFORMATION);
                WriteConsole(ID, ' /cc3, Alt+3  - ' + Cmmd2RealCmmd(player[ID].task + team, 3),
                    INFORMATION);
                WriteConsole(ID, ' /cc4, Alt+4  - ' + Cmmd2RealCmmd(player[ID].task + team, 4),
                    INFORMATION);
            end;
    ...
    {'asd': begin
      ...
      WriteLnFile('cmmds.txt', '[/table]');
        ...
        'maps', 'voteh': begin
            WriteConsole(ID,
                'Use /vote ID or /vote <name> (partial map name matches) to vote for the specified map, example: /vote 1',
                WHITE);
            WriteConsole(ID,
                'Other vote options: "/vote rand" for a random map, "/vote next" for the next map',
                WHITE);
        ...
        'modeh':
            try
                ...
                begin
                    ...
                    if (z < 1) or (z > MAX_MODES) then
                    begin
                        WriteConsole(ID, 'There are only ' + IntToStr(MAX_MODES) +
                            ' modes available', WHITE);
                        TimeStats_End('OnPlayerCommand');
                        exit;
                    end;
                    if Mode[z].Enabled then
                    begin
                        WriteInfo(ID, z + 6);
                    end
                    else
      ...
      {try
        ...
        'info': begin
            SayToPlayer(ID, 'The server is running the official Last Stand mod, version ' +
                S_VERSION);
        ...
        begin//if neither of commands above match then check command in the 2nd case of
            if Cop.ID = ID then
            ...
            case LowerCase(GetPiece(Text, ' ', 0)) of
                '/t', '/T': if (Modes.CurrentMode <> 1) and ((GameRunning) or (StartGame)) then
                    begin
                        Text := Copy(Text, 4, length(Text) - 3);
                        if Text <> nil then
                        begin
                            a := player[ID].Participant;
                            if a = 1 then str := '(SURIVORS)'
                            else if a = -1 then str := '(ZOMBIES)'
                            else
                                str := '(SPEC)';
                            ...
                            WriteConsole(ID, 'Syntax error, type /t <text> (example ''/t hi!'')', RED);
                    end;
                    ...
                    try
                        ...
                        begin
                            ...
                            begin
                                ...
                                if Copy(Text, 1, 4) = 'rand' then
                                begin
                                    a := VOTE_RANDOM;
                                end
                                else
                                    for i := 0 to MapList.Length - 1 do
                                        if ContainsString(LowerCase(MapList.List[i]), Text) then
                                        begin
                                            a := i + 1;
                                            break;
                                        end;
                                ...
                                begin
                                    WriteConsole(ID,
                                        'No matching map name found, type /votehelp for the available maplist', RED);
                                    TimeStats_End('OnPlayerCommand');
                                    exit;
                                end;
                            end
                            ...
                            begin
                                WriteConsole(ID,
                                    'Syntax: /vote <ID> or /vote <mapname> (partial name matches, at least 3 letters long, prefix not required)',
                                    RED);
                                TimeStats_End('OnPlayerCommand');
                                exit;
                            end;
                ...
                '/autovote': begin
                    ...
                    WriteConsole(ID, 'Auto vote ' + iif(Player[ID].AutoVote,
                        'enabled', 'disabled'), GREEN);
                end;
                    ...
                    begin
                        for i := 0 to MapList.Length - 1 do
                            ...
                            begin
                                ...
                                WriteConsole(ID,
                                    'Use /votehelp for more information, /list for list of votes', GREEN);

                                TimeStats_End('OnPlayerCommand');
                                Exit;
                            end;
                        WriteDebug(5, 'Votemap(try) started by ' + Players[ID].Name +
                            ': ' + GetPiece(Text, ' ', 1));
                    end
                    else
                        WriteConsole(ID, 'Only Last Stand maps (ls_*) can be played on this server.', RED);
                ...
                '/mode': begin
                    ...
                    try
                        ...
                        if i > MAX_MODES then
                            WriteConsole(ID,
                                'No matching mode found, type /modehelp for the available modelist', RED);
                    end;
                end;

                '/place':
                    if DemoMan.ID = ID then
                    begin
                        try
                            Charges_TryPlace(ID, StrToInt(GetPiece(Text, ' ', 1)), False);
                        except
                            WriteConsole(ID, 'Syntax: /place <time>, ie /place 3', RED);
                        end;
                    end
                    else
                        WriteConsole(ID, 'You are not the demolition expert', RED);
                '/bug': begin
                    ...
                    begin
                        WriteLnFile('bugz.txt', FormatDate('mmm/dd hh:nn') +
                            ' [' + IDToIp(ID) + '] <' + Players[ID].Name + '> ' + Text);
                        WriteConsole(ID, '"' + Text + '"', ORANGE);
                        WriteConsole(ID,
                            'Thank you for reporting the bug, your IP and nickname have been saved', ORANGE);
                        ...
                        WriteConsole(ID, 'Syntax error, type /bug <text>', RED);
                end;
            end;
        end;
    end;
...
begin
  ...
  {$IFNDEF FPC}
    case copy(Text, 2, 6) of
                ...
                begin
                    ...
                    begin
                        ...
                        begin
                            ...
                            if j < 0 then j := 0;
                            for i := j to GetArrayLength(file2) - 1 do
                                WriteMessage(ID, file2[i], INFORMATION);
                        end;
                    end;
                end;
            except
        ...
        'revive':
            try
                TaskMedic_RevivePlayer(StrToID(GetPiece(Text, ' ', 1), ID));
            except
            end;
        ...
        'list2': begin
            ...
            for i := 1 to MAX_UNITS do
                ...
                begin
                    sleep(4);
                    WriteMessage(ID, Players[i].Name + StringOfChar(' ',
                        22 - length(Players[i].Name)) + 'part:' + IntToStr(player[i].Participant) +
                        ' team:' + IntToStr(players[i].team) + '  stat:' + IntToStr(player[i].Status) +
                        '  ' + TaskToShortName(player[i].task, player[i].Zombie) + '     ' +
                        floattostr(RoundTo(Players[i].x, 2)) + ' ' + floattostr(
                        RoundTo(Players[i].y, 2)), WHITE);
                end;
        end;
        'list3': if id > 32 then
            begin
                for i := 1 to MAX_UNITS do
                    if Players[i].Active then
                        if not player[i].Zombie then
                            WriteMessage(ID, Players[i].Name + StringOfChar(' ',
                                22 - length(Players[i].Name)) + 'part:' + IntToStr(player[i].Participant) +
                                '  stat:' + IntToStr(player[i].Status) + ' task: ' +
                                TaskToShortName(player[i].task, player[i].Zombie) +
                                StringOfChar(' ', 6 - Length(TaskToShortName(player[i].task, player[i].Zombie))) +
                                ' vote:' + iif(player[i].VoteReady, IntToStr(player[i].VotedMap), ''), GREEN);
            end;
        'start': begin
            WriteConsole(0, 'Game reset', GREEN);
            PerformStartMatch := True;
        end;
        'stop': endGame();
        'demoeq': begin
            player[DemoMan.ID].charges := MAX_CHARGES - 1;
            player[DemoMan.ID].Mines := MAX_MINES;
        end;
        ...
        'free': if GameRunning then
                WriteMessage(ID, 'Free tasks: ' + FreeTasksString(False), WHITE);
        ...
        'bugs': WriteMessage(ID, GetPiece(ReadFile('bug_n'), '-', 0) +
                ' bugs reported', GREEN);
        ...
        'status': begin
            ...
            case Modes.CurrentMode of
                1: begin
                    k := CalculatePoints();
                    Data := Data + ', score: ' + IntToStr(k) +
                        iif(k < MapRecord.Survival.Value, ' (' +
                        IntToStr(k - MapRecord.Survival.Value) + ')', '');
                end;
                2: begin
                    if VersusRound = 1 then
                        Data := Data + ' [Team 1]: ' + IntToStr(Score[1][1]) + ' [Team 2]: ' +
                            IntToStr(Score[2][1])
                    else
                        Data := Data + br + ' [Team 1]: Current: ' + IntToStr(Score[1][2]) +
                            '; Total: ' + IntToStr(Score[1][1] + Score[1][2]) + ' [Team 2]: Current: ' +
                            IntToStr(Score[2][2]) + '; Total: ' + IntToStr(Score[2][1] + Score[2][2]);
                end;
                //not included 3 as there's an idea to remove score from infection at all
            end;
            WriteMessage(ID, Data, WHITE);
        end;
        'fsentr': if ID <= 32 then
            begin
                GetPlayerXY(ID, x, y);
        ...
        'nws': begin
            ...
            WriteMessage(ID, 'New WeaponSystem ' + iif(WeaponSystem.Enabled,
        ...
        'forcem': begin
            ...
            WriteMessage(ID, 'Forced Mode: ' + iif(Config.ForceMode, 'enabled',
                'disabled'), WHITE);
        end;
        'godmod': begin
            player[ID].GodMode := not player[ID].GodMode;
            WriteMessage(ID, 'God mode: ' + iif(player[ID].GodMode, 'enabled',
                'disabled'), WHITE);
        end;
        else
            case Copy(Text, 2, 2) of
                'bl': begin
                    ...
                    if ID <= 32 then WriteConsole(ID, 'Player ' + Players[j].Name +
                            ' blinded for ' + IntToStr(i) + ' seconds.', GREEN)
                    else
                        WriteDebug(10, 'Player ' + Players[j].Name + ' blinded for ' +
                            IntToStr(i) + ' seconds.');
                ...
                'dp': begin
                    ...
                    WriteMessage(ID, 'X: ' + FloatToStr(player[i].X) + ', Y: ' +
                        FloatToStr(player[i].Y), WHITE);
                    ...
                    if i = Sharpshooter then
                        WriteMessage(ID, Players[i].Name + ' is Sharpshooter', WHITE);
                    ...
                    if i = DemoMan.ID then
                        WriteMessage(ID, Players[i].Name + ' is Demolition Expert', WHITE);
                ...
                'fw': begin
                    ...
                    try
                        str := GetPiece(Text, ' ', 2);
                        if str <> nil then i := WeapStrToInt(str)
                        else
                            i := Players[ID].Secondary.WType;
                        j := WeapStrToInt(GetPiece(Text, ' ', 1));
                        Weapons_Force(ID, j, i, iif(j <> 255, WeaponConfig[j].Ammo, 0),
                            iif(i <> 255, WeaponConfig[i].Ammo, 0));
                    except
                ...
                'so':
                    try
                        ...
                        begin
                            ...
                            statgun[SG.Num].X := x;
                            statgun[SG.Num].Y := y;
                            statgun[SG.Num].reference :=
                                Objects_Spawn(statgun[SG.Num].X, statgun[SG.Num].Y - 15, 15);
                            SG.Num := SG.Num + 1;
                        end
                        else
                            Objects_SpawnX(ID, i, 1);
                    except
                        WriteMessage(ID, S_ERROR_MSG, RED);
                        exit;
                    end;
                ...
                'st': if ID <= 32 then
                        try
                            if not ((Text[4] <> '') and ((Ord(Text[4]) - 48 < 0) or (Ord(Text[4]) - 48 > 9)))
                            then i := Pos(' ', Text);
                            j := StrToIntDef(Copy(Text, 4, i - 4), ID);
                            if player[j].Status < 0 then
                                player[j].task := StrToIntDef(GetPiece(Text, ' ', 1), player[j].task)
                            else
                            begin // everyeverything was ok here
                                i := TaskStrToInt(GetPiece(Text, ' ', 1));
                                if i > 0 then
                                begin // 0 means invalid argument
                                    ...
                                    if j <> ID then
                                        WriteMessage(ID, Players[j].Name + ' is now ' + TaskToName(i,
                                            player[j].participant = -1), WHITE)
                                    else
                                        WriteMessage(ID, 'You''re now ' + TaskToName(i, player[j].participant = -1), WHITE);
                                end
                                else
                                    WriteMessage(ID, S_ERROR_ARG + '/' + S_ERROR_MSG, RED);
                            end;
                    ...
                    case GetPiece(Text, ' ', 0) of
                        ...
                        '/callfunc': begin
                            ...
                            for i := 0 to GetArrayLength(file2) - 2 do
                            begin
                            ...
                            try
                                ...
                                WriteMessage(ID, 'Error: ' + ExceptionToString(ExceptionType,
                                    ExceptionParam), RED);
                            end;
                            try
                                if res <> Null then
                        ...
                        '/s':
                            try
                                if Sentry_SetStat(GetPiece(Text, ' ', 1),
                                    StrToInt(GetPiece(Text, ' ', 2))) then
                                    WriteMessage(ID, 'No such stat', RED);
                            except
                                WriteMessage(ID, 'Invalid arguments', RED)
                            end;
                        '/spell', '/cast':
                        begin
                            if GetPiece(Text, ' ', 1) = 'list' then
                            begin
                                for i := 60 to 90 do
                                begin
                            ...
                            try
                                ...
                                if (str = 'rand') or (str = '') then
                                    i := lookForTarget(HaxID, x, y, 0, 400, True)
                                else
                                    i := StrToInt(str);
                        ...
                        '/civs':
                            try
                                Civilians := Civilians + StrToInt(GetPiece(Text, ' ', 1));
                            except;
                                WriteMessage(ID, S_ERROR_MSG, RED);
                            end;
                        ...
                        '/resp', '/respz': begin
                            ...
                            begin
                                case LowerCase(str) of
                                    'last': if Players[LastJoinedID].Active then k := LastJoinedID
                                        else
                                            exit;
                                    else
                                    begin
                            ...
                            begin
                                ...
                                begin
                                    ...
                                    if i <> Player[k].Task then
                                    begin
                                        if (i <> 0) and (player[k].task <> 0) then
                                        begin
                                            OnCommand_(P, '/untask ' + IntToStr(k) + ' ' +
                                                IntToStr(Player[k].Task));
                                        end;
                                    end;
                                end
                                else
                                ...
                                begin
                                    ...
                                    begin
                                        ...
                                        WriteMessage(ID, S_ERROR_ARG + '/ID ' + IntToStr(k) + 'not active', RED);
                                end
                                else
                                    WriteMessage(ID, S_ERROR_ARG + '/No Task given', RED);
                            end;
                        end;
                        ...
                        '/untask':
                            try
                                ...
                                TaskAvailable[i] := TaskAvailable[i] + 1;
                                WriteMessage(ID, TaskToName(i, False) + ' task has been removed from ' +
                                    Players[j].Name, WHITE);
                            except
                                WriteMessage(ID, S_ERROR_ARG + '/' + S_ERROR_MSG, RED);
                                exit;
                            end;
                        '/setm':
                            try
                                str := GetPiece(Text, ' ', 1);
                                if (StrToInt(str) < 1) or (StrToInt(str) > MAX_MODES) then
                                begin
                                    WriteMessage(ID, 'there are only ' + IntToStr(MAX_MODES) +
                                        ' modes available', RED);
                                end
                                else
                                begin
...
begin
    ...
    begin
        ...
        P.WriteConsole('Do NOT press the chat button [default T] before typing the command',
            INFORMATION);
                ...
                'pause', 'p': if (GameRunning) then
                        if not Game.Paused then
                            ...
                            begin
                                ...
                                WriteConsole(ID, 'Only survivors can pause the game.', RED)
                        else
                            WriteConsole(ID, 'Game is ' + iif(PlayerPause, 'already paused',
                                'already paused by an admin'), RED);
                        ...
                        if (Game.Paused) then
                            if (UPCountDown = 0) then
                                if (player[ID].status = 1) then
                                    ...
                                    WriteConsole(ID, 'Only survivors can unpause the game.', RED)
                            else
                                WriteConsole(ID, 'Game is already unpausing.', RED)
                        else
                            WriteConsole(ID, 'Game isn''t paused.', RED);
          ...
          if News.Active then begin
            ...
            for i := 0 to News.Height[News.Last] do
              WriteConsole(0, News.Text[News.Last][i], News.Color);
                ...
                'score': if Game.TickCount - ScoreAntifloodTime >= 300 then
                    begin
                        ScoreAntifloodTime := Game.TickCount;
                        DisplayScore(0);
                    end
                    else
                        WriteConsole(0, 'Antiflood: wait ' + IntToStr(1 +
                            (300 - Game.TickCount + ScoreAntifloodTime) div 60) + ' seconds', RED);

                'statu': if (GameRunning) or (StartGame) then
                    begin
                        case Modes.CurrentMode of
                            1: begin
                                points := CalculatePoints();
                                WriteConsole(0, 'Survival (' +
                                    iif(Modes.RealCurrentMode = 1, 'Hardcore', 'Veteran') + ' mode) - Wave: ' +
                                    IntToStr(NumberOfWave) + '; Score: ' + IntToStr(points) +
                                    iif(points < MapRecord.Survival.Value, ' (' +
                                    IntToStr(MapRecord.Survival.Value - points) + ')', ''), WHITE);
                            end;
                            2: WriteConsole(0, 'Versus - Round: ' + IntToStr(VersusRound) +
                                    '/2; Wave: ' + IntToStr(NumberOfWave), WHITE);
                            3: WriteConsole(0, 'Infection - Wave: ' + IntToStr(NumberOfWave), WHITE);
                        end;
                    end
                    else
                    begin
                        Text := Mode[Modes.RealCurrentMode].Name + '  [map vote: ' +
                            IntToStr(MapVotes_VotedNum()) + '/' + IntToStr(Players_HumanNum);
                        i := Modes_VotedPlayersNum();
                        if i > 0 then Text :=
                                Text + ', mode vote: ' + IntToStr(i) + '/' + IntToStr(Players_HumanNum) + ']'
                        else
                            Text := Text + ']';
                ...
                'fixsg': if Players[ID].Alive then
                    begin
                        if Game.TickCount - SG.FixTimer >= 1200 then
                        begin
                            ...
                            begin
                                ...
                                WriteConsole(0, 'You can''t use fixsg during statgun build or retrieve process', RED);
                        end
                        else
                            WriteConsole(0, 'Antiflood: wait ' + IntToStr(1 + (1200 - Game.TickCount + SG.FixTimer) div
                                60) + ' seconds', RED);
                    end
                    else
                        WriteConsole(0, 'Only alive players can use this command.', RED);


                'rules': if Game.TickCount - RulesAntiflood >= 600 then
                    begin
                        WriteInfo(0, 5);
                        ...
                        WriteConsole(0, 'Antiflood: wait ' + IntToStr(1 +
                            (600 - Game.TickCount + RulesAntiflood) div 60) + ' seconds', RED);
                'sbug', 'spect':
                    if Players[ID].Alive then
...
begin
    ...
    if player[ID].justjoined then
    begin
        SetTeam(5, ID, True);
        P.WriteConsole(' Welcome to {LS} Last Stand ' + S_VERSION +
            ' - Use /help to get started', WHITE);
    ...
    begin
        ...
        case player[ID].Status of
            0: begin
                SetTeam(5, ID, True);
                P.WriteConsole('The game is currently running. Wait until the round ends',
                    INFORMATION);
            end;
...
begin
    ...
    if not player[ID].Respawned then
    begin
...
procedure OnFlagGrab_(P: TActivePlayer; TFlag: TActiveFlag; Team: byte;
    GrabbedInBase: boolean);
var
    i, ID, h: byte;
    X, Y: single;
begin
    ...
    begin
        ...
        else if ID = Satan.ArtifactID then h := MAXHEALTH div 3
        else
            h := MAXHEALTH;
        ...
        if (not Players[ID].Alive) and (Player[ID].Task <> 91) then
            player[ID].KickTimer := 2;
        civilians := civilians - 1;
        Players.WriteConsole('Zombie slipped through. ' + LSMap.CivsName + ' left: ' +
            IntToStr(civilians), RED);
        Base_ReturnFlag();
...
begin
    ...
    {$IFDEF FPC}TDamageType.{$ENDIF}
        SentryGun: Result := player[VID].Resistance.SentryGun;
...
function OnDamage_ApplyWeaponResistance(VID, SID: integer; weap: byte;
    Special: boolean): single;
begin
    if SID = Sentry.ID then
    begin
        Result := player[VID].Resistance.SentryGun;
    end
    else
        case weap of
            WTYPE_SPAS12: if Special then
                begin
            ...
            WTYPE_BOW2: Result := (player[VID].Resistance.Heat +
                    player[VID].Resistance.Explosion) * 0.5;
            WTYPE_LAW: if Special then
                begin
...
function OnDamage_ApplyHeadshotChance(Victim, Shooter: tActivePlayer;
    BulletID: byte): single;
var
    p: single;
    d: single;
begin
    if Players[Victim.ID].Y - Map.Bullets[BulletID].Y > 18.0 then
    begin
...
function OnZombieDamage(Shooter, Victim: TActivePlayer; Damage: single;
    BulletId: byte; Special: boolean): single;
var
    dmg, boost: single;
    resistance: single;
    VID, SID: integer;
...
begin
    ...
    begin
        ...
        begin
            dmg := Damage * player[SID].DamageFactor /
                OnDamage_ApplyResistance(VID, Damage_Type);
            if Damage_Debug then
            begin
                WC('Zombie, direct, rel: ' + FormatFloat('0.0', Damage) +
                    '->' + IntToStr(round(dmg)) + ', ' + OnDamage_DebugType());
            end;
        end;
        weap := WTYPE_NOWEAPON;
    ...
    begin
        ...
        if Damage_Debug then
        begin
            WC('Zombie, indirect: ' + FormatFloat('0.0', Damage) + '->' +
                IntToStr(round(dmg)) + ', weap: ' + IntToStr(weap) + ', ' +
                OnDamage_DebugWeapType(SID, weap, Special));
        end;
    end;
    ...
    if not Damage_Absolute then
        case player[VID].Task of
            ...
            1: begin  // Kamikaze zombie
                ...
                if (players[VID].Health - dmg <= 1) then
                begin
            ...
            2: begin  // Vomiting zombie
                ...
                if Victim.Human then
                    if (Timer.Value - player[VID].FlakTime >= 15) then
                    begin
            ...
            11: begin  // Ticking Bomb
                ...
                if (players[VID].Health - dmg <= 1) then
                begin
                    if player[VID].KamiDetonate = 0 then
                    begin
                ...
                if (Timer.Value - player[VID].FlakTime >= 120) then
                begin
                    player[VID].FlakTime := Timer.Value;
                    Botchat(VID, '^Tick');
                end;
            ...
            3, 5, 6, 7, 8, 9: begin
                ...
                player[VID].bitten := True;
                // Text with bosses hp will be displayed in AppOnIdle.
                if player[VID].Task = 8 then
                    if Satan.ArtifactID > 0 then
                    begin
                        dmg := dmg * 0.0001;
                        player[VID].bitten := False;
                    end;
                if VID <> SID then
                begin    // Let sp grow when they are fightng the boss.
                    ZombieFightTime := Timer.Value;
                end;
            end;
    ...
    if not Damage_Direct then
        ...
        begin
            case weap of
                ...
                WTYPE_RUGER77:
                    if not SkipWeaponEffect then
                    begin
                        boost := OnDamage_ApplyHeadshotChance(Victim, Shooter, BulletID);
                        if boost > 0.1 then
                        begin
                            dmg := dmg * (1.0 + boost);
                            BigText_DrawMap(SID, 'Head!', 60, $CCCC66, 0.04, Round(Victim.X - 10.0),
                                Round(Victim.Y - 30.0));
                        end;
                    end;
                WTYPE_BARRETT: begin
                    ...
                    begin
                        ...
                        begin
                            dmg := dmg * (1.0 + boost);
                            BigText_DrawMap(SID, 'Head!', 60, $CCCC66, 0.04,
                                Round(Victim.X - 10.0), Round(Victim.Y - 30.0));
                        end;
                        if player[SID].Task = 5 then
                            dmg := dmg * Math.Pow(player[VID].Resistance.General, 0.333);
                    end;
                    Victim.SetVelocity(Victim.VelX + Map.Bullets[BulletID].VelX *
                        0.04, Victim.VelY + Map.Bullets[BulletID].VelY * 0.04);
                end;
                WTYPE_MINIGUN: begin // flak cannon
                    if Timer.Value - player[SID].FlakTime > 10 then
                    begin
                        if RandInt(1, 1000) <= FLAKCRITCHANCE then
                        begin
                ...
                WTYPE_CHAINSAW: begin
                    ...
                    if not RayCast(Shooter.x, Shooter.y - 10, Victim.x, Victim.y -
                        10, True, True, True) then dmg := dmg * 0.7;
                    if not RayCast(Shooter.x + 5, Shooter.y - 5, Victim.x + 5, Victim.y -
                        5, True, True, True) then dmg := dmg * 0.7;
                    if not RayCast(Shooter.x - 5, Shooter.y - 5, Victim.x, Victim.y -
                        5, True, True, True) then dmg := dmg * 0.7;
                end;
            end;
        end;
    ...
    begin
        ...
        if dmg > player[VID].MaxDamagePerHit then
        begin
            dmg := player[VID].MaxDamagePerHit;
        end;

        // Limit amout of damage per second for the zombie.
        if player[VID].DamagePerSec + dmg > player[VID].MaxDamagePerSec then
        begin
    ...
    if player[VID].Boss then
        if (SID = Sentry.ID) or (player[SID].Status = 1) then
        begin
    ...
    if Damage_Debug then
        WC('Z result: ' + FormatFloat('0.0', Result) + ' (' +
            FloatToStr(dmg) + ') -----------');
end;

// When a Dummy bot is damaged
function OnDummyDamage(Shooter, Victim: TActivePlayer; Damage: single;
    BulletId: byte): single;
begin
    if player[Shooter.ID].Boss then
    begin
        case player[Shooter.ID].Task of
            6, 7, 8: begin
                if Victim.ID = Sentry.ID then
                begin
                    Sentry_Clear(True);
                end
                else if Victim.ID = Scarecrow.ID then
                begin
                    Scarecrow_Kaboom();
                end;
            end;
...
function OnSurvivorDamage(Shooter, Victim: TActivePlayer; Damage: single;
    BulletId: byte; Special: boolean): single;
var
    VID, SID: integer;
    tmp: single;
...
begin
    ...
    begin
        ...
        begin
            Damage := Damage * player[SID].DamageFactor /
                OnDamage_ApplyResistance(VID, Damage_Type);
    ...
    begin
        ...
        begin
            ...
            weap := WTYPE_NOWEAPON;
        if Damage_Debug then WC('Surv, indirect, rel: ' +
                FormatFloat('0.##', Damage) + ', weap: ' + IntToStr(weap));
    end;

    if not Damage_Absolute then
        if VID <> SID then
        begin
            if player[SID].Zombie then
            begin
                case player[VID].Task of
                    // Farmer
                    4: Damage := Math.Pow(Damage, 0.5);
                    // Priest. Lower input damage during exorcism.
                    7: if player[VID].ExoTimer < 0 then
                        begin
                            Damage := Math.Pow(Damage, 0.5);
                            if BulletID > 0 then
                                BigText_DrawMap(0, '*', 80, BLUE, RandFlt(0.04, 0.08),
                                    Round(Map.Bullets[BulletID].X), Round(Map.Bullets[BulletID].Y));
                        end;
                end;
...
function OnPlayerDamage_(Shooter, Victim: TActivePlayer; Damage: single;
    BulletId: byte): single;
var
    Special: boolean;
begin
    if player[Victim.ID].GodMode then
    begin
        Result := -99999;
    ...
    if (not Damage_Healing) and (Damage < 0) then
    begin
        Special := True;
        Damage := -Damage;
    end;
    ...
    // Victim is a zombie
    if player[Victim.ID].Zombie then
    begin
    ...
    if Players[Victim.ID].Dummy then
    begin
...
begin
    PerformProgressCheck := True;
    if not Player[Killer.ID].Zombie then player[Killer.ID].kills :=
            player[Killer.ID].kills + 1;
    if zombiesLeft > 2 then player[Victim.ID].KickTimer := 3
    else
        player[Victim.ID].KickTimer := 2;
    ...
    if Victim <> Killer then
        ...
        if (Killer.ID = mechanic) or (Killer.ID = medic.ID) then
        begin
...
begin
    ...
    begin
        ...
        begin
            ...
            if player[Victim.ID].task = 5 then
                Weapons_Force(Victim.ID, WTYPE_BARRETT,
                    iif(player[Sharpshooter].molotovs > 0, WTYPE_KNIFE, WTYPE_NOWEAPON), 255, 255);
            Exit;
        end
        else
        begin
            Players.WriteConsole(taskToName(player[Victim.ID].task, False) +
                ' ' + Players[Victim.ID].Name + ' has been killed!', RED);
            SurvPwnMeter := SurvPwnMeter + 0.2;
            if SurvPwnMeter > 1.0 then SurvPwnMeter := 1.0;
        end;
    end
    ...
    begin
        Players.WriteConsole(taskToName(player[Victim.ID].task, False) +
            ' ' + Players[Victim.ID].Name + ' has been infected!', RED);
        SurvPwnMeter := (2.0 + SurvPwnMeter) / 3.0;
        if Modes.CurrentMode = 2 then
            if Player[Killer.ID].Status < 0 then Player[Killer.ID].Survkills :=
                    Player[Killer.ID].Survkills + 1;
...
begin
    ...
    else if player[Victim.ID].Status < 0 then
    begin
        player[Victim.ID].Status := -2;
        player[Victim.ID].SpawnTimer := -1;
        // it will be set properly soon in InfectedDeath, just for now so he's not accidentally spawned
    end;
    ...
    //Victim is a Survivor
        ...
        OnSurvivorKill(Killer, Victim, BulletId);
...
begin
    ...
    begin
        ...
        begin
            if Modes.CurrentMode = 2 then
            begin
                if VersusRound = 1 then VSStats[7] := ID
                else
                    VSStats[8] := ID;
                SlotInfoCountdown := 2;
            end;
    ...
    if player[ID].Status > 0 then
    begin
        if player[ID].task > 0 then
        begin
            Players.WriteConsole(TaskToName(player[ID].task, False) + ' ' +
                Players[ID].Name + ' has left the battle', INFORMATION);
            player[ID].Participant := 0;
            if WeaponSystem.Enabled then
            begin
                BaseWeapons_Refresh(False);
    ...
    begin
        ...
        if Players[ID].Dummy then
        begin
            if ID = scarecrow.ID then
            begin
                scarecrow.ID := 0;
                scarecrow.owner := 0;
            end
...
begin
    ...
    if Game.TickCount > 60 then
    begin// if the script has been recompiled propably
        if ReadTmpFlag('spawnsmodified') = '1' then
        begin // if spawns on the map may be messed
      {$IFDEF FPC}AZSS.{$ENDIF}
    ...
    Command('/UNPAUSE');
    if Command('/REALISTIC') = 1 then MAXHEALTH := 65
    else
        MAXHEALTH := 150;
  ...
  {$IFNDEF FPC}
    WaveMessages := ['HERE THEY COME!', 'Does this never end?',
        'The undead advance', 'Their numbers seem limitless', 'My god!',
        'Come to daddy!', 'The dead rise again', 'May god has mercy on our souls',
        'Hail to the King, baby', 'This... is my BOOM STICK',
        'No brains and a big mouth', // 11
        'They''re coming for you! ', 'Groovy.', 'For God''s sake!',
        'How do you stop them?', 'Got you, didn''t I?!',
        'Buckle up boneheads!', 'Honey, you got reeal ugly! ',
        'Keep your filthy bones outta my mouth', 'That''s it, go ahead and run',
        'Run home and cry to mama!', // 21
        'I''ll cut off your gizzard ', 'Get offa me, ya crazy bitch!',
        'I got a bone to pick with you ', 'Who wants some?',
        'Yo! She-bitch! Let''s go', 'Now whoa right there spinach chin!',
        'Pick up a shovel and get digging!', 'Surrounded by evil',
        'I''ll swallow your soul!', 'We''ve been savaged', // 31
        'Groovy!', 'What an excellent day for an exorcism',
        'Be afraid... Be very afraid', 'The dead are not quiet',
        'Fresh meat', 'It smells like a graveyard',
        'We all go a little mad sometimes', 'I can smell your brains',
        'Freaky', 'Zombies almost had my ass for dinner!', // 41
        'God has fled. Hell reigns. Darkness prevails',
        'Good... bad... I''m the guy with the gun', 'That''s why you''re dead, asswipe.' +
        br + 'No brains and a big mouth!',
        'Stand back, boy! This calls for some divine intervention!',
        'Don''t you know what''s goin'' on out there?' +
        br + 'This is no Sunday School picnic!', 'We may not enjoy living together,' +
        br + 'but dying together isn''t going to solve anything',
        'When there''s no more room in hell,' + br + 'the dead will walk the earth',
        'Well, that isn''t stopping them from walking around',
        'Don''t bury dead, first shoot in head', 'Ha ha ha ha ha ha!'];
  {$ENDIF}
