...
procedure MapRecord_SaveElement(var Map: string; Data: string; index: char;
    AppendIndex: boolean);
...
procedure MapRecord_ClearFile(Map: string);
begin
    if Map = MapRecord.BuffFile.MapName then
    begin
...
begin
    ...
    begin
        ...
        if MapRecord.BuffFile.Fstr <> '' then
        begin
            MapRecord.BuffFile.MapName := Map;
...
procedure MapRecord_SaveFile();
var
    i, x, k, max_i: smallint;
    h, n: byte;
    ignore: array of boolean;
begin
    if not MapRecord.BuffFile.Saved then
    begin // overwrite on hdd only if the  file has been modifed
        ...
        begin
            h := 0;
            for i := 0 to max_i do
            begin // save entries in alfabetic order of indexes, so the hash of the entire file always is the same
        ...
        WriteFile(MapRecord.Path + MapRecord.BuffFile.MapName + '.txt',
            MapRecord.BuffFile.Fstr);
        MapRecord.BuffFile.Saved := True;
    end;
end;
...
begin
    if Map <> MapRecord.BuffFile.MapName then
    begin // if file kept in memory is different than requested one then reload it
    ...
    if MapRecord.BuffFile.Loaded then
        try
            for i := 0 to GetArrayLength(MapRecord.BuffFile.Entries) - 1 do
                if MapRecord.BuffFile.Entries[i] <> '' then
                    if MapRecord.BuffFile.Entries[i][1] = index then
                    begin
                        Result := Copy(MapRecord.BuffFile.Entries[i], 3, 9999);
                        exit;
                    end;
        except
            WriteDebug(10, 'Error occured parsing map record for ' + Map +
                ' (' + index + '): ' + ExceptionToString(ExceptionType, ExceptionParam));
        end;
end;
...
begin
    MapRecord_ReadFile(Map);
    if not MapRecord.BuffFile.Loaded then
    begin
        MapRecord_Create(Map, False);
    end;
    ...
    begin
        ...
        begin // if some empty line found (?)
            if AppendIndex then
            begin
                MapRecord.BuffFile.Entries[i] := index + ':' + Data;
            end
    ...
    try
        case Modes.CurrentMode of
            1: begin
                ...
                MapRecord.Line[1] := ' Score: ' + arr[0];
                // + '  (in ' + iif(arr[2] = '1', 'classic', 'hardcore') + ' mode)';
                MapRecord.Line[2] := ' Zombies Killed: ' + arr[3] + ',  Civilians alive: ' + arr[4];
                MapRecord.Line[3] := ' Waves survived: ' + arr[5] + ',  Time Survived: ' +
                    IntToStr(t div 60) + ':' + iif(t mod 60 <= 9, '0', '') + IntToStr(t mod 60);
                ...
                for i := 0 to j do
                begin
                    if Length(MapRecord.Line[Line]) > 100 then
                    begin
                        Line := Line + 1;
            ...
            2, 3: begin
                for x := 0 to 1 do
                    for i := 0 to INF_RECORD_POSITIONS - 1 do
                    begin
                ...
                for x := 0 to 1 do
                begin// two teams, survivors and zombies
                    SetArrayLength(MapRecord.Line, Line + 1);
                    if x = 0 then
                    begin
                        if arr[x] <> '' then
                    ...
                    for i := 0 to j - 1 do
                    begin
                        MapRecord.TopPlayers.Team[x][i].Points :=
                            StrToIntDef(GetPiece(players[i], chr(175), 0), 0);
                    ...
                    begin
                        ...
                        h := 0;
                        for i := 0 to INF_RECORD_POSITIONS - 1 do
                        begin // sorting the scores in the list
                            if not added[x][i] then
                            begin
                                if MapRecord.TopPlayers.Team[x][i].Points > h then
                                begin
                        ...
                        if n = 255 then break;
                        // break this loop loop if all players have been placed on the list
                        if MapRecord.TopPlayers.Team[x][n].Points > 0 then
                        begin
                            SetArrayLength(MapRecord.Line, Line + 1);
                            MapRecord.Line[Line] :=
                                '  - ' + MapRecord.TopPlayers.Team[x][n].Name + ': ' +
                                IntToStr(MapRecord.TopPlayers.Team[x][n].Points);
                            added[x][n] := True;
                            Line := Line + 1;
                        end;
                    end;
        ...
        MapRecord.Survival.Value := 0;
        MapRecord.TopPlayers.Loaded := False;
        WriteDebug(7, 'Error loading highscore for ' + Map + '(' +
            IntToStr(Modes.CurrentMode) + '): ' + ExceptionToString(ExceptionType, ExceptionParam));
    end;
    //end;
end;
...
begin
    ...
    begin
        WriteConsole(0, 'The record for this map (' + Map +
            ') has been deleted by an admin', PINK);
...
procedure IC_Initialize();
begin
    IC.ModuleID := Crossfunc([Script.Name, IC_MODULE_PREFIX, True],
        IC_PATH + '.RegisterModule');
    if IC.ModuleID > 0 then
    begin // if registation successful
        IC.Active := True;
    ...
    begin
        IC.Active := False;
        WriteDebug(10,
            'Couldn''t register LS module, Interserver Communication script v2.2+ required (/scripts/'
            + IC_PATH + ')');
    end;
end;
    ...
    begin
        ...
        for ServID := 1 to Length(IC.Serv) - 1 do
            if ServID <> IC.CurrentServ then
                if IC.Serv[ServID].Style = 2 then
                begin
                    Crossfunc([Packet, IC.ModuleID, ServID], IC_PATH + '.SendModuleMsg');
                end;
end;
...
begin
    ...
    if (index = 's') or (index = '*') then
        str := str + 's:' + uIntToHex(FNV32(MapRecord_ReadElement(Map, 's'))) +
            PACKET_SEPARATOR;
    // checksum of the survival record
    if (index = 'i') or (index = '*') then
        str := str + 'i:' + uIntToHex(FNV32(MapRecord_ReadElement(Map, 'i'))) +
            PACKET_SEPARATOR;
    // checksum of the infection record
    if (index = 'v') or (index = '*') then
        str := str + 'v:' + uIntToHex(FNV32(MapRecord_ReadElement(Map, 'v')));
    // checksum of the infection record
    ...
    begin
        ...
        for i := 1 to Length(IC.Serv) - 1 do
            if i <> IC.CurrentServ then
                if IC.Serv[i].Style = 2 then
                begin// send request to all ls type servers
                    IC_SendData(str, i);
                end;
end;
    ...
    for i := 1 to Length(IC.Serv) - 1 do
        if i <> IC.CurrentServ then
            ...
            begin// send request to all ls type servers
                ...
                IC_SendData('h ' + Map + PACKET_SEPARATOR + hash, i);
                // send hash of the map to all the servers
            end;
end;
    ...
    for i := 1 to Length(IC.Serv) - 1 do
        if i <> IC.CurrentServ then
            ...
            begin// send request to all ls type servers
                if buff = '' then
                begin
                    buff := 'm ' + Map + PACKET_SEPARATOR + IntToStr(wave) +
                        PACKET_SEPARATOR + IntToStr(points);
                end;
                IC_SendData(buff, i);
            end;
end;
...
procedure BindNSelectTopPlayerRec(var Map: string; var Packet: string; index: char);
...
begin
    if Packet <> '' then
    begin // bind player arrays from file and received packet, select the best players only
        str := MapRecord_ReadElement(Map, index);
        if str <> '' then
        begin
            ...
            begin
                ...
                begin
                    ...
                    for j := 0 to i do
                    begin // remove player if his name already exists in the array (result of binding two records)
                        if i <> j then
                        begin
                            if players[x][i].Name = players[x][j].Name then
                            begin
                                if players[x][i].Points > players[x][j].Points then players[x][j].Points := 0
                                else
                                    players[x][i].Points := 0;
                                break;
                            end;
                        end;
                    end;
                ...
                k := 0;
                while k < INF_RECORD_POSITIONS do
                begin // choose only INF_RECORD_POSITIONS players (default 3)
                    n := 255;
                    h := 0;
                    for i := 0 to l - 1 do
                    begin
                        if players[x][i].Points > 0 then
                        begin
                            if players[x][i].Points > h then
                            begin
    ...
    case GetPiece(Packet, ' ', 0) of
        ...
        'h': begin // <mapname> <separator> <hash>
            ...
            begin
                ...
                if hash <> GetPiece(Packet, PACKET_SEPARATOR, 1) then
                begin // if hashes are diffrent, ask for exact score values
        ...
        'H': begin // if hash of the specific record entries received
            ...
            if arr[0] <> nil then
            begin // map
                for i := 1 to GetArrayLength(arr) - 1 do
                begin
                    ...
                    begin
                        ...
                        if hash <> Copy(arr[i], 3, $FFFF) then
                            ...
                            case index of
                                ...
                                'i', 'v': begin
                                    if str = nil then
                                    begin // if local infection/vs record doesn't exists then request it
                                        Data := Data + index + ':~' + PACKET_SEPARATOR;
                                    end
                                    else // else send back local record so the second server parses it, compares with existing one, binds and sends back a new one
                    ...
                    begin
                        ...
                        case index of
                            's': begin
                                ...
                                if Q > P then
                                begin // request survival record part
                                    Data := Data + 's:~';
                                end
                                else
                            ...
                            's', 'i', 'v': begin
                                ...
                                begin
                                    ...
                                    begin
                                        ...
                                        if (Modes.CurrentMode = 1) and (CurrentMap = arr[0]) then
                                        begin
                                            WriteConsole(0,
                                                'Received higher record for this map from another LS server, high score has been updated',
                                                PINK);
        ...
        'm': begin
            Delete(Packet, 1, 2);
            arr := Explode2(Packet, PACKET_SEPARATOR, False);
            WriteConsole(0, 'High score for ' + arr[0] + ' has been beaten on server #' +
                IntToStr(ServID) + '! (Wave: ' + arr[1] + ', Points: ' + arr[2] + ')', PINK);
        end;
    end;
  {$endif}
end;
...
begin
    ...
    if not IC.ReceivedResponse then
    begin
        for i := 1 to Length(IC.Serv) - 1 do
...
var
    i, l: shortint;
begin
    if ((Modes.CurrentMode = 1) and (MapRecord.Survival.Value > 0)) or
        (((Modes.CurrentMode = 3) or (Modes.CurrentMode = 2)) and
        (MapRecord.TopPlayers.Loaded)) then
    begin
...
procedure MapScore_UpdateRecord(Score: integer; Send: boolean);
var
    i, killer, kills: integer;
    PlayerList, kn: string;
begin
    ...
    MapRecord_SaveElement(CurrentMap2,
        IntToStr(Score) + chr(182) + FormatDate('h:nn, d.m.yyyy') + chr(182) +
        '2' + chr(182) + IntToStr(ZombiesKilled) + chr(182) + IntToStr(Civilians) +
        chr(182) + IntToStr(NumberOfWave) + chr(182) + IntToStr(
        (Timer.Value div 60)) + chr(182) + IntToStr(kills) + chr(182) + kn +
        chr(182) + PlayerList,
        's', True);
...
begin
    for x := 0 to 1 do
    begin // both teams, survivors, infection
        for i := 1 to MaxID do
        begin
            if players[i].Active then
            begin
                if (x = 1) then
                begin // filter out survivors
                    if player[i].Participant >= 0 then continue;
                    Value := player[i].zombdamage;
                end
                else // filter out zombs
                begin
                    if player[i].Participant <= 0 then continue;
                ...
                n := 255;
                str := Players[i].Name;
                for j := 0 to INF_RECORD_POSITIONS - 1 do
                begin // choose the lowest ones from record to replace if players have higher scores
                    //    writeconsole(0, 'asd'+IntToStr(MapRecord.TopPlayers.Team[x][j].Points), red);
                    if MapRecord.TopPlayers.Team[x][j].Name = str then
                    begin // if such player already exists in record then place him on the same position
                        if Value > MapRecord.TopPlayers.Team[x][j].Points then
                        begin
                            n := j;
                            break;
                        end
                ...
                begin
                    ...
                    begin
                        WriteConsole(i,
                            'You have been added to the Infection mode, top player list of this map', PINK);
                    end
                    else
                        WriteConsole(i, 'You have been added to the Versus mode, top player list of this map',
                            PINK);
    ...
    begin // create a new record
        ...
        begin
            for i := 0 to INF_RECORD_POSITIONS - 1 do
            begin
                str := str + IntToStr(MapRecord.TopPlayers.Team[x][i].Points) +
                    chr(175) + MapRecord.TopPlayers.Team[x][i].Name;
