...
var
    i, j: integer;
begin
    DrawTextEx(Medic.ID, DTL_RADAR, '.', Time * 60, RADAR_PLAYER_COL,
        RADAR_SCALE, RADAR_X, RADAR_Y);
    for i := 0 to 1 do
    begin // packetloss
        for j := 1 to round(2 * ANG_PI / RADAR_ALPHA) do
        begin
            DrawTextEx(Medic.ID, DTL_RADAR + 1 + j, '.', Time * 60, RADAR_COL, RADAR_SCALE,
                RADAR_X + round(cos(RADAR_ALPHA * j) * RADAR_RADIUS),
                RADAR_Y + round(sin(RADAR_ALPHA * j) * RADAR_RADIUS)
                );
        end;
    end;
end;
            ...
            for a := 1 to MaxID do
                if Players[a].Active then
                    ...
                    begin
                        ...
                        DrawTextEx(Medic.ID, DTL_RADAR - a, str, 60,
                            RG_Gradient(1.0 * player[a].SpecTimer / LSMap.DeathTimer, 0.9),
                    ...
                    if Distance(X, Y, X2, Y2) < 33.0 then
                        case Damage_Heal(j, hp) of
                            1: begin
                                ...
                                BigText_DrawScreenX(DTL_HEALING, j, 'You are being healed...', 100,
                                    $3040FF, 0.08, 20, 370);
                                hp := hp div 2;
                            end;
                            2: begin
                                BigText_DrawScreenX(DTL_HEALING, ID, 'Soldier is at full health', 100,
                                    $3040FF, 0.08, 20, 370);
...
begin
    if Medic.ID > 0 then
        ...
        begin
            ...
            if PointsInRange(X, Y, Medic.X, Medic.Y, 40, False) then
            begin
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    if x1 > x2 then
                    begin
                        Players[Medic.ID].WriteConsole(Players[ID].Name +
                            ' is calling for help to your right!', GREEN);
                        Players[ID].WriteConsole('Doctor ' + Players[Medic.ID].Name +
                            ' is on your left (' + IntToStr(dx) + 'm)', GREEN);
                        BigText_DrawScreenX(DTL_NOTIFICATION, Medic.ID,
                            'Call for help! (' + IntToStr(dx) + 'm)> -->', 100, RGB(255, 80, 80), 0.1, 20, 370);
                    end
                    else
                    begin
                        Players[Medic.ID].WriteConsole(Players[ID].Name +
                            ' is calling for help to your left!', GREEN);
                        Players[ID].WriteConsole('Doctor ' + Players[Medic.ID].Name +
                            ' is on your right (' + IntToStr(dx) + 'm)', GREEN);
                        BigText_DrawScreenX(DTL_NOTIFICATION, Medic.ID,
                            '<-- <(' + IntToStr(dx) + 'm) Call for help!', 100, RGB(255, 80, 80), 0.1, 20, 370);
                    end;
                end
                else
                    Players[ID].WriteConsole('The doctor is dead!', RED);
            end
            else
                Players[ID].WriteConsole('You are the doctor...', RED);
        end
        else
            Players[ID].WriteConsole('Your team has no doctor!', RED);
    end;
end;
...
begin
    ...
    BigText_DrawScreenX(DTL_NOTIFICATION, 0, Players[ID].Name +
        ' has been revived!', 180, RGB(100, 255, 100), 0.1, 20, 370);
...
begin
    ...
    if LowerCase(Copy(Text, 2, 3)) = 'rev' then
    begin
        if Players[Medic.ID].Alive then
        begin
            TaskMedic_TryRevive();
