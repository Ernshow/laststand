...
function ClusterBomb_Spread(X, Y, Spread: single; Owner: byte;
    Time: word; N: byte; CastingCharge: boolean): boolean;
...
begin
    ...
    begin
        ...
        if not (Raycast(x, y + 3, x, y - 8, False, True, True) and
            Raycast(x - 8, y, x + 8, y, False, True, True)) then
        begin
            found := False;
            for r := 1 to 3 do
            begin
                ...
                begin
                    ...
                    if Raycast(x2, y2 - 8, x2, y2 + 3, False, True, True) then
                        if Raycast(x2 - 8, y2, x2 + 8, y2, False, True, True) then
                        begin
                            found := True;
                            break;
                        end;
            ...
            if found then
            begin // replace the spread point with a found, better one
                x := x2;
                y := y2;
            end;
        end;
        ...
        begin
            ...
            d := (Spread + RandFlt(10, 15)) / Sqrt(Bomb[a].Fragment[i].vx * Bomb[a].Fragment[i].vx +
                Bomb[a].Fragment[i].vy * Bomb[a].Fragment[i].vy);
            if not Raycast(x, y, x + Bomb[a].Fragment[i].vx * d, y + Bomb[a].Fragment[i].vy * d,
    ...
    begin // if all ids in use, spam with bullets in classic way
        for i := 1 to N do
            CreateBulletX(X, Y, RandFlt(-Spread, Spread), RandFlt(-Spread * 0.6, -Spread * 0.2),
                0.1, 4, Owner);
    end;
end;
...
begin
    for i := 1 to MAX_BOMBS do
        if Bomb[i].Active then
            ...
            begin
                for j := 0 to Length(Bomb[i].Fragment) - 1 do
                begin
                    CreateBulletX(Bomb[i].Fragment[j].x, Bomb[i].Fragment[j].y,
                        Bomb[i].Fragment[j].vx, Bomb[i].Fragment[j].vy, -10, 4, Bomb[i].Owner);
                    Damage_ZombiesAreaDamage(Bomb[i].Owner, Bomb[i].Fragment[j].x,
                        Bomb[i].Fragment[j].y, 5, 90, 4000, Explosion);
                end;
