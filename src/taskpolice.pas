...
function TaskPolice_TryBuyWeapon(x: byte; Amount: smallint;
    CheckPriceOnly: boolean): smallint;
var
    cost: smallint;
begin
    Result := -1;
    ...
    begin
        if Cop.SupplyPoints >= cost * Amount then
        begin
            Result := Amount;
        end
        else
        ...
        if (WeaponSystem.Enabled) {and (Base.Found)} and (x <> 10) then
        begin
...
begin
    ...
    begin
        ...
        begin
            if (not player[Cop.ID].JustResp) and (not WeaponMenu_IsGettingWeapon(Cop.ID)) then
            begin
                if Players_OnGround(Cop.ID, True, 20) > 0 then
                begin
                    Players[Cop.ID].WriteConsole(
                        'Welcome to the Weapon Shop, "/shop" to exit', NWSCOL);
                ...
                Players[Cop.ID].WriteConsole(
                    'Can''t open the shop now, you haven''t selected your weapons yet', RED);
        end
        else
...
begin
    ...
    Cop.Shop.HP := 0;
    Cop.Shop.Nades := 0;
    Players[Cop.ID].WriteConsole('Weapon Shop closed, supply points left: ' +
        FormatFloat('0.0', Cop.SupplyPoints), NWSCOL);
    BigText_DrawScreenX(DTL_WEAPONLIST, Cop.ID, 'Weapon Shop closed',
        150, NWSCOL, 0.07, 250, 80);
end;
...
begin
    ...
    if (x = WTYPE_USSOCOM) or (Cop.Shop.Status = 0) then
    begin
    ...
    begin
        ...
        if n > 0 then
        begin
            if (Weapon[y].Used) and (Players_ParticipantNum(1) > 1) then
            begin
                for i := 1 to MaxID do
                    if player[i].Participant = 1 then
                        if player[i].ActiveWeapons[y] then
                        begin
                            if str <> '' then str := str + ', ';
...
begin
    ...
    begin
        ...
        begin
            if i <> 11 then
                if i <> 10 then
                    if TaskPolice_TryBuyWeapon(i, 1, True) > 0 then
                    begin
                ...
                if Cop.Shop.Status > 0 then
                    for i := 1 to MaxID do // close the shop if zombie is around
                        if Players[i].Active then
                            ...
                            begin
                                if IsInRange(i, X, Y, 80, False) then
                                begin
                                    BigText_DrawScreenX(DTL_WEAPONLIST, Cop.ID, 'Zombies!' + br +
                                        'Click anything to exit', 150, $FF2222, 0.07, 250, 80);
                                    TaskPolice_Shop_Switch(False);
                                    exit;
                                end;
                            end;
                if not PointsInRange(X, Y, Cop.Shop.X, Cop.Shop.Y, 20, False) then
                begin
                    TaskPolice_Shop_Exit();
                end;
                if MainCall then str := str + IntToStr(Trunc(Cop.SupplyPoints)) +
                        ' supply points left' + br;
                case Cop.Shop.Status of
                    1: if Cop.Shop.Timer > 0 then
                        begin
                            if MainCall then
                            begin
                                Cop.Shop.Timer := Cop.Shop.Timer - 1;
                                if Cop.Shop.Timer <= 5 then
                                begin
                            ...
                            if not TaskPolice_Shop_WeapSelect(Players[Cop.ID].Primary.WType) then
                    ...
                    0: if MainCall then
                            BigText_DrawScreenX(DTL_WEAPONLIST, Cop.ID,
                                'Quitting shop' + Dots((Timer.Value div 60) mod 4) + br +
                                'Click anything to exit',
                                120, NWSCOL, 0.08, 250, 80);
                end;
                if MainCall then if Cop.Shop.Status > 0 then
                    begin
                        str := str + 'Weapons in base:' + br;
                        for i := 1 to 14 do
                            if i <> 11 then
                                if i <> 12 then
                                    if (Weapon[i].Used) or (Weapon[i].Num > 0) then
                                    begin
                                        if (player[Cop.ID].ActiveWeapons[i]) or (not Weapon[i].Used) then
                                        begin
                                            if Cop.Shop.JustBought = i then
                                            begin
                                                str := str + ' +  ';
...
begin
    Players[Cop.ID].WriteConsole(
        ' /mark       - To place a marker for an airstrike', GREEN);
    Players[Cop.ID].WriteConsole(' /strike     - To bomb the marked area: ' +
        IntToStr(STRIKECOST), GREEN);
    Players[Cop.ID].WriteConsole(' /heli       - To call a helicopter strike: ' +
        IntToStr(HELICOST), GREEN);
    Players[Cop.ID].WriteConsole(' /buy <item> - To buy the selected item', GREEN);
    Players[Cop.ID].WriteConsole('   "vest"   - Bulletproof vest (' +
        IntToStr(VESTCOST) + ' sp),    "nade" - Grenades (' + IntToStr(NADECOST) +
        ' sp)', GREEN);
    Players[Cop.ID].WriteConsole('   "medi"   - Medical kit (' +
        IntToStr(MEDICOST) + ' sp),         "clus" - Cluster nades (' +
        IntToStr(CLUSTERCOST) + ' sp)', GREEN);
    if mechanic > 0 then
    begin
        Players[Cop.ID].WriteConsole('   "wire"   - Barbed wire ( ' +
            IntToStr(WIRECOST) + ' sp),         "sg"   - Stationary gun for the mechanic (' +
            IntToStr(STATCOST) + ' sp)', GREEN);
        Players[Cop.ID].WriteConsole('   "sentry" - Sentry gun (' +
            IntToStr(SENTRYCOST) + ' sp),          "ammo" - Sentry gun ammo for the mechanic (' +
            IntToStr(SENTRYAMMOCOST) + ' sp)', GREEN);
    end;
    if DemoMan.ID > 0 then
        Players[Cop.ID].WriteConsole('   "mines"  - Mines pack [' +
            IntToStr(MINEPACK) + '] (' + IntToStr(MINECOST) +
            ' sp),        "char" -  Charges for the demolition man ( ' +
            IntToStr(CHARGECOST) + ' sp)', GREEN);
    if priest > 0 then
        Players[Cop.ID].WriteConsole('   "water"  - Holy Water for the priest ( ' +
            IntToStr(WATERCOST) + ' sp)', GREEN);
    if sharpshooter > 0 then
        Players[Cop.ID].WriteConsole('   "molo"   -  Molotovs pack for the sharpshooter (' +
            IntToStr(MOLOTOVCOST) + ' sp)', GREEN);
    if Players_IDByTask(4) > 0 then
        Players[Cop.ID].WriteConsole('   "scare"  - Scarecrow for the farmer (' +
            IntToStr(SCARECROWCOST) + ' sp)', GREEN);
    Players[Cop.ID].WriteConsole(
        ' /weaps      - Shows a list with costs of buyable weapons', GREEN);
end;
...
begin
    ...
    begin
        Players[Cop.ID].WriteConsole(
            'Type /buy <weapon name> [number] to buy weapon(s), partial name matches', WHITE);
        Players[Cop.ID].WriteConsole('  MP5:     ' + IntToStr(Weapon[2].Cost) +
            ' sp      AK-74:    ' + IntToStr(Weapon[3].Cost) + ' sp', INFORMATION);
        Players[Cop.ID].WriteConsole('  Steyr:   ' + IntToStr(Weapon[4].Cost) +
            ' sp      Minimi:   ' + IntToStr(Weapon[9].Cost) + ' sp', INFORMATION);
        Players[Cop.ID].WriteConsole('  Deagles: ' + IntToStr(Weapon[1].Cost) +
            ' sp      Ruger:    ' + IntToStr(Weapon[6].Cost) + ' sp', INFORMATION);
        Players[Cop.ID].WriteConsole('  Spas:    ' + IntToStr(Weapon[5].Cost) +
            ' sp      M79:      ' + IntToStr(Weapon[7].Cost) + ' sp', INFORMATION);
        Players[Cop.ID].WriteConsole('  Flak:    ' + IntToStr(Weapon[10].Cost) +
            ' sp', INFORMATION);
        Players[Cop.ID].WriteConsole('  Law:     ' + IntToStr(Weapon[14].Cost), INFORMATION);
        Players[Cop.ID].WriteConsole(
            'Type /shop to activate your weapon shop interface', WHITE);
    end
    else
    begin
        Players[Cop.ID].WriteConsole(
            'Type /buy <weapon name> to buy a weapon, partial name matches', WHITE);
        Players[Cop.ID].WriteConsole('  MP5:    ' + IntToStr(Weapon[2].Cost) +
            ' sp      AK-74:  ' + IntToStr(Weapon[3].Cost) + ' sp', INFORMATION);
        Players[Cop.ID].WriteConsole('  Steyr:  ' + IntToStr(Weapon[4].Cost) +
            ' sp      Minimi: ' + IntToStr(Weapon[9].Cost) + ' sp', INFORMATION);
        Players[Cop.ID].WriteConsole('  Spas:   ' + IntToStr(Weapon[5].Cost) +
            ' sp      M79:    ' + IntToStr(Weapon[7].Cost) + ' sp', INFORMATION);
        Players[Cop.ID].WriteConsole('  Law:    ' + IntToStr(Weapon[14].Cost) +
            ' sp      Flak:   ' + IntToStr(Weapon[10].Cost) + ' sp', INFORMATION);
    end;
end;
...
begin
    ...
    begin
        if Cop.ID > 0 then
            if player[Cop.ID].Status = 1 then
                if ZombieFightTime + 1800 >= Timer.Value then
                begin // don't let them keep last ozmbies alive to get sp
                    if (Timer.Value div 60) mod SPTime = 1 then
                    begin
                        Cop.SupplyPoints := Cop.SupplyPoints + 1.0;
                        Players[Cop.ID].WriteConsole('Supply points: ' +
                            IntToStr(Trunc(Cop.SupplyPoints)), ORANGE);
                        if Trunc(Cop.SupplyPoints) mod 5 = 0 then
                            Players[Cop.ID].WriteConsole(
                                'Type /supply for a list of supplies you can order', ORANGE);
                        if Cop.Shop.Active then
                            if Cop.Shop.Status > 0 then
...
procedure TaskPolice_OnNextWave();
begin
    SPTime := Trunc((SUPPLYTIME + Round(SUPPLYTIME2 / Players_HumanNum)) *
        Mode[Modes.RealCurrentMode].SPTimeFactor);
end;
...
function TaskPolice_TryBuyItem(Name: string; Amount: integer): string;
    // Amount: if zero then default value is used
var
    b, taken: boolean;
    i, x: smallint;
    cost: single;
  ...
  {$IFNDEF FPC}
    case Copy(Name, 1, 4) of
        ...
        'medi', 'med': begin
            ...
            begin
                if Objects_SpawnX(Cop.ID, OBJECT_MEDICAL_KIT, Amount) then
                    Cop.SupplyPoints := Cop.SupplyPoints - MEDICOST * Amount;
            end
            else
                b := True;
            Name := 'Medical kit';
        ...
        'nade', 'gren': begin
            ...
            if Cop.SupplyPoints >= cost then
            begin
                if Objects_SpawnX(Cop.ID, OBJECT_GRENADE_KIT, Amount) then
        ...
        'wate', 'holy': begin
            ...
            if taken then
            begin
                if Amount < 1 then Amount := WATERML
                else
                if Amount < WATERML / 2 then
                begin
                    if Amount > 0 then
                        WriteConsole(Cop.ID, 'Can''t buy less than ' + IntToStr(WATERML / 2) +
                            ' ml at once', RED);
                    Amount := WATERML;
                end;
                ...
                begin
                    ...
                    WriteConsole(priest, 'The police officer has ordered ' + IntToStr(
                        Amount) + ' ml of Holy Water for you', GREEN);
                end
                else
                    b := True;
            end
            else
                WriteConsole(Cop.ID, 'There is no priest at the moment', RED);
            Name := 'HW';
        end;
        ...
        'molo', 'mol': begin
            ...
            begin
                ...
                begin
                    ...
                    WriteConsole(sharpshooter, 'The police officer has ordered ' +
                        IntToStr(Amount) + ' Molotovs for you', GREEN);
                end
                else
                    b := True;
            end
            else
                WriteConsole(Cop.ID, 'There is no sharpshooter at the moment', RED);
            Name := 'Molotov';
        end;
        ...
        'stat', 'sg': begin
            ...
            begin
                if Amount < 1 then Amount := 1;
                if SG.Num + player[mechanic].statguns + Amount >
                    Mode[Modes.RealCurrentMode].MaxSG then
                    if 1.0 * Mode[Modes.RealCurrentMode].MaxSG - SG.Num -
                    player[mechanic].statguns < 1 then
                    begin
                        WriteConsole(Cop.ID, 'Your team can''t have more stationary guns', RED);
                        exit;
                    end
                    else
                        Amount := Mode[Modes.RealCurrentMode].MaxSG - SG.Num - player[mechanic].statguns;

                cost := Amount * STATCOST;
                if Cop.SupplyPoints >= cost then
                begin
                    Cop.SupplyPoints := Cop.SupplyPoints - cost;
                    WriteConsole(mechanic, 'The police officer has ordered ' +
                        IntToStr(Amount) + ' Stationary Gun' + pl(Amount) + ' for you!', GREEN);
                ...
                WriteConsole(Cop.ID, 'There is no mechanic at the moment', RED);
            Name := 'Stationary gun';
        end;
        ...
        'sent': begin
            ...
            if taken then
            begin
                if player[mechanic].Sentrys = 0 then
                begin
                    ...
                    begin
                        ...
                        WriteConsole(mechanic, 'The police officer has ordered a Sentry Gun for you!',
                            GREEN);
                    ...
                    WriteConsole(Cop.ID, 'The mechanic has already got a Sentry Gun', RED);
            end
            else
                WriteConsole(Cop.ID, 'There is no mechanic at the moment', RED);
            Name := 'Sentry gun';
        ...
        'ammo': begin
            ...
            begin
                ...
                begin
                    Cop.SupplyPoints := Cop.SupplyPoints - cost;
                    WriteConsole(mechanic, 'The police officer has ordered ' +
                        IntToStr(Amount) + ' sentry gun Ammo Belt' + pl(Amount) + ' for you!', GREEN);
        ...
        'mine': begin
            ...
            begin
                ...
                WriteConsole(Cop.ID, 'There is no demolition man at the moment', RED);
            Name := 'Mine';
        end;
        ...
        'char': begin
            ...
            begin
                if Amount < 1 then Amount := 1;
                if Cop.SupplyPoints >= (CHARGECOST * Amount) then
                begin
        ...
        'wire': begin
            ...
            begin
                if Amount < 1 then Amount := 1;
                if Cop.SupplyPoints >= (WIRECOST * Amount) then
                begin
                    Cop.SupplyPoints := Cop.SupplyPoints - (WiRECOST * Amount);
                    WriteConsole(mechanic, 'The police officer has ordered ' + IntToStr(Amount) +
                        ' wire' + pl(Amount) + ' for you!', GREEN);
        ...
        'scar': begin
            ...
            b := False;
            if Cop.SupplyPoints >= (SCARECROWCOST * Amount) then
            begin
                Cop.SupplyPoints := Cop.SupplyPoints - (SCARECROWCOST * Amount);
                WriteConsole(i, 'The police officer has ordered ' + IntToStr(Amount) +
                    ' scarecrow' + pl(Amount) + ' for you!', GREEN);
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ...
                        WriteConsole(Cop.ID, 'Insufficient funds to buy ' + IntToStr(Amount) +
                            'x ' + WeaponName(i), RED);
                end;
            end;
            exit;
...
begin
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    WriteConsole(Cop.ID, 'Unassigned quantifier (' + arg[i] +
                        '), proper syntax: "/buy <name> [number] <name> [numer] ... "', RED);
            end
            else
            begin
                if last <> '' then
                ...
                if res <> '' then res := res + ', ' + TaskPolice_TryBuyItem(arg[i], 0)
                    ...
                    res := TaskPolice_TryBuyItem(arg[i], 0);
                ...
                begin
                    ...
                    begin
                        if notfirst then Text := Text + ', '
                            ...
                            notfirst := True; // don't add comma before the first listed item
                    end
                    else
                    begin
    ...
    if sp <> Cop.SupplyPoints then
    begin
        if Length(Text) <= 90 then
        begin
        ...
        WriteConsole(Cop.ID, 'Supply points left: ' + FormatFloat('.0',
            Cop.SupplyPoints), ORANGE);
    end;

    if l = 1 then
    begin
        Cop.SingleOrders := Cop.SingleOrders + 1;
        if Cop.SingleOrders mod 10 = 0 then
        begin
            WriteConsole(Cop.ID,
                'Hint: You can buy multiple items at once, example: "/buy molotovs grenade wires 3 mp5" (will buy 3 wires instead of one)',
                HINTCOL);
        end
        else
        if Cop.SingleOrders = 5 then
        begin
            WriteConsole(Cop.ID,
                'Hint: You can buy multiple items at once, example: "/buy statgun mines spas"',
                HINTCOL);
        end;
    end;
...
begin
    ...
    if Players[Cop.ID].IsAdmin = False then // so it doesn't appear twice
        if (str = '/qleqp') or (str = '/teqp') then
        begin
            TaskPolice_WriteSupplyInfo();
            exit;
        end;

    if Copy(str, 2, 4) = 'weap' then
    begin
        TaskPolice_WriteWeaponInfo();
        exit;
    end;

    if Players[Cop.ID].Alive then
    begin

        if Copy(str, 2, 3) = 'buy' then
        begin
        ...
        begin
            ...
            begin
                ...
                Players[Cop.ID].WriteConsole(
                    'Weapon Shop is disabled in current weapon system', RED);
            exit;
        end;
        ...
        begin
            ...
            begin
                if Marker.Active[0] then
                begin
                    if Marker.Active[1] then
                    begin
                        if Cop.SupplyPoints >= STRIKECOST then
                        begin
                            Cop.SupplyPoints := Cop.SupplyPoints - STRIKECOST;
                            Strike_Call(1);
                        end
                        else
                            Players[Cop.ID].WriteConsole('Insufficient points', RED);
                    end
                    else
                        Players[Cop.ID].WriteConsole('Marked area can''t be bombed from the air', RED);
                end
                else
                    Players[Cop.ID].WriteConsole('Marker not set, type /mark to set a marker', RED);
            end
            else
                Players[Cop.ID].WriteConsole('Strike already in progress', RED);
            exit;
        end;
        ...
        begin
            ...
            begin
                ...
                begin
                    ...
                    begin
                        if Cop.SupplyPoints >= HELICOST then
                        begin
                        ...
                        Players[Cop.ID].WriteConsole('Marked area can''t be struck by the helicopter', RED);
                end
                else
                    Players[Cop.ID].WriteConsole('Marker not set, type /mark to set a marker', RED);
            end
