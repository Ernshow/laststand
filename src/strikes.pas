...
var
    a, b, c: smallint;
    X, Y: single;
    DeltaFactor, DeltaAngle: single;
    inrange: boolean;
begin
    if Strike.Active then
    begin
        if (Strike.Style = 1) then
        begin // bomb strike
            if MainCall then
            begin // 1hz
                if Strike.Timer > 0 then
                begin
                    WriteConsole(0, 'Incoming Airstrike [' + IntToStr(Strike.Timer) + ']', GREEN);
                    for a := 0 to 4 do
                        CreateBulletX(Strike.X - 250 + a * 100, Strike.Y - Strike.HeightDiff -
                            400 - a * (Strike.Shift * 20), 0, 11 - a * Strike.Shift, 0, 14, Strike.Owner);
                end
                else
                begin
                    if Strike.Timer = 0 then
                    begin
                        ...
                        Damage_ZombiesAreaDamage(Strike.Owner, Strike.X, Strike.Y, 180,
                            260, 4000, Explosion);
                    ...
                    else if Strike.Timer = -1 then
                    begin
                        Fire_CreateBurningArea(Strike.X - 40, Strike.Y, 20, 300, 14, 5, Strike.Owner);
                    end
                    else if Strike.Timer = -2 then
                    begin
                        Damage_ZombiesAreaDamage(Strike.Owner, Strike.X, Strike.Y, 180, 260, 4000, Heat);
                        Strike.Active := False;
                    end;
                    if Strike.Timer <> -2 then
                    begin
                        b := RandInt(-40, 40);
                        for a := 0 to RandInt(4, 5) do
                        begin
                            ThrowBomb(Strike.X - 100 * Strike.Shift - 240 + a * 90 + b,
                                Strike.Y - Strike.HeightDiff - 400 - a * (Strike.Shift * 50),
                                RandFlt(3, 4) * Strike.Shift, RandFlt(8, 9) - a, Strike.Owner, Strike.Timer = -1);
                        end;
                    end;
        ...
        begin // helicopter strike

            if MainCall then
            begin // 1hz
                if Strike.Timer = HELITIME then
                begin
                    WriteConsole(0, 'Helicopter strike incoming in ' + IntToStr(HELITIME) +
                        ' seconds!', GREEN);
                end
                else
                begin
                    ...
                    begin
                        ...
                        for a := 1 to 5 do
                        begin
                            if Raycast(Strike.X, Strike.Y, Strike.X - Strike.Shift * b,
                                Strike.Y2, True, False, False) then
                            begin
                    ...
                    else if Strike.Timer < 0 then
                    begin // strike in progress
                        if -Strike.Timer >= HELIATTACKTIME then
                        begin
                            Strike_Reset();
                            WriteConsole(0, 'The helicopter flies away', GREEN);
                            exit;
                        end;
                        if Strike.CoolDown = 0 then
                        begin
                            Strike.Target := lookForTarget(Strike.Owner, Strike.X2, Strike.Y2,
                                100, 800, True);
                        end
                        else
                        begin
                        ...
                        if strike.Timer2 > 0 then
                        begin
                            Strike.Timer2 := Strike.Timer2 - 1;
                        end
                        else
                        if Strike.Target > 0 then
                        begin
                            if Strike.Timer2 = 0 then
                            begin // switching ammo
                                for a := 1 to MaxID do // find a group of zombies
                                    ...
                                    begin
                                        if PlayersInRange(a, Strike.Target, 310, False) then //TODO: false?
                                            if player[a].Zombie then c := c + 1
                                            else
                                            if player[a].Status = 1 then
                                            begin
                                                if PlayersInRange(a, Strike.Target, 150, False) then
                                                begin
                                                    c := 0;
                                                    break;
                                                end;
            ...
            begin
                if Strike.AmmoType = 1 then
                begin // Gatling
                    if not Players[Strike.Target].Alive then
                    begin
                        Strike.AimTimer := 0;
                    ...
                    begin
                        ...
                        if Strike.AimTimer <= 0 then
                        begin
                            Strike.AimTimer := 1;
                            GetPlayerXY(Strike.Target, X, Y);
                            Y := Y - 10.0;
                            Strike.CurrentAngle := BallisticAimX(Strike.X2, Strike.Y2 - 100,
                                30, Players[Strike.Target], InRange);
                        end
                        else
                            Strike.AimTimer := Strike.AimTimer - 1;

                        if Strike.CurrentAngle < 0.0 then Strike.CurrentAngle :=
                                Strike.CurrentAngle + ANG_2PI;
                        ...
                        if Strike.LastDeltaAngle = 0 then
                        begin
                            Strike.LastDeltaAngle := DeltaAngle;
                        end
                        else
                        begin
                            if DeltaAngle > ANGLE_5 then
                            begin
                                DeltaAngle := ANGLE_5;
                            end
                            else if DeltaAngle < -ANGLE_5 then
                                DeltaAngle := -ANGLE_5;
                        end;
                        ...
                        Strike.CurrentAngle := Strike.LastAngle + Strike.LastDeltaAngle * DeltaFactor +
                            DeltaAngle * (1.0 - DeltaFactor);
                        Strike.LastAngle := Strike.CurrentAngle;
                        Strike.LastDeltaAngle := DeltaAngle;

                        if Strike.Recoil < 1.0 then
                        begin
                            Strike.Recoil := Strike.Recoil + 0.08;
                            if Strike.Recoil > 1.0 then Strike.Recoil := 1.0;
                        end;
                        Strike.HeliFireN := Strike.HeliFireN + 1;
                        Shoot(Strike.X2, Strike.Y2 - 100.0, Strike.CurrentAngle, 0, 0,
                            28, 32, 0, Strike.Recoil * 3.0 * DEG_2_RAD, -HELI_DMG, 14, Strike.Owner, 1);
                        PlaySound(0, 'dist-gun3.wav', Strike.X2, Strike.Y2);
                        if Strike.HeliFireN >= 35 then
                        begin
                ...
                begin // missiles
                    ...
                    begin
                        Strike.HeliFireN := Strike.HeliFireN + 1;
                        if Strike.HeliFireN mod 2 = 0 then
                        begin
                            for a := 1 to MaxID do // find a group of zombies
                                if player[a].Status = 1 then
                                begin
                            ...
                            if Strike.HeliFireN = 2 then c := 4
                            else
                                c := 12;
                            if Strike.HeliFireN = 3 then DeltaAngle := 0.0
                            else
                                DeltaAngle := ANGLE_10; // make at least one missile accurate
                            X := Players[Strike.Target].VELX / 2;
                            Shoot(Strike.X2, Strike.Y2 - 100, Strike.CurrentAngle, X, 0,
                                18, 22, 0, DeltaAngle, -20, c, Strike.Owner, 1);
                            PlaySound(0, 'law.wav', Strike.X2, Strike.Y2);
                            if Strike.HeliFireN / 2 >= RandInt(3, 4) then
                            begin
...
begin
    ...
    if not RayCast2(0, 40, 200, Marker.X, Marker.Y) then
    begin
        ...
        if Raycast(Marker.X, Marker.Y - 5, Marker.X, Marker.Y - 500, False, True, True) then
        begin
            Marker.Active[1] := Raycast(Marker.X - 150, Marker.Y - 400, Marker.X + 150,
                Marker.Y - 400, False, True, True);
            Marker.Active[2] := Raycast(Marker.X - 250, Marker.Y - 400, Marker.X + 250,
                Marker.Y - 400, False, True, True);
            if Marker.Active[2] then
                Marker.Active[2] := Raycast(Marker.X, Marker.Y - 700, Marker.X,
                    Marker.Y - 100, False, True, True);
        end;
        WriteConsole(ID, 'Marked area can' + iif(Marker.Active[1], '', 'not') +
            ' be bombed from the air', iif(Marker.Active[1], GREEN, RED));
        WriteConsole(ID, 'Marked area can' + iif(Marker.Active[2], '', 'not') +
            ' be struck by the helicopter', iif(Marker.Active[2], GREEN, RED));
        BigText_DrawMapX(WTL_MARKER, Marker.Owner, '!', 99999999,
            iif(Marker.Active[1] and Marker.Active[2], $22FF22, $FF2222), 0.05,
            Trunc(Marker.X), Trunc(Marker.Y));
        BigText_DrawMapX(WTL_MARKER, Marker.Owner, '!', 99999999,
            iif(Marker.Active[1] and Marker.Active[2], $22FF22, $FF2222), 0.05,
            Trunc(Marker.X), Trunc(Marker.Y));
    end
