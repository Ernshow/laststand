...
type
    tSentry = record
        Active: boolean;
        ID, Owner, FireDelay, FireCountdown, Style, AmmoTimer, Hp, Status,
        State, Timer, AmmoType: byte;
...
function Sentry_SetStat(Stat: string; Value: integer): boolean;
// use for oncommand only!
...
begin
    if Sentry.ID > 0 then
        if Players[Sentry.ID].Active then
        begin
...
const
    SQR_SENTRY_RANGE = SENTRY_RANGE * SENTRY_RANGE;

procedure Sentry_LookForTargetRC(X, Y: single);
var
    i: byte;
    X2, Y2, Vx, Vy, DistSqr, MaxSqrDist: single;
                ...
                if Players[i].Alive then
                    if player[i].Zombie then
                        ...
                        begin
                            ...
                            Vy := Y2 - Y;
                            DistSqr := Vx * Vx + Vy * Vy;
                            if DistSqr < MaxSqrDist * 1.44 then
                            begin
                                head :=
                                    RayCast(X, Y - 10, X2, Y2 - 16, False, True, True);
                                feet :=
                                    RayCast(X, Y - 10, X2, Y2 - 3, False, True, True);
                                if not (head or feet) then continue;
                                if head and feet then DistSqr := DistSqr * 0.64;
                                mid :=
                                    RayCast(X, Y - 10, X2, Y2 - 10, False, True, True);
                                if not mid then DistSqr := DistSqr * 1.44;
                                if DistSqr < MaxSqrDist then
                                begin
                                    Sentry.Aim.Target := i;
                                    MaxSqrDist := DistSqr;
                                end;
...
const
    SENTRY_iAIMSPEED = 1 - SENTRY_AIMSPEED;

procedure Sentry_Fire();
var
    X, Y, X2, Y2, ang: single;
    target_changed, InRange: boolean;
label
    start;
begin
    if Sentry.Aim.Target > 0 then
    begin
        if Players[Sentry.Aim.Target].Alive then
        begin
            start:
                ...
                begin
                    ...
                    ang := BallisticAimX(X, Y, Sentry.Speed,
                    Players[Sentry.Aim.Target], InRange);
                    if InRange then
                    begin
                        if ang < 0 then ang := ang + ANG_2PI;
                        if Sentry.FireAngle <> 0 then
                            if Abs(ShortenAngle(ang - Sentry.FireAngle)) < ANGLE_70 then
                            begin
                                if Sentry.FireAngle <> 0 then
                                begin
                                    ang :=
                                        ang * SENTRY_AIMSPEED + Sentry.FireAngle * SENTRY_iAIMSPEED;
                                end;
                            end
                            else if target_changed then exit;
                        ...
                        Shoot(X, Y, ang, 0, 0, Sentry.Speed - 2, Sentry.Speed + 2, 0,
                        Sentry.Accuracy * Sentry.Recoil, -Sentry.Damage,
                        Sentry.AmmoType, Sentry.ID, 1);
...
var
    ...
    hp: shortint;
begin
    if Sentry.Hp > dmg then Sentry.Hp := Sentry.Hp - dmg
    else
        Sentry.Hp := 0;
    if Sentry.Hp = 0 then
    begin
...
begin
    ...
    begin
        case Sentry.Status of
            1: begin // gun ready to fire
                ...
                for i := 1 to MaxID do
                    if Players[i].Alive then
                        if player[i].Zombie then
                            if IsInRange(i, X, Y, 20, False) then
                            begin
                                Sentry_OnDamage(1);
                                break;
                            end;
                if Sentry.State <= 3 then
                begin
                    if Sentry.Ammo > 0 then
                    begin
                        if Sentry.State = 1 then
                        begin
                        ...
                        begin
                            ...
                            begin
                                ...
                                begin
                                    ...
                                    Sentry.Timer := 0;
                                    Weapons_Force(Sentry.ID, WTYPE_BOW2,
                                        WTYPE_NOWEAPON, 1, 1);
                                end;
                            end
                            else
                            begin
                                if Sentry.Recoil > 0 then
                                    Sentry.Recoil := Sentry.Recoil - 0.4;
                                ...
                                if Sentry.Timer > 0 then Sentry.Timer := Sentry.Timer - 1
                                else
                                begin
                                    Sentry.State := 1; // "idle"
                                    Weapons_Force(Sentry.ID, WTYPE_NOWEAPON,
                                        WTYPE_NOWEAPON, 0, 0);
                    ...
                    begin
                        ...
                        begin
                            ...
                            Weapons_Force(Sentry.ID, WTYPE_NOWEAPON,
                                WTYPE_NOWEAPON, 0, 0);
                            WriteConsole(Sentry.Owner,
                                'Sentry has run out of ammo, load it if you have any (/ammo)',
                                ORANGE);
                        end;
                    end;
                end
                    ...
                    case Sentry.State of
                        6: if (Timer.Value div 60) mod 3 = 1 then
                            begin // broken
                ...
                if Players[Sentry.Owner].Alive then
                    if IsInRange(Sentry.Owner, X, Y, 40, False) then
                    begin
                        str :=
                            'State: ' + Sentry_State(Sentry.State) + #13#10 + 'Ammo: ' +
                            Sentry_AmmoStatus() + #13#10 + 'HP: ' + IntToStr(100 *
                            Sentry.Hp div SENTRY_HP) + '%';
                        ...
                        BigText_DrawScreenX(DTL_COUNTDOWN, Sentry.Owner, str, 120,
                            DT_CONSTRUCTION, 0.06, 20, 370);
                    end;
            end;
            2: begin // under construction
                if Sentry.Timer > 0 then
                begin
                    if (Players[Sentry.Owner].Alive) and
                        (IsInRange(Sentry.Owner, Sentry.X, Sentry.Y, 40, False)) then
                    begin
                        BigText_DrawScreenX(DTL_COUNTDOWN, Sentry.Owner,
                            'Construction [' + IntToStr(Sentry.Timer) +
                            ']', 100, DT_CONSTRUCTION, 0.08, 20, 370);
                        Sentry.Timer := Sentry.Timer - 1;
                    end
                    else
                    begin
                        BigText_DrawScreenX(DTL_COUNTDOWN, Sentry.Owner,
                            'Construction failed.', 100,
                            DT_FAIL, 0.08, 20, 370);
                        Sentry_Clear(False);
                    end;
                end
                ...
                begin
                    ...
                    WriteConsole(0, TaskToName(player[Sentry.Owner].Task, False) +
                        ' ' + Players[Sentry.Owner].Name +
                        ' has placed a sentry gun', GREEN);
                    if player[Sentry.Owner].Sentrys > 0 then
                        WriteConsole(Sentry.Owner,
                            IntToStr(player[Sentry.Owner].Sentrys) +
                            ' sentry guns left', GREEN);
                end;
            end;
            3: begin // being retrieved
                ...
                begin
                    ...
                    begin
                        BigText_DrawScreenX(DTL_COUNTDOWN, Sentry.Owner,
                            'Deconstruction [' + IntToStr(Sentry.Timer) +
                            ']', 100, DT_CONSTRUCTION, 0.08, 20, 370);
                        Sentry.Timer := Sentry.Timer - 1;
                    end
                    else
                ...
                begin
                    ...
                    WriteConsole(Sentry.Owner, IntToStr(player[Sentry.Owner].Sentrys) +
                        ' sentry guns left', GREEN);
                    Sentry_Clear(False);
                end;
            end;
                ...
                begin
                    ...
                    begin
                        ...
                        begin
                            ...
                            begin
                                str :=
                                    'Reparation: ' + IntToStr(100 * Sentry.Hp div SENTRY_HP) + '%';
                            end
                            else
                            begin
                                str := 'Sentry repaired';
                                WriteConsole(Sentry.Owner,
                                    'Reparation complete!', GREEN);
                                Sentry.Status := 1; // normal
                                Sentry.State := 1; // idle
                            end;
                            BigText_DrawScreenX(DTL_COUNTDOWN, Sentry.Owner, str, 100,
                                DT_CONSTRUCTION, 0.08, 20, 370);
                        end
                        else
                        begin
                            Sentry.Status := 1;
        ...
        if (Timer.Value div 60) mod 10 = 5 then
        begin
...
begin
    ...
    if Sentry.State = 3 then
    begin // "active" (shooting)
        if Sentry.FireCountdown > 0 then
        begin
...
begin
    ...
    begin
        if ID = Mechanic then
        begin
            if Sentry.Status > 0 then
            begin
                if (Sentry.Status <> 2) and (Sentry.Status <> 3) then
                begin
                    GetPlayerXY(Sentry.ID, Sentry.X, Sentry.Y);
                    if IsInRange(ID, Sentry.X, Sentry.Y, 40, False) then
                    begin
                        if _type = retrieve then
                        begin
                            Sentry.Status := 3; // deconstruction
                            Sentry.Timer := SENTRY_RETRIEVALTIME;
                            WriteConsole(ID,
                                'Deconstruction started, do not move!', GREEN);
                        end
                        else
                        if _type = fix then
                        begin
                            if Sentry.Hp < SENTRY_HP then
                            begin
                                if Sentry.Status <> 4 then
                                begin
                                    Sentry.Status := 4; // reparation
                                    WriteConsole(ID,
                                        'Reparation started, do not move!', GREEN);
                                    Weapons_Force(Sentry.ID, WTYPE_NOWEAPON,
                                        WTYPE_NOWEAPON, 0, 0);
                                end
                                else
                                    WriteConsole(ID,
                                        'The sentry gun is being fixed at the moment', RED);
                            end
                            else
                                WriteConsole(ID, 'The sentry is not damaged', RED);
                        end
                        else
                        begin
                            if player[ID].SentryAmmo > 0 then
                            begin
                                if Sentry.Ammo <= 4 * SENTRY_AMMOINBELT then
                                begin
                                    Sentry.Ammo := Sentry.Ammo + SENTRY_AMMOINBELT;
                                    if Sentry.State < 5 then Sentry.State := 1;
                                    WriteConsole(ID, 'Ammo belt loaded, sentry ammo: ' +
                                        IntToStr(Sentry.Ammo), GREEN);
                                    player[ID].SentryAmmo := player[ID].SentryAmmo - 1;
                                    if player[ID].SentryAmmo > 0 then
                                        WriteConsole(ID, 'Ammo belts left: ' +
                                            IntToStr(player[ID].SentryAmmo), GREEN);
                                end
                                else
                                    WriteConsole(ID,
                                        'You can''t load more ammo, maximal ammo capacity is 5 belts (' +
                                        IntToStr(5 * SENTRY_AMMOINBELT) + ')', RED);
                            end
                            else
                                WriteConsole(ID,
                                    'You don''t have any sentry gun ammo', RED);
                        end;
                    end
                    else
                        WriteConsole(ID, 'You must be near to the sentry gun', RED);
                end
                else
                    WriteConsole(ID,
                        'The sentry is now under constuction/deconstruction', RED);
            end
            else
                WriteConsole(ID, 'There is no sentry gun in game', RED);
        end
        else
            WriteConsole(ID, 'You are not the mechanic', RED);
    end
    else
        WriteConsole(ID, 'You have to be alive to get a sentry gun', RED);
end;
...
var
    ...
    i: byte;
    x, x2: single;
    sY: array[0..1] of single;
begin
    if Players[ID].Alive then
    begin
        if Players_OnGround(ID, True, 10) > 0 then
        begin
            if ID = Mechanic then
            begin
                if player[ID].Sentrys > 0 then
                begin
                    if Sentry.Status = 0 then
                    begin
                        ...
                        if not b then
                        begin
                            if Abs(sY[0] - sY[1]) <= 13 then
                            begin
                                Sentry_StartConstruction(ID);
                            end
                            else
                                WriteConsole(ID,
                                    'You cannot build a sentry gun here, ground is too steep', RED);
                        end
                        else
                        begin
                            WriteConsole(ID,
                                'You cannot build a sentry gun here, ground is too uneven', RED);
                            b := False;
                        end;
                    end
                    else if Sentry.Status = 2 then
                        WriteConsole(ID, 'The sentry is already under construction', RED)
                    else
                        WriteConsole(ID, 'There is already a sentry gun in game', RED);
                end
                else
                    WriteConsole(ID, 'You do not have any sentry guns', RED);
            end
            else
                WriteConsole(ID, 'You are not the mechanic', RED);
        end
        else
            WriteConsole(ID, 'You have to be on the ground to place a sentry gun', RED);
    end
    else
        WriteConsole(ID, 'You have to be alive to place a sentry gun', RED);
end;
