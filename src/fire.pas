...
type
    Fires_InFireCallback = procedure(ID, owner: byte);
...
/// @param owner owner
procedure Fire_CreateBurningArea(X, Y, ParticleVel, CastingRange: single;
    duration, n: integer; owner: byte);

/// Creates burning area consisiting of multiple flames from tVector;
/// @param flame Array of vectors
/// @param duration Time of burning 
/// @param owner owner
procedure Fire_CreateFromVector(var flame: array of tTVector;
    duration: integer; owner: byte);
...
begin
    ...
    while i < n * 2 do
    begin
        if i mod 2 = 0 then ang := RandFlt(-ANG_PI, -ANGLE_110)
        else
            ang := RandFlt(-ANGLE_70, 0);
        // Do some casting
        ...
        if not NotInRange then
        begin
            if not PointNotInPoly(vec.X, vec.Y, True, True, True) then
            begin
                if PointNotInPoly(vec.X, vec.Y - 10.0, True, True, True) then
                begin
                    vec.Y := vec.Y - 10.0;
                end
                else if PointNotInPoly(vec.X, vec.Y + 10.0, True, True, True) then
                begin
                    vec.Y := vec.Y + 10.0;
                end;
            end;
            // Find the next free slot
            while (i <= MAX_FIRES) do
            begin
                if Fires[i].t = 0 then
                begin
                    break;
...
begin
    while MaxFireID > 0 do
    begin
        if Fires[MaxFireID].t > 0 then
        begin
            break;
        end;
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                owner := Fires[i].owner;
                if Fires[i].t > RandInt(0, 26) then
                begin // last 4 seconds or so the fire is lowering
                    for j := 1 to MaxID do
                    ...
                    Map.CreateBullet(Fires[i].X + RandFlt(-3.0, 3.0), Fires[i].Y, 0,
                        RandFlt(-0.7, -0.3), FLAMEDMG, 5, Players[owner]);
                end;
            end;
