        ...
        case cmd of
            '/vacuum': begin
                ...
                Players[ID].WriteConsole(
                    '[Hax] Vacuum ' + iif(HaxPlayers[ID].Vacuum, 'enabled. Use the Flag Throw key.',
                    'disabled.'),
                    $64FF64);
                Result := True;
            end;
        ...
        if Players[i].Active then
            if Players[i].Alive then
                if Players[i].Team <> Players[ID].Team then
                    ...
                    begin
                        r := Distance(Players[ID].MouseAimX, Players[ID].MouseAimY,
                            Players[i].X, Players[i].Y);
                        if r < VACUUM_RANGE then
                        begin
    ...
    begin
        if Players[i].Active then
            if Players[i].Alive then
                if Players[i].Team <> Players[ID].Team then
                    if not Players[i].Dummy then
                        if not Map.RayCast(Players[ID].X, Players[ID].Y - 7.0, Players[i].X,
                            Players[i].Y - 7.0, False, False, True, True, 0) then
                        begin
                            r := Distance(0.5 * (Players[ID].MouseAimX + Players[ID].X), 0.5 *
                                (Players[ID].MouseAimY + Players[ID].Y), Players[i].X, Players[i].Y);
                            if r < max_r then
                            begin
                                t := i;
                                max_r := r;
    ...
    begin
        ...
        begin
            Map.CreateBullet(Players[ID].X, Players[ID].Y - 8.0,
                cos(r - 0.005) * AIMBOT_VELOCITY + Players[ID].VelX, sin(r - 0.005) * AIMBOT_VELOCITY +
                Players[ID].VelY,
                10, 1, Players[ID]);
            Map.CreateBullet(Players[ID].X, Players[ID].Y - 8.0,
                cos(r + 0.005) * AIMBOT_VELOCITY + Players[ID].VelX, sin(r + 0.005) * AIMBOT_VELOCITY +
...
begin
    ...
    begin
        if Players[i].Active then
            if Players[i].Alive then
            begin
                if HaxPlayers[i].Vacuum then
                begin
                    if Players[i].KeyFlagThrow then
                    begin
                        Hax_Vacuum(i);
                    end;
                end;
                if HaxPlayers[i].AimBot then
                begin
                    Hax_AimBot(i);
                end;
            end;
