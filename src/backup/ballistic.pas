...
begin
    ...
    begin
        ...
        F := x * (s / c + g / (K_AIR * vc)) + (g * Math.ln(1 - kx / vc)) / (K_AIR * K_AIR);
...
function BallisticAimX2(shooter, target: tActivePlayer; velocity, inheirted: double;
    var target_in_range: boolean): double;
begin
    Result := _bax(shooter.X, shooter.Y - 10, velocity, inheirted * shooter.VelX,
        inheirted * shooter.VelY, target, target_in_range);
end;
...
begin
    ...
    Result := _bax_solve(a, t, x0, y0, vx0, vy0, x1, y1, vx1, vy1, ax1,
        ay1, velocity, Game.Gravity / 0.06, target_in_range);
end;
