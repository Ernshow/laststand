...
type
    tAdvancedZombieSpawnSystem = record
        ...
        TmpBaseX, TmpBaseY, TmpBaseR, BaseX, BaseY, BaseR, Progression,
        ProgressionMeter, TmpZombHpFactor: single;
...
begin
    for i := 1 to MaxID do
        if (Players[i].Alive) and (player[i].Status = 0) then
        begin
...
begin
    ...
    if AZSS.ArrayNum >= 0 then
    begin
        // the num must be >= 0 anyway since this piece of code is processed, just to be sure...
...
begin
    ...
    begin
        ...
        if AZSS.Debug then WriteLn('<AZSS> Progression (' +
                IntToStr(AZSS.ArrayNum) + ')');
    end
    else
    begin
...
begin
    ...
    begin
        ...
        for i := 1 to AZSS.PointNum do
            // increment last visit time for all points by time they weren't observed
...
var
    i, index, mindistindex: smallint;
    x, y, d, sqdist, mindist: single;
    min, prev: longint;
begin
    AZSS.TmpBaseX := 0.0;
    AZSS.TmpBaseY := 0.0;
    for i := 0 to AliveSurvivors.length - 1 do
    begin
        // get the middle point of player group (arithmetic mean)
        d := d + 1.0;
    ...
    for i := 1 to AZSS.PointNum do
        if AZSS.Point[i].Active then
            if AZSS.Point[i].ZombieNum >= 10 then
            begin
                sqdist := SqrDist(AZSS.TmpBaseX, AZSS.TmpBaseY,
                    AZSS.Point[i].x, AZSS.Point[i].y);
                if sqdist < mindist then
                begin
                    mindist := sqdist;
                    mindistindex := i;
                end;
                if sqdist < d then
                begin
                    if AZSS.Point[i].AvgZombieTime < min then
                    begin
    ...
    begin
        ...
        if AZSS.Debug then if prev <> AZSS.UpperTimeLimit then WriteLn(
                    '<AZSS> base time (in range): ' +
                    IntToStr(t2s(AZSS.UpperTimeLimit)));
    end
    else
    begin
        AZSS.UpperTimeLimit := AZSS.Point[mindistindex].AvgZombieTime;
        if AZSS.Debug then if prev <> AZSS.UpperTimeLimit then WriteLn(
                    '<AZSS> base time (closest): ' + IntToStr(t2s(AZSS.UpperTimeLimit)));
    end;
end;
...
begin
    for i := 1 to AZSS.PointNum do
        if AZSS.Point[i].Active then
            WriteLn(IntToStr(i) + '. n: ' + IntToStr(AZSS.Point[i].ZombieNum) +
                ', t: ' + IntToStr(t2s(AZSS.Point[i].AvgZombieTime)));
end;
...
procedure AZSS_SetSpawn(time: longint);
var
    h, l, j, i, k, n, last_ok, numback, numfront: smallint;
    front, diff: integer;
    x, y: single;
begin
    last_ok := -1;
    for h := 0 to AZSS.ArrayNum do
    begin
        // we start looping from the most distant points to zombie base
        ...
        if h < AZSS.ArrayNum then
            ...
            k := -1;
        if AZSS.Point[j].AvgZombieTime <= AZSS.UpperTimeLimit then
        begin
            // allow only points "before" the tmp surv base
            // find two points close to this time, one on the left, on on the right
            if (j = 0) or (SqrDist(AZSS.TmpBaseX, AZSS.TmpBaseY,
                AZSS.Point[j].x, AZSS.Point[j].y) > AZSS.TmpBaseR * AZSS.TmpBaseR) then
            begin
                // point cannot be in range of surv tmp base
                for i := 0 to AliveSurvivors.length - 1 do
                begin // (*1) look for close survivors
                    GetPlayerXY(AliveSurvivors.arr[i], x, y);
                    if SqrDist(x, y, AZSS.Point[j].x, AZSS.Point[j].y) < 640000 then
                        // 800^2
                        break;
                end;
                if i >= AliveSurvivors.length then
                begin // there mustn't be any survivor close
                    last_ok := j;
                    // if we don't find two neighboring spawns, then at least save index of the last spawn that could be used (just in case) (*2)
                    if k > -1 then
                    begin
                        if IsBetween(AZSS.Point[j].AvgZombieTime,
                            time, AZSS.Point[k].AvgZombieTime) then
                        begin
                            ...
                            diff :=
                                AZSS.Point[k].AvgZombieTime -
                                AZSS.Point[j].AvgZombieTime;
                            numback := front * AZSS_TMP_SPAWN_NUM div diff;
                            numfront := AZSS_TMP_SPAWN_NUM - numback;

                            if numfront > 0 then
                            begin
                                // check if the front point is not close to survivors, for now only backpoint was checked (ad.1)
                                for i := 0 to AliveSurvivors.length - 1 do
                                begin // look for close survivors
                                    GetPlayerXY(AliveSurvivors.arr[i], x, y);
                                    if SqrDist(x, y, AZSS.Point[k].x, AZSS.Point[k].y) <
                                        640000 then
                                    begin// 800^2
                            ...
                            if j = 0 then
                            begin // index 0 means base
                                if AZSS.SpawnsModified then
                                    // activate default spawns points
                                    AZSS_SetDefaultSpawns();
                            end
                            else
                            begin // if it's not base area anymore
                                if not AZSS.SpawnsModified then
                                    // deactivate default spawns if active
                                    AZSS_DisableDefaultSpawns();
                                for l := 1 to numback do
                                begin // activate back spawns
                                    AZSS_ActivateSpawn(AZSS.TmpPoint.arr[n], j);
                                    n := n + 1;
                                end;
                            end;
                            for l := 1 to numfront do
                            begin // activate front spawns
                                AZSS_ActivateSpawn(AZSS.TmpPoint.arr[n], k);
                                n := n + 1;
                            end;
                            if AZSS.Debug then
                                WriteLn('<AZSS> spawn - time: ' +
                                    IntToStr(t2s(time)) + '/' +
                                    IntToStr(t2s(AZSS.UpperTimeLimit)) + ', ' +
                                    IntToStr(numback) + 'x' + IntToStr(
                                    t2s(AZSS.Point[j].AvgZombieTime)) + ', ' + IntToStr(numfront) +
                                    'x' + IntToStr(t2s(AZSS.Point[k].AvgZombieTime)));
                            exit; // <- (*3)
                        end;
                    end;
                end;
            ...
            break; // if came out of allowed range
    end;

    if last_ok >= 0 then
    begin
        // since we reached this place (ad.3), no two neighboring spawnpoints were found. If there is a spawnpoint which could be used though (ad.2), set it.
        AZSS_ActivateSpawn(AZSS.TmpPoint.arr[0], last_ok);
        if AZSS.Debug then WriteLn('<AZSS> spawn - time: ' +
                IntToStr(t2s(time)) + '/' + IntToStr(t2s(AZSS.UpperTimeLimit)) +
                ', ' + IntToStr(t2s(AZSS.Point[last_ok].AvgZombieTime)));
        n := 1;
    end
    else
    begin // if completely no place to spawn available
        AZSS_SetObservation(True);
        AZSS_SetProgression(False);
        if AZSS.Debug then WriteLn('<AZSS> spawn - time: ' +
                IntToStr(t2s(time)) + ', none');
        exit; // <-
    end;

    for n := n to AZSS_TMP_SPAWN_NUM - 1 do
        // if didnt find a position for some of the temp spawns, disable them
...
begin
    ...
    if AZSS.SpawnsModified then
    begin
        // if zombie have spawned on temporary spawn points, not default
        GetPlayerXY(ID, x, y);
        for i := 1 to AZSS.PointNum do // we check at what point it was spawned
            if AZSS.Point[i].Active then
                // ... and decrement time of it's respawn, by average zombie time on that point, so the zombie will look older and not mess average times on the next points with it's low, "new" time
                if (AZSS.Point[i].ZombieNum >= Azss.PointReady_THRESHOLD) then
                    if SqrDist(x, y, AZSS.Point[i].x, AZSS.Point[i].y) <
                        AZSS_SQR_POINT_R then
                    begin
                        player[ID].RespawnTime :=
                            player[ID].RespawnTime - AZSS.Point[i].AvgZombieTime;
                        if AZSS.Debug then
                            WriteLn('<AZSS> resp ' +
                                IntToStr(t2s(AZSS.Point[i].AvgZombieTime)));
                        break;
                    end;
    end;
end;
...
begin
    if AZSS.Idle then
    begin
        if AZSS.IdleTimer <= 1 then AZSS.Idle := False
        else
    ...
    begin
        RefreshAliveSurvList();
        // we will use a quick list of alive survivors in functions below, perfomance reasons

        // in this state, bots are being observed, potential spawn points are found
        if AZSS.Observation then
        begin
            ...
            begin
                ...
                begin
                    ...
                    begin // bot zombie
                        if player[i].RespawnTime = 0 then continue;
                        // if respaned, but his position hasn't been updated yet (just in case)
                        GetPlayerXY(i, x, y);
                        if SqrDist(x, y, AZSS.ObservedZombie[h].prevX,
                            AZSS.ObservedZombie[h].prevY) < 10000 then
                        begin // we check is zombie didn't get stuck
                            AZSS.ObservedZombie[h].StuckCD :=
                                AZSS.ObservedZombie[h].StuckCD + 1;
                            if AZSS.ObservedZombie[h].StuckCD >= 3 then
                            begin
                                player[i].AZSS_DontObserve := True;
                                AZSS_ResetObservedSlot(h);
                                if AZSS.Debug then
                                    WriteLn('<AZSS> zombie ' +
                                        IntToStr(i) + ' ignored (stuck)');
                                continue;
                            end;
                        end
                        else
                        ...
                        if SqrDist(x, y, AZSS.BaseX, AZSS.BaseY) >
                            AZSS.BaseR * AZSS.BaseR then
                        begin
                            // don't try to create points in base area
                            for j := 0 to AliveSurvivors.length - 1 do
                                // if zombie hasn't seen anyone
                                if SqrDist(x, y, x2, y2) < 360000 then// 600 ^ 2
                                    if RayCast(x, y - 12.0, x2, y2 -
                                        12.0, False, False, False) then
                                    begin
                                        player[i].AZSS_DontObserve := True;
                                        AZSS_ResetObservedSlot(h);
                                        break;
                                    end;
                            if j >= AliveSurvivors.length then
                            begin
                                k := -1;
                                for j := 1 to AZSS.PointNum do
                                    if AZSS.Point[j].Active then
                                    begin
                                        if not AZSS.ObservedZombie[h].Point[j] then
                                            if SqrDist(x, y, AZSS.Point[j].x,
                                                AZSS.Point[j].y) < AZSS_SQR_POINT_R then
                                            begin
                                                t := Timer.Value - player[i].RespawnTime;
                                                AZSS.ObservedZombie[h].Point[j] := True;
                                                if (t div 2 >=
                                                    AZSS.Point[j].AvgZombieTime) then
                                                    if (AZSS.Point[j].AvgZombieTime >
                                                        600) and
                                                        (AZSS.Point[j].ZombieNum >= 10) then
                                                        break;
                                                // <- if the zombie time is much higher (twice) than average time at this point, skip it
                                                // almost-arithmetic mean, older values become less significant in time
                                                p :=
                                                    Math.Pow(
                                                    AZSS.Point[j].ZombieNum, 0.8);

                                                // Timer.Value-player[i].RespawnTime represents time, in which zombie reached this point
                                                AZSS.Point[j].AvgZombieTime :=
                                                    Trunc(
                                                    (p * AZSS.Point[j].AvgZombieTime + t) / (p + 1.0));
                                                AZSS.Point[j].ZombieNum :=
                                                    AZSS.Point[j].ZombieNum + 1;
                                                if AZSS.Debug then
                                                    if (AZSS.Point[j].ZombieNum >=
                                                        AZSS.PointReady_THRESHOLD) then
                                                        WriteLn(
                                                            '<AZSS> point ready ' +
                                                            IntToStr(j) + '-> zn: ' +
                                                            IntToStr(AZSS.Point[
                                    ...
                                    if k = -1 then k := j;
                                // if we don't find anything, we will be able to use this slot
                                if j > AZSS.PointNum then
                                begin // if no point in range of zombie found
                                    if k >= 0 then j := k;
                                    if j <= AZSS_MAXPOINTS then
                                    begin // create a new one
                                        if PointNotInPoly(x, y - 10.0,
                                            True, False, False) then
                                            if Players_OnGround(i, True, 40) <> 0 then
                                            begin
                                                if j > AZSS.PointNum then
                                                    AZSS.PointNum := j;
                                                if AZSS.Debug then
                                                    WriteLn('<AZSS> new ' +
                                                        IntToStr(j) + '/' + IntToStr(AZSS.PointNum));
                                                AZSS.Point[j].Active := True;
                                                AZSS.Point[j].AvgZombieTime :=
                                                    Timer.Value - player[i].RespawnTime;
                                                AZSS.Point[j].LastVisit := Timer.Value;
                                                AZSS.Point[j].ZombieNum := 1;
                                                AZSS.Point[j].x := x;
                                                AZSS.Point[j].y := y;
            ...
            if Timer.Value >= AZSS.LastSpawnReview + 7200 then
            begin
                // from time to time (each 2 mins), clear loop though all points, and if some seem unused anymore (for 5 mins), get rid of them
                for j := 1 to AZSS.PointNum do
                    if AZSS.Point[j].Active then
                    begin
                        if Timer.Value > AZSS.Point[j].LastVisit + 18000 then
                        begin
        ...
        if AZSS.InProgress then
        begin
            if SurvPwnMeter >= 0.8 then
            begin
                if AZSS.ActivationTimer < 3 then
                begin
            ...
            else if AZSS.ActivationTimer > 0 then
            begin
                AZSS.ActivationTimer := AZSS.ActivationTimer - 1;
            end;
            ...
            if Spawn.Active then
                ...
                if AZSS.HighProgress then
                    AZSS.Progression :=
                        AZSS.Progression + (0.012 + 0.001 * NumberOfWave -
                        SurvPwnMeter / 100.0) *
                        (AZSS_PROGRESSION_FACTOR + AZSS_PROGRESSION_FACTOR_PER *
            ...
            if SurvPwnMeter > AvgSurvPwnMeter then
                AZSS.Progression :=
                    AZSS.Progression - AZSS_REGRESSION_FACTOR *
                    (SurvPwnMeter - AvgSurvPwnMeter);
            // make sure value is in range 0...1
            if AZSS.Progression < 0.0 then AZSS.Progression := 0.0
            else
            ...
            if AZSS.Progression >= AZSS.ProgressionMeter then
            begin
                // tending to current progression
                AZSS.ProgressionMeter :=
                    (AZSS.ProgressionMeter + AZSS.Progression) / 2.0;
            end
            else
            begin
                x := ZombiesLeft;
                // i wrote it this way to be sure about valid int-float conversion, so division result is ok
                x := x / MAX_ZOMBIES;
                if x > 0.9 then x := 0.9
                else
                if x < 0.3 then x := 0.3;
                ...
                AZSS.ProgressionMeter :=
                    AZSS.ProgressionMeter * x + AZSS.Progression * (1.0 - x);
            end;
            ...
            AZSS.TmpZombHpFactor :=
                1.0 - Sqr(AZSS.ProgressionMeter * SurvPwnMeter) -
                AvgSurvPwnMeter / 3.0 - AZSS.ProgressionMeter *
                AZSS.ProgressionMeter / 6.0;
            if AZSS.TmpZombHpFactor < 0.3 then AZSS.TmpZombHpFactor := 0.3;

            if AZSS.Debug then
                WriteLn('<AZSS> SPM: ' + IntToStr(Round(SurvPwnMeter * 100.0)) +
                    '%, ASPM: ' + IntToStr(Round(AvgSurvPwnMeter * 100.0)) +
                    '%, PROGRESS: ' + IntToStr(Round(AZSS.Progression * 100.0)) +
                    '(' + IntToStr(Round(AZSS.ProgressionMeter * 100.0)) +
                    ')%, ZHPF: ' + IntToStr(Round(AZSS.TmpZombHpFactor * 100.0)) + '%');

        end
        else
        begin
            if SurvPwnMeter < 0.01 then
            begin // leaning towards activation
                if AZSS.ActivationTimer < 10 then
                begin // need 15 secs to fully activate
            ...
            else // leaning towards deactivation
            if AZSS.ActivationTimer > 0 then AZSS.ActivationTimer :=
                    AZSS.ActivationTimer - 1;
...
begin
    ...
    for i := 1 to MAX_SPAWNS do
        if (Map.Spawns[i].style = HUMANTEAM) or (Map.Spawns[i].style =
            HUMANTEAM + 4) then
        begin
            d := d + 1.0;
        ...
        // and since we are looping through spawns already, let's save zombies spawns' ids
    ...
    if AZSS.Debug then WriteLn('<AZSS> Base: R = ' + IntToStr(Trunc(AZSS.BaseR)) +
            ', X = ' + IntToStr(Trunc(AZSS.BaseX)) + ', Y = ' +
            IntToStr(Trunc(AZSS.BaseY)));
    ...
    for i := 1 to MAX_SPAWNS do
        if Map.Spawns[i].style = ZOMBIETEAM then
        begin
            if SqrDist(Map.Spawns[i].x, Map.Spawns[i].y, AZSS.BaseX, AZSS.BaseY) <
                AZSS.BaseR * AZSS.BaseR then
            begin
                WriteDebug(8,
                    'AZSS_Init(): Cannot set the system for this map, spawn structure is too complicated');
                exit;
            end;
        end;
    ...
    for i := MAX_SPAWNS downto 1 do
        if Map.Spawns[i].Active = False then
        begin
