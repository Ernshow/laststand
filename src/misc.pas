...
function Explode2(Source: string; const Delimiter: string;
    Append: boolean): TStringArray;
...
procedure nova_2(X, Y, dir_x, dir_y, r, speed, power, cutoff, start: single;
    n: word; style, id: byte);

procedure nova_3(X, Y, dir_x, dir_y, r, speedmin, speedmax, power, cutoff, start: single;
    n: word; style, id: byte);
...
procedure Shoot(x, y, Angle, add_vx, add_vy, vmin, vmax, Spread,
    Accuracy, Damage: single;
    Style, ID, n: byte);
        ...
        begin
            ...
            for i := 1 to MaxID do
                if Players[i].Active then
                    if ContainsString(LowerCase(Players[i].Name), s) then
                    begin
                        Result := i;
                        break;
                    end;
...
begin
    ...
    begin
        sine := sin(start);
        cosine := cos(start);
        CreateBulletX(cosine * r + X, sine * r + Y, cosine * speed + dir_x, sine * speed +
            dir_y, power, style, ID);
        start := start + angle;
    end;
end;
