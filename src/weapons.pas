...
begin
    ...
    begin
        WeaponConfig[i].Ammo := StrToInt(INI_Get(ini, WeaponConfigName(i),
            'Ammo', '0', foundKey));
    end;
end;
...
function Weapons_IsRegular(Wtype: byte): boolean;
begin
    Result := (WType <> WTYPE_NOWEAPON) and (WType <> WTYPE_FLAMER) and
        (WType <> WTYPE_USSOCOM) and (WType <> WTYPE_KNIFE) and
        (WType <> WTYPE_BOW) and (WType <> WTYPE_BOW2);
end;
  ...
  {$ifndef FPC}
    ...
    case s2 of
        ...
        'mi', 'fl': case Copy(s, 1, 5) of
                ...
                'flame': if ContainsString(s, 'bow') then Result := WTYPE_BOW2
                    else
                        Result := WTYPE_FLAMER;
                else
                    if s2 = 'fl' then Result := WTYPE_FLAMER;
            end;
        'bo': Result := WTYPE_BOW;
