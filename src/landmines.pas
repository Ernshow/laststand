...
begin
    for a := 1 to MAX_MINES do
        if not Mines.Mine[a].placed then
        begin
            found := True;
            break;
        end;
    ...
    begin
        ...
        RayCast2(0, 4, 20, Mines.Mine[a].X, Mines.Mine[a].Y);
        // move mine pointer to the ground so marker doesn't jump
...
procedure Mines_TryPlace(ID: byte);
begin
    if player[ID].task = 2 then
    begin
        if Players[ID].Alive then
        begin
            case Players_OnGround(ID, False, 20) of
                1: if player[ID].Mines > 0 then
                    begin
                        if Mines_Place(ID) then
                        begin
                            player[ID].Mines := player[ID].Mines - 1;
                            WriteConsole(ID, 'Mine placed (' + IntToStr(player[ID].Mines) +
                                ' left)! Keep distance to activate it!', GREEN);
                        end
                        else
                            WriteConsole(ID, 'Mines limit in game exceeded', RED);
                    end
                    else
                        WriteConsole(ID, 'You do not have any mines left', RED);
                -1: WriteConsole(ID, 'You have to be on the solid ground to place a mine', RED);
                else
                    WriteConsole(ID, 'You have to be on the ground to place a mine', RED);
            end;
        end
        else
            WriteConsole(ID, 'You have to be alive to place a mine', RED);
    end
    else
...
begin
    ...
    Mines.MaxAngle :=
        40 // max angle between vectors (player_speed, distance_mine_player) in degrees
        * DEG_2_RAD; // deg -> rad
end;
...
var
    i, j, k, owner: byte;
    x, y, x2, y2, v1x, v1y, v2x, v2y, a, r: single;
begin
                    ...
                    if Players[j].Alive then
                        ...
                        begin
                            ...
                            begin
                                ...
                                begin
                                    if Players[Mines.Mine[i].owner].Alive then
                                    begin
                                    ...
                                    if IsInRange(j, Mines.Mine[i].X + v1x * Mines.PS * r /
                                        Mines.RMax, Mines.Mine[i].Y + v1y * Mines.PS * r / Mines.RMax, r, False) then
                                    begin //TODO: false?
                                        ...
                                        if r > Mines.RMin + 5 then
                                        begin
                                            // calculate angle between vectors
                                            a := Math.arccos((v1x * v2x + v1y * v2y) / Sqrt(
                                        ...
                                        CreateBulletX(Mines.Mine[i].X, Mines.Mine[i].Y - 19, 0,
                                            0, 99, 4, Mines.Mine[i].owner);
                                        CreateBulletX(Mines.Mine[i].X + 19, Mines.Mine[i].Y - 5,
                                            0, 0, 99, 4, Mines.Mine[i].owner);
                                        CreateBulletX(Mines.Mine[i].X - 19, Mines.Mine[i].Y + 5,
                                            0, 0, 99, 4, Mines.Mine[i].owner);
                                        CreateBulletX(Mines.Mine[i].X + RandFlt(-24, 24), Mines.Mine[i].Y -
                                            RandFlt(8, 16), 0, 0, 99, 4, Mines.Mine[i].owner);
                                        CreateBulletX(Mines.Mine[i].X, Mines.Mine[i].Y - 10,
                                            RandFlt(-7, 7), RandFlt(-4, 0), 0, 14, Mines.Mine[i].owner);
                                        if player[Mines.Mine[i].owner].Status > 0 then owner := Mines.Mine[i].owner
                                        else
                                            owner := 0;
                                        Damage_ZombiesAreaDamage(owner, Mines.Mine[i].X, Mines.Mine[i].Y,
                                            75, 130, 4000, explosion);
                                        WriteConsole(Mines.Mine[i].owner, 'Your mine detonated!', GREEN);
                                        // refresh max mine id
                                        if i = Mines.MaxMineID then
                                        begin
                                            for k := MAX_MINES downto 1 do
                                                if Mines.Mine[k].placed then
                                                begin
                                                    Mines.MaxMineID := k;
                                                    break;
                                                end;
