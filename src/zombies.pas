...
function Zombies_GetZombieCandidate(var ID: byte;
    AllowPunishmentZombie: boolean): boolean;

function Zombies_SpawnOne(hp, dmg: single; playerZomb, style: byte;
    x1, y1: single; ForcePos: boolean; param: byte): byte;
...
begin
    ...
    begin // decide how long he will have to wait for spawn
...
begin
    ...
    for i := 1 to MaxID do
        ...
        begin // if he's a dead human-zombie
            if player[i].BossPlayTime < max then
            begin // choose the one who has been waiting for the longest time
                max := player[i].BossPlayTime;
                Result := i;
            end;
        end;
    if (not JustCheck) and (Result > 0) then
    begin
...
begin
    ...
    for i := 1 to MaxID do
        ...
        begin // if he's a dead human-zombie
            if player[i].SpawnTimer = 0 then
            begin // if his wait time to spawn is over
                if player[i].ZombiePlayTime < max then
                begin // choose the one who has been waiting for the longest time
                    if (not player[i].PunishmentZombie.Active) or
                        (AllowPunishmentZombie and player[i].PunishmentZombie.Active) then
                    begin
...
begin
    ...
    if NumberOfWave < 4 then
    begin // waves 2, 3
        b := RandInt_(10);
        if b <= 1 then Result := 200
        else // 2/11
        if b <= 6 then Result := 2
        else // 5/11
    ...
    begin // waves 4+
        ...
        begin
            up:
                if NumberOfWave <= 5 then
                begin // waves 4, 5
                    b := RandInt_(14);
                    if b <= 2 then Result := 1
                    else // 3/15
                    if b <= 7 then Result := 2
                    else // 5/15
                    if b <= 11 then Result := 4
                    else // 4/15
                ...
                begin // waves 6+
                    ...
                    if b <= 4 then Result := 2
                    else // 3/10
                    if b <= 7 then Result := 4
                    else // 3/10
            ...
            begin
                for a := 1 to MaxID do
                    if player[a].Status = -1 then
                        if player[a].Task = Result then
                        begin
                            tryagain := True;
                            // if such zombie is already in game then try to randomize once more (for diversity of zombie team)
                            break;
                        end;
...
begin
    ...
    if playerZomb = 0 then
    begin
        case style of
            200: begin // versus spawn mode
                if Zombies_GetZombieCandidate(ID, False) then
                begin
            ...
            0, 2, 4, 91: begin // normal, vomiting, flame
                if Zombies_GetZombieCandidate(ID, style = 0) then
                begin
                    if not Zombies_Respawn(ID, ForcePos, x1, y1) then exit;
                    if player[ID].PunishmentZombie.Active then
                    begin // if he's a punishment zombie
                        hp := Sqrt(hp);
                        dmg := 1.0;
            ...
            5: begin // priest
                ...
                if ID > 0 then
                begin
                    if not Zombies_Respawn(ID, False, 0, 0) then
                    begin
