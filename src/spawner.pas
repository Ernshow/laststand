...
begin
    ...
    begin
        if (Spawn.Active) and (Spawn.CurrentWave > 0) then
        begin
            if (Spawn.Wave[Spawn.CurrentWave].Counter > 0) and
                (player[ID].task = Spawn.Wave[Spawn.CurrentWave].Style) then
            begin
                if Modes.CurrentMode > 1 then // not survival
                    if Zombies_GetZombieCandidate(temp,
                        Spawn.Wave[Spawn.CurrentWave].Style = 0) then
                    begin // if there is a human-zombie player waiting and ready to spawn, kick this zombie and make a place for him
                        player[ID].kicked := True;
                        Players[ID].Kick(TKickSilent);
                        exit; // skip the rest of code \/
                    end;
                Spawn.Wave[Spawn.CurrentWave].Counter :=
                    Spawn.Wave[Spawn.CurrentWave].Counter - 1;
                // in this case we just let the zombie bot respawn again
            end
            else
            begin
                player[ID].kicked := True;
...
begin
    for j := 1 to MAX_SPAWNWAVES do // find a slot
        if not Spawn.Wave[j].Active then
        begin
            k := j;
            break;
        end;
    ...
    case Style of
        0, 1, 2, 4: begin
            if Modes.RealCurrentMode = 2 then
                Spawn.Wave[k].ZombieHp :=
                    Trunc(ZombieHpInit + V_Z_W_HPINC * Modes.DifficultyPercent * LSMap.DifficultyPercent /
                    10000 * Math.Pow(NumberOfWave, (Modes.DifficultyPercent + EQUALIZATIONDIFF) /
                    (100 + EQUALIZATIONDIFF)))
            else
                Spawn.Wave[k].ZombieHp := Trunc(Z_W_HPINC * Modes.DifficultyPercent *
                    LSMap.DifficultyPercent / 10000 * Math.Pow(NumberOfWave,
                    (Modes.DifficultyPercent + EQUALIZATIONDIFF) / (100 + EQUALIZATIONDIFF)));
            Spawn.Wave[k].ZombieHp :=
                Trunc(Z_W_HPINIT + Z_W_HPINC * Modes.DifficultyPercent * LSMap.DifficultyPercent /
                10000 * Math.Pow(NumberOfWave, (Modes.DifficultyPercent + EQUALIZATIONDIFF) /
                (100 + EQUALIZATIONDIFF)));
            if Style = 2 then Spawn.Wave[k].ZombieHp := Spawn.Wave[k].ZombieHp + 150;
            Spawn.Wave[k].ZombieDmg :=
                Trunc(Z_W_DMGINC * Math.Pow(NumberOfWave, 0.875) * Modes.DifficultyPercent *
                LSMap.DifficultyPercent / 10000);
        ...
        3, 5: begin // butcher, priest
            case Modes.CurrentMode of
                1: begin
                    Spawn.Wave[k].ZombieHp :=
                        (10000 + NumberOfWave * 120) * Modes.DifficultyPercent *
                        LSMap.DifficultyPercent div 10000;
                    Spawn.Wave[k].ZombieDmg :=
                        500 * Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
                end;
                2: begin
                    Spawn.Wave[k].ZombieHp :=
                        (5000 + NumberOfWave * 120) * Modes.DifficultyPercent *
                        LSMap.DifficultyPercent div 10000;
                    Spawn.Wave[k].ZombieDmg :=
                        200 * Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
                end;
                3: begin
                    Spawn.Wave[k].ZombieHp :=
                        (8000 + NumberOfWave * 120) * Modes.DifficultyPercent *
                        LSMap.DifficultyPercent div 10000;
                    Spawn.Wave[k].ZombieDmg :=
                        400 * Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
                end;
            end;
        ...
        6: begin // firefighter
            Spawn.Wave[k].ZombieHp :=
                18500 + 10000 * Players_StatusNum(1) div Game.MAXPLAYERS *
                Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
        ...
        7: begin // satan
            Spawn.Wave[k].ZombieHp :=
                19000 + 10000 * Players_StatusNum(1) div Game.MAXPLAYERS *
                Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
        ...
        8: begin // satan2
            Spawn.Wave[k].ZombieHp :=
                25000 + 10000 * Players_StatusNum(1) div Game.MAXPLAYERS *
                Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
            Spawn.Wave[k].ZombieDmg := 666;
            if Players_ParticipantNum(-1) > 0 then
            begin
        ...
        11: begin // kamikaze2
            Spawn.Wave[k].ZombieHp :=
                (2500 + NumberOfWave * 120) * Modes.DifficultyPercent *
                LSMap.DifficultyPercent div 10000;
        ...
        9: begin // plague
            Spawn.Wave[k].ZombieHp :=
                18000 + 8000 * Players_StatusNum(1) div Game.MAXPLAYERS *
                Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
...
begin
    ...
    begin
        ...
        pos1:
            ...
            begin // if current wave is active then keep spawning
                if Spawn.Wave[Spawn.CurrentWave].Counter > 0 then
                begin // if something left in the wave to spawn then keep spawning
                    Style := Spawn.Wave[Spawn.CurrentWave].Style;
                    Hp := Spawn.Wave[Spawn.CurrentWave].ZombieHp;
                    if Style = 0 then
                    begin
                        ZombiePowerDivergence := Trunc(0.7 * Hp);
                        Hp := ToRangeI(100, Hp + RandInt(-ZombiePowerDivergence,
                        ZombiePowerDivergence), 10000000);
                    end;
                    if Modes.CurrentMode = 2 then //versus
                    begin
                        if Style = 0 then
                            // in VS, instead of spawning as normal zombie, make Zombies_SpawnOne decide what species it will be
                            Style := 200;
                    end
                    else
                    if Modes.RealCurrentMode = 2 then
                    ...
                    if Zombies_SpawnOne(Hp / 100, Spawn.Wave[Spawn.CurrentWave].ZombieDmg /
                    100, 0, Style, 0, 0, False, 0) > 0 then
                    begin
                ...
                begin // otherwise reset the wave, go to the begining, try to find another one
            ...
            begin // otherwise try to find another wave of spawn
                for i := 1 to MAX_SPAWNWAVES do
                    if Spawn.Wave[i].Active then
                        if Spawn.Wave[i].Counter > 0 then
                        begin
                            if ZombiesLeft > Spawn.Wave[i].ActivationNum then
                            begin
                                exit; // if there are too many zombies in game for the incoming wave then wait
                            end;
                            if Spawn.Wave[i].Style > 0 then
                            begin
