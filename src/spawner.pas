...
begin
	...
	case Style of
		0,1,2,4: begin
			if Modes.RealCurrentMode = 2 then
				Spawn.Wave[k].ZombieHp :=
					Trunc(
						ZombieHpInit
							+ V_Z_W_HPINC
							* Modes.DifficultyPercent * LSMap.DifficultyPercent / 10000
							* Math.Pow(NumberOfWave, (Modes.DifficultyPercent + EQUALIZATIONDIFF) / (100 + EQUALIZATIONDIFF))
					)
			else
				Spawn.Wave[k].ZombieHp :=
					Trunc(
			...
			if Style = 2 then
				Spawn.Wave[k].ZombieHp := Spawn.Wave[k].ZombieHp + 150;
			Spawn.Wave[k].ZombieDmg :=
				Trunc(
					Z_W_DMGINC
						* Math.Pow(NumberOfWave, 0.875)
						* Modes.DifficultyPercent * LSMap.DifficultyPercent / 10000
				);
		...
		3,5: begin // butcher, priest
			case Modes.CurrentMode of
				1: begin
					Spawn.Wave[k].ZombieHp :=
						(10000 + NumberOfWave * 120) * Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
					Spawn.Wave[k].ZombieDmg :=
						500 * Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
				end;
				2: begin
					Spawn.Wave[k].ZombieHp :=
						(5000 + NumberOfWave * 120) * Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
					Spawn.Wave[k].ZombieDmg :=
						200 * Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
				end;
				3: begin
					Spawn.Wave[k].ZombieHp :=
						(8000 + NumberOfWave * 120) * Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
					Spawn.Wave[k].ZombieDmg :=
		...
		11: begin // kamikaze2
			Spawn.Wave[k].ZombieHp :=
				(2500 + NumberOfWave * 120) * Modes.DifficultyPercent * LSMap.DifficultyPercent div 10000;
	...
	if Spawn.Active then begin
		...
		if Spawn.CurrentWave > 0 then begin // if current wave is active then keep spawning
			if Spawn.Wave[Spawn.CurrentWave].Counter > 0 then begin // if something left in the wave to spawn then keep spawning
				...
				if Zombies_SpawnOne(Hp / 100, Spawn.Wave[Spawn.CurrentWave].ZombieDmg / 100, 0, Style, 0, 0, false, 0) > 0 then
				begin
