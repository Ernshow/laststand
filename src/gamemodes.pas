...
type
    tGameMode = record
        ...
        ModeType, SpecialWaveDelay, Maxplayers, MaxSG, KillsForClusts,
        KillsForNades, RespawnTime, MinRespawnTime, MaxRespawnTime: byte;
        Color: integer;
        SpawnMode, Enabled, WeaponSys, AZSS, Sentry, BurningsEat, AZSS_Hard,
        DefaultReloadMode: boolean;
...
begin
    ...
    begin
        ...
        WriteConsole(0, 'Server mode changed to ' + IntToStr(RealMode) +
            ': ' + Mode[RealMode].Name, Mode[Modes.RealCurrentMode].Color);
        //+' (difficulty: '+IntToStr(Modes.DifficultyPercent)+'%)'
    end;
    if Game.MaxPlayers <> Mode[RealMode].Maxplayers then
    begin
        Command('/MAXPLAYERS ' + IntToStr(Mode[RealMode].Maxplayers));
        if Verbose then
            WriteConsole(0, 'Maximal number of players set to ' + IntToStr(
                Mode[RealMode].Maxplayers), YELLOW);
    end;
end;
...
begin
    ...
    begin // or...
        for i := 1 to MAX_MODES do
        begin
            if 1.0 * n[i] >= 0.01 * m * MINVOTEMODEPERCENT then
            begin
    ...
    for i := 1 to MAX_MODES do
    begin
        if n[i] > 0 then str := str + Mode[i].Name + ': ' + IntToStr(n[i]) +
                '/' + IntToStr(m) + ' = ' + IntToStr(100 * n[i] div m) + '%' + #13#10;
        if n[i] > h then
        begin
            h := n[i];
            k := 1;
    ...
    BigText_DrawScreenX(DTL_NOTIFICATION, 0, str, 360, Mode[a].Color,
        0.08, 20, 360 - m * 15);
    startMode := False;
    Modes_Set(a, True);
    Modes_ResetVotes();
...
begin
    ...
    if startMode then
        ...
        begin
            ...
            WriteConsole(0, Players[ID].Name + ' removed his vote for changing gamemode',
                MODEVOTE);
        end;
end;
...
begin
    ...
    WriteConsole(ID, 'To see help about choosen mode do /modehelp <ModeID>, i.e: /modehelp 1',
        GREEN);
end;
...
begin
    if (GameRunning) then
    begin
        WriteConsole(ID, 'You can''t vote during a round. Please wait until the game ends',
            INFORMATION);
        exit;
    end;
    if Config.ForceMode then
    begin
        WriteConsole(ID, 'Can''t change the game mode, feature disabled on this server',
            INFORMATION);
        exit;
    end;
    // check if the vote option exists
    if (modeID > 0) and (modeID <= MAX_MODES) then
    begin
        if Mode[modeID].Enabled then
        begin
            if Players_HumanNum <= Mode[modeID].MaxPlayers then
            begin
                // if the vote got started
                if startMode then
                begin
                    // if the vote got started
                    if not player[ID].ModeReady then
                    begin
                        player[ID].VotedMode := modeID;
                        player[ID].ModeReady := True;
                        str := Players[ID].Name + ' has voted for the ' + Mode[modeID].Name +
                            ' mode [' + IntToStr(Modes_VotedPlayersNum()) + '/' + IntToStr(Players_HumanNum) + ']';
                        WriteDebug(5, str);
                    ...
                    begin
                        WriteConsole(ID,
                            'Your vote has already been counted, type /unmode to cancel your vote',
                            INFORMATION);
                        exit;
                    end;
                end
                ...
                begin
                    ...
                    player[ID].VotedMode := modeID;
                    player[ID].ModeReady := True;
                    WriteConsole(0, Players[ID].Name + ' voted to change mode to ' +
                        Mode[modeID].Name + ' [' + IntToStr(Modes_VotedPlayersNum()) + '/' +
                        IntToStr(Players_HumanNum) + ']', MODEVOTE);
                ...
                WriteConsole(ID, 'Can''t vote this mode, there are too many players in game (' +
                    Mode[modeID].Name + ' max players is ' + IntToStr(Mode[modeID].MaxPlayers) +
                    ')', MODEVOTE);
        end
        else
            WriteConsole(ID, 'Can''t vote this mode, ' + Mode[modeID].Name +
                ' is disabled on this server', MODEVOTE);
    end
    else
        WriteConsole(ID, 'That vote option does not exits', MODEVOTE);
end;
