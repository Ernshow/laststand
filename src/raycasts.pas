...
function RayCast(P1X, P1Y, P2X, P2Y: single;
    CheckPlayerOnly, CheckBulletOnly, CheckColliders: boolean): boolean;

function PointNotInPoly(x, y: single;
    CheckPlayerOnly, CheckBulletOnly, CheckColliders: boolean): boolean;
...
function RayCast3(X1, Y1, X2, Y2: single; NumberOfCasts: integer;
    MaxFails: integer): single;
...
function RayCast(P1X, P1Y, P2X, P2Y: single;
    CheckPlayerOnly, CheckBulletOnly, CheckColliders: boolean): boolean;
begin
    Result := not Map.RayCast(P1X, P1Y, P2X, P2Y, CheckPlayerOnly, False,
        CheckBulletOnly, CheckColliders, 0);
end;

function PointNotInPoly(x, y: single;
    CheckPlayerOnly, CheckBulletOnly, CheckColliders: boolean): boolean;
begin
    Result := not Map.RayCast(x, y, x + 0.01, y + 0.01, CheckPlayerOnly,
        False, CheckBulletOnly, CheckColliders, 0);
end;

function PointsInRange(X, Y, X2, Y2, R: single; Visible: boolean): boolean;
begin
    if SqrDist(X, Y, X2, Y2) <= R * R then
    begin
        if Visible then
...
function RayCast3(X1, Y1, X2, Y2: single; NumberOfCasts: integer;
    MaxFails: integer): single; // returns value 0 - 1 (1 is 100% clearance)
var
    Vx, Vy: single;
    n, Casts: word;
begin
    ...
    begin
        ...
        X1 := X1 + Vx;
        Y1 := Y1 + Vy;
        if not Map.RayCast(X1, Y1, X2, Y2, True, False, False, False, 0) then
            Casts := Casts + 1
        else
            MaxFails := MaxFails - 1;
        if MaxFails <= 0 then
