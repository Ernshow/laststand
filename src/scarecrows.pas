    ...
    begin
        ...
        begin
            ...
            begin
                if player[ID].task = 4 then
                begin
                    if (player[ID].Scarecrows > 0) then
                    begin
                        ...
                        begin
                            ...
                            WriteConsole(ID, 'There is one scarecrow in game', RED);
                    end
                    else
                        WriteConsole(ID, 'You do not have any scarecrows left', RED);
                end
                else
                    WriteConsole(ID, 'You are not the farmer ', RED);
            end
            else
                WriteConsole(ID, 'You have to be on the ground to place a scarecrow', RED);
        end
        else
            WriteConsole(ID, 'You have to be alive to place a scarecrow', RED);
    end
    else
        WriteConsole(ID, 'You are not the farmer', RED);
end;
        ...
        begin
            ...
            if Players[i].Alive then
                if player[i].Zombie then
                    if player[i].ScarecrowLureImmunity = 0 then
                    begin
                        X2 := Players[i].X;
                        Y2 := Players[i].Y - 10.0;
                        dx := X2 - X;
                        ...
                        begin
                            dy := Y2 - Y;
                            if Math.Abs(dy) < 50.0 then
                            begin
                                if RayCast(X, Y, X2, Y2, True, False, False) then
                                begin
                                    if not player[i].Boss then
                                    begin
                                        ...
                                        if player[i].LuredByScarecrow = False then
                                        begin
                                        ...
                                        if r > 10.0 then
                                            if player[i].HurtTime + 2 >= Timer.Value then
                                            begin
