...
begin
    ...
    begin
        case player[ID].task of
            ...
            1: begin
                WriteConsole(ID, 'You are now a Kamikaze zombie. HOLD the [Grenade] key to explode',
                    $FF3728);
            end;
            2: begin
                WriteConsole(ID, 'You are now a Vomiting zombie. HOLD the [Shoot] key to vomit',
                    $99CC33);
            ...
            4: begin
                WriteConsole(ID, 'You are now a Burning zombie. HOLD the [Shoot] key to breathe',
                    $FF7830);
            end;
            5: begin
                WriteConsole(ID,
                    'You are now the Perished Priest. HOLD the [Grenade] key to attack a near enemy',
                    $A070E0);
            end;
            6: begin
                WriteConsole(ID,
                    'You''re now the Undead Firefighter! Hold the key or type the command:',
                    ORANGE);
            ...
            9: begin
                ...
                WriteConsole(ID, ' Hold the [Change Weapon] key near dead zombies to revive them',
                    INFORMATION);
        ...
        case player[ID].task of
            1: begin
                ...
                WriteConsole(ID, 'Type /mre   (default Alt+1) to eat your meal ready to eat',
                    INFORMATION);
                WriteConsole(ID, 'Type /wire  (default Alt+2) to place a barbed wire', INFORMATION);
                WriteConsole(ID,
                    'Type /build (default Alt+3) to build a statgun, /get (Alt+4) to pick one up',
                    INFORMATION);
                WriteConsole(ID, 'Type /sentry to place a sentry gun, /get to pick one up',
                    INFORMATION);
            end;
            2: begin
                WriteConsole(ID, 'You are now the Demolition Expert', WHITE);
                WriteConsole(ID, 'Type /mine (default Alt+1) to place a landmine if you have one',
                    INFORMATION);
                WriteConsole(ID, 'Type /place <time> to place a timed charge', INFORMATION);
                WriteConsole(ID, 'Type /rch  (default Alt+2)to place a remote charge',
                    INFORMATION);
            end;
            3: begin
                WriteConsole(ID, 'You are now the Doctor', WHITE);
                WriteConsole(ID, 'You can heal and revive dead players by switching weapons',
                    INFORMATION);
            ...
            4: begin
                ...
                WriteConsole(ID, 'Type /mre   (default Alt+1) to eat your meal ready to eat',
                    INFORMATION);
                WriteConsole(ID,
                    'Type /scare (default Alt+3) to place a scarecrow (a decoy for zombies)',
                    INFORMATION);
            end;
            5: begin
            ...
            6: begin
                ...
                WriteConsole(ID, 'You are the leader of the group. Your task is to order supplies',
                    INFORMATION);
