...
function BallisticAimX(x, y, v: double; target: tActivePlayer;
    var target_in_range: boolean): double;

function BallisticAimX2(shooter, target: tActivePlayer; velocity, inheirted: double;
    var target_in_range: boolean): double;

//function BallisticAimX32(x0, y0, vx0, vy0, x1, y1, vx1, vy1, ax1, ay1, velocity: double; var target_in_range: boolean): double;

function BallisticCast(x, y, g: single; var vec: tTVector;
    CheckPlayerOnly, CheckBulletOnly: boolean): boolean;
...
begin
    ...
    begin
        ...
        c := cos(a);
        vc := v * c;
        if K_AIR * x / vc >= 1.0 then
        begin
            break;
        end;
        F := x * (s / c + g / (K_AIR * vc)) + (g * Math.ln(1 - kx / vc)) /
            (K_AIR * K_AIR);
        J := (x * (-vc * v + kx * v + g * x * s)) / (vc * c * (kx - vc));
        err := F - y;
        a := a - BA_GAMMA * err / J;
        if abs(err) < BA_EPSILON then
        begin
            break;
        end;
        n := n - 1;
...
function _bax_solve(a, t, xb0, yb0, vxb0, vyb0, xp0, yp0, vpx, vpy,
    accpx, accpy, vb, gravity_factor: double; var target_in_range: boolean): double;
var
    n: integer;
    F, TMP: Vector2;
    J, Ji: Matrix2;
...
begin
    ...
    begin
        ...
        if F.v1 * F.v1 + F.v2 * F.v2 < BA_S_EPSILON then
        begin
...
function _bax(x, y, v, vb0x, vb0y: double; target: tActivePlayer;
    var target_in_range: boolean): double;
var
    a0, t0: double;
    px, py: double;
...
begin
    ...
    ground := ground or Map.RayCast(px, py, px + 10, py + 50, True,
        False, False, False, 0);
    if ((ground) or (target.OnGround)) and (vy > -3.0) then
    begin
        ay := 0;
    end
    else
    ...
    // Aim
    Result := _bax_solve(a0, t0, x, y, vb0x, vb0y, px, py, vx, vy,
        ax, ay, v, gravity, target_in_range);
...
function BallisticAimX2(shooter, target: tActivePlayer; velocity, inheirted: double;
    var target_in_range: boolean): double;
begin
    Result := _bax(shooter.X, shooter.Y - 10, velocity, inheirted *
        shooter.VelX, inheirted * shooter.VelY, target, target_in_range);
end;

function BallisticAimX3(x0, y0, vx0, vy0, x1, y1, vx1, vy1, ax1, ay1, velocity: double;
    var target_in_range: boolean): double;
var
    t, a: double;
begin
    a := _ba_solve(x0, y0, x1, y1, velocity, Game.Gravity / 0.06, t);
    Result := _bax_solve(a, t, x0, y0, vx0, vy0, x1, y1, vx1, vy1,
        ax1, ay1, velocity, Game.Gravity / 0.06, target_in_range);
end;
...
begin
    ...
    begin
        ...
        y := y + vec.vy;
        vec.vy := vec.vy * ik_;
        if Map.RayCast(x, y, x2, y2, CheckPlayerOnly, False, CheckBulletOnly,
            CheckBulletOnly, 0) then
        begin
            vec.X := x2;
            vec.Y := y2;
            exit;
