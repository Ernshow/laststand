...
function INI_Get(var IniFile: tINI; Section, Key, Default: string;
    var FoundKey: boolean): string;
...
begin
    ...
    for i := 0 to Length(IniFile.Section) - 1 do
        if IniFile.Section[i].Name = Section then
        begin
            for j := 0 to Length(IniFile.Section[i].Key) - 1 do
                if IniFile.Section[i].Key[j].Name = Key then
                begin
...
begin
    ...
    begin
        ...
        for j := 0 to Length(IniFile.Section[i].Key) - 1 do
            ffile := ffile + IniFile.Section[i].Key[j].Name + '=' +
                IniFile.Section[i].Key[j].Value + #13#10;
        ffile := ffile + #13#10;
    end;
